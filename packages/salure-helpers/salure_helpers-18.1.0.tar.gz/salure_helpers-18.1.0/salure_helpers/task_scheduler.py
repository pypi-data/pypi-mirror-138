...
class TaskScheduler(object):
    ...
    def start_task(self):
        ...
        """
        # Get the actual run_id, based on the running task in the execution_log table. We set the reload id in the
        # task scheduler itself because then both this code and the task scheduler can use this reload_id
        self.run_id = self.mysql.raw_query(f"SELECT reload_id FROM task_scheduler_log WHERE task_id = {self.task_id} AND reload_status = 'Running' ORDER BY reload_id DESC LIMIT 0, 1;")[0][0]
