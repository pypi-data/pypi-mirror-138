...
class LTML(HTMLParser):
    ...
    def handle_startendtag(self, tag, attrs):
        if tag not in self.TAGS_SE:
            ...
            what = None
            if tag == 'br':
                what = model.TextElement_br()
            else:
                raise NotImplementedError(f'Tag {tag}')  # Should never reach this
    ...
    def handle_data(self, string: str):
        # TODO: Support chunked `feed`, do this on `handle_enddata`
        main_tag = self._ctag
        tags = []
        ...
        if self._stags and main_tag in self.TAGS_SIMPLE and len(tags) == 0 and len(data) == 0:
            ...
            main_tag = None
        self.cmdlist.append(model.TextElement_span(string,
                                                   tag=main_tag,
