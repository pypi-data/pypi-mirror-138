...
def summarise(o: JObj, ctx: Context, title_override: str = ""):
    ...
    if TYPE_KEY in o:
        ...
        if ctx.parent_obj.get(TYPE_KEY) == ARRAY_TYPE and not title_override:
            ctx.write(f"- ({type_description})<br>")
        else:
            title: str = title_override or ctx.current_key
            ctx.write(
                f"- {bold(code(quote(title)))} {bold(italic('REQUIRED*')) if ctx.is_required else ''} ({type_description})<br>"
            )
    elif ENUM_TYPE in o:
        ctx.write(
            f"- {bold(code(quote(ctx.current_key)))} {bold(italic('REQUIRED*')) if ctx.is_required else ''} (type: **`Enum`**)<br>"
        )
    elif any(k in o for k in COMPOSITION_KEYS):
        key = next((k for k in COMPOSITION_KEYS if k in o))
        ctx.write(
            f"- {bold(code(quote(ctx.current_key)))} {bold(italic('REQUIRED*')) if ctx.is_required else ''} (type: {bold(code(key.title()))})<br>"
        )
...
def parse_array_type(o: JObj, ctx: Context):
    ...
    if ITEMS_KEY in o:
        ctx.indent().write(f"**<br>Each item of {ctx.current_key} array must be:<br>**")
        ctx.indent().write("[")
        ctx.write_empty()
...
def parse_properties(o: JObj, ctx: Context):
    ctx.indent().write(f"**<br>Inner Properties of {ctx.current_key}:<br>**")
    ctx.indent().write("{")
    ctx.write_empty()
