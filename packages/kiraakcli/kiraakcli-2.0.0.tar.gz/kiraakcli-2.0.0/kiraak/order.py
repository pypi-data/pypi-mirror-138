...
class OrderProduct:
    def __init__(self, raw) -> None:
        ...
        self.name = raw["name"]
        self.qty = int(raw["qty"])
        self.price = raw["item_price"]
...
class Order:
    def __init__(self, raw) -> None:
        ...
        res = re.match(Config.FLAT_RE, raw["billing_address"])
        if not res:
            raise ParseFlatError(f"Could not parse flat from address {raw['billing_address']}")
        self.flat = f'{res.group("block")}-{res.group("flat")}'
        self.phone = raw["billing_phone"]
        self.total = raw["order_subtotal"]
        self.prods = [OrderProduct(x) for x in raw["products"]]
...
class OrderList:
    def __init__(self, raw) -> None:
        ...
        self.unique_names = set([x.name for y in self.orders for x in y.prods])
