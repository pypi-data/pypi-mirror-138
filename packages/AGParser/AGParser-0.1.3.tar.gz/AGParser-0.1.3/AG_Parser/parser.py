...
class Parser:
    ...
    turism_url = 'https://xn----7sba3acabbldhv3chawrl5bzn.xn--p1ai/displayAccommodation/index?Accommodation%5BFullName%5D=&Accommodation%5BRegion%5D=' + region + '&Accommodation%5BKey%5D=&Accommodation%5BOrganizationId%5D=&Accommodation%5BCertificateNumber%5D=&Accommodation%5BInn%5D=&Accommodation%5BOgrn%5D=&Accommodation%5BSolutionNumber%5D=&yt0=%D0%9D%D00%D09%D1D08&Accommodation%25BFullName%25D=&AccommodationRegion=D0%9C%D0E%D1D0A%D02%D00%25&AccommodationKey=&AccommodationOrganizationId=&AccommodationCertificateNumber=&AccommodationBInn=&AccommodationBOgrn=&AccommodationSolutionNumber=&yt0=%D0%9D%D00%D09%D1D08&Accommodation_page=' + page
    
    r = requests.get(turism_url, headers = headers)
    if status_code == 'on':
        print(r.status_code)
    #print(r.url)
    ...
    for page in range(1, int(max_page) + 1):
        turism_url = turism_url.split('Accommodation_page=')[0]  + 'Accommodation_page=' + str(page)
        if url == 'on':
            print(turism_url)
    ...
    def search_table_url(turism_url_lst, url = 'off', status_code = 'off',headers = {'User-Agent': \
    'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/94.0.4606.81 Safari/537.36'}):
    hotel_url_lst = []
    ...
    def hotel_table_get_df(hotel_url_lst, url = 'off', status_code = 'off'):
    ...
    for url in hotel_url_lst:
        ...
        if len(soup.select('div.detail-field')[7].find_all('span', class_ = 'detail-value')) != 0:
            ...
            ogrn = None
        #postcode = None
        if len(soup.select('div.detail-field')[8].find_all('span', class_ = 'detail-value')[0].text.split(',')) != 0:
