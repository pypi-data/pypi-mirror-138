        ...
        for i, (current_data_e, check_data_e) in enumerate(zip(current_data, check_data)):
            ...
            for x in current_data_e.keys():
                if type(current_data_e[x]) == float:
                    ...
                    self.assertLessEqual(d / current_data_e[x], 0.01, f"Failed on index {i}, key {x}. old value: {current_data_e[x]}, new value {check_data_e[x]}, {current_data_e} vs {check_data_e}")
                else:
                    self.assertEqual(
...
async def new_pipe(p, tb_client: TinyB, replace=False, check=True, populate=False, wait_populate=False, skip_tokens=False, tag='', skip_table_checks=False, ignore_sql_errors=False, only_response_times=False, timeout=None, run_tests=False):  # noqa: C901
    exists = False
    materialized = False
...
async def exec_file(r, tb_client, override, check, debug, populate, wait_populate, tag, skip_table_checks=False, ignore_sql_errors=False, skip_confirmation=False, only_response_times=False, timeout=None, run_tests=False):
