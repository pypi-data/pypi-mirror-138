...
class AutoAuthenticator(AutoEvaluator):
    ...
    def _do_eval_vec(self, possible_thresholds, x_val, y_val):
        ...
        def tester(test_sample_id, possible_thresholds, autoencoders):
            lst = []

            for t in possible_thresholds:
                tp, fp, fn, tn = 0, 0, 0, 0

                for id in [test_sample_id]:  # df['x_val_id'].unique():  # determine closest ae for all samples
                    ...
                    accept: bool = (t <= minimum_threshold)
                    reject: bool = (t > minimum_threshold)
                    ...
                    lst.append({'t': t, 'tp': tp, 'fp': fp, 'tn': tn, 'fn': fn})
            return lst
        ...
        evaluation_lst = Parallel(n_jobs=os.cpu_count())(delayed(tester)(id, possible_thresholds.copy(), autoencoders.copy()) for id in df['x_val_id'].unique())
        evaluation_lst_flat = [item for sublist in evaluation_lst for item in sublist]

        results_df = pd.DataFrame(evaluation_lst_flat)
