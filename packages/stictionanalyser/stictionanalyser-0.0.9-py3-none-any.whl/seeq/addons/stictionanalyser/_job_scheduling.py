...
class JobsTable(v.VuetifyTemplate):
    ...
    logger : a configured logger providing the interface for logging supported by the built-in python logging library
    """
    ...
    def __init__(self, realtime_notebook_url, logger, displayed_columns=None):
        super().__init__()
        self.logger = logger
    ...
    def retrieve_jobs_from_spy(self):
        ...
        if isinstance(self.active_jobs, pd.DataFrame) and not self.active_jobs.empty:
            if not set(self.displayed_columns.values()).issubset(set(self.active_jobs.columns)):
                self.logger.error("The existing jobs do not have all the included displayed columns.")
...
class SchedulingUI(v.VuetifyTemplate):
    ...
    lookback_duration = traitlets.Unicode().tag(sync=True)
    execution_condition = traitlets.Unicode().tag(sync=True)
            ...
            <v-card>
                ...
                <v-container>
                  <v-row no-gutters justify='space-around'>
                    ...
                    <v-col md="2">
                        <v-text-field v-model="lookback_duration" label='Lookback' color='green'
                                      hint="How far back in time the model should execute on startup or after re-starting"></v-text-field> 
                    </v-col>
                    <v-col md="2">
                        <v-spacer/>
                        <v-select :items="conditions" v-model="execution_condition" 
                        label='Execution Condition' color='green'
                        hint="Capsules in which Qsearch should be executed (optional, default All)"></v-select>
                    </v-col>
    ...
    def __init__(self, workbook_id, worksheet_id, realtime_notebook_url, constant_job_parameters, logger):
        super().__init__()
        self.logger = logger
    ...
    def vue_schedule_job(self, _):
        ...
        # Validate the frequency and lookback duration...
        try:
            ...
            duration_parseable = type(pd.to_timedelta(self.lookback_duration.lower())) == pd.Timedelta
            duration_valid = any(duration in self.lookback_duration.lower() for duration in valid_durations)
            ...
            if not duration_parseable:
                raise TypeError('Lookback Error : Provide a valid look back duration, i.e. "3 hours"')
            if not duration_valid:
                raise ValueError('Lookback Error : Must contain a valid duration... i.e. "30 minutes"')
        except Exception as e:
            self.snackbar_msg = str(e)
            self.error_snackbar = True
            return
        ...
        self.logger.info(
            f"""Job scheduled successfully by {spy.user.first_name} {spy.user.last_name}.""")
