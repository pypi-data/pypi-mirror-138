...
#from ._job_scheduling import JobsTable, SchedulingUI
...
def seeq_add_on(sdl_notebook_url):
    def pull_only_signals(url, grid=None):
        ...
        clear_output()
        return df,df_capsule,search_signals_df,search_capsules_df
    ...
    def on_click_push(widget, event, data):
        ...
        try:
            ...
            if checkbox_selected_local.v_model==True:
                global_send=True
            else:
                global_send=False
            ...
            if len(overall_results)>0 and checkbox_selected_stiction.v_model==True:
                ...
                if select_capsule.v_model=="":
                    status=PushSignals(df,overall_results,worksheet_url,name,global_send,workbook_id_splitted) #Pass workbook_id_splitted and global_send 
                else:
                    status=PushSignalsCap(df,overall_results,worksheet_url,name,df[select_capsule.v_model],global_send,workbook_id_splitted)
            else:
                if len(overall_results)<=0 and checkbox_selected_stiction.v_model==True:
                    name=text_field_signal_name.v_model
                    if select_capsule.v_model=="":
                        status=PushSignalsNoOsci(df,worksheet_url,name,global_send,workbook_id_splitted)
                    else:
                        print('here')
                        status=PushSignalsCapNoOsci(df,worksheet_url,name,df[select_capsule.v_model],global_send,workbook_id_splitted)
                        print(status)
            ...
            if len(overall_results_oscillation)>0 and checkbox_selected_oscillation.v_model==True:
                ...
                if select_capsule.v_model=="":
                    status=PushSignalsOsci(df,overall_results_oscillation,worksheet_url,name,global_send,workbook_id_splitted)
                else:
                    status=PushSignalsCapOsci(df,overall_results_oscillation,worksheet_url,name,df[select_capsule.v_model],global_send,workbook_id_splitted)
            else:
                if len(overall_results_oscillation)<=0 and checkbox_selected_oscillation.v_model==True:
                    name=text_field_signal_name_osci.v_model
                    if select_capsule.v_model=="":
    ...
    def on_click_checkbox_local():
        #checkbox_selected_local.v_model=True
        pass
    ...
    worksheet_url_splitted=worksheet_url.split('/')
    global workbook_id_splitted
    workbook_id_splitted=worksheet_url_splitted[-3]
    df,df_capsule,search_signals_df,search_capsules_df=pull_only_signals(worksheet_url)

    #Check if the length of the dataframes match
    if not df_capsule.empty and len(df)-len(df_capsule)!=0:
    ...
    if df.empty:
        ...
        if not df_capsule.empty:
            #Items Conditions
            items_cond = np.intersect1d(search_capsules_df['Name'].values, df_capsule.columns)
            #items_cond=df_capsule.columns
            items_cond=list(items_cond)
    ...
    checkbox_selected_local=v.Checkbox(v_model=False,disabled=False,label='Send Signal with Global Scope')
    ...
    #######
    # scheduling ui widgets (if using Datalab...)
    # _is_data_lab = spy.utils.get_data_lab_project_url().split('/')[-1] != 'None'
    # if _is_data_lab:

    #     datalab_url = spy.utils.get_data_lab_project_url()
    #     realtime_notebook_url = f'{datalab_url}/notebooks/Stiction_RealTime_Notebook.ipynb'

    #     constant_params = {}

    #     _new_jobs_ui = SchedulingUI(workbook_id, worksheet_id,
    #                                     realtime_notebook_url, constant_params, logger=None)

    #     jobs_table_columns = {
    #         'Job Name': 'job_name',
    #         'User': 'user',
    #         'Schedule': 'Schedule',
    #         'Execution Condition': 'execution_condition'
    #     }
    #     _jobs_table = JobsTable(realtime_notebook_url, logger, jobs_table_columns)

    #     # add a callback to update our jobs table when we submit a new job...
    #     _new_jobs_ui.add_submit_callback(_jobs_table.retrieve_jobs_from_spy)

    # # end scheduling ui widgets
    # children = [
    #     _title_box,
    #     _model_box,
    #     _asset_swap_accordion,
    #     _signals_list,
    #     _batch_accordion,
    #     _date_range_box,
    #     _investigation_box,
    ...
    #         v.Card(outlined=True, children=[_jobs_table])]
    #     children.extend(scheduling_widgets)
    # ######
