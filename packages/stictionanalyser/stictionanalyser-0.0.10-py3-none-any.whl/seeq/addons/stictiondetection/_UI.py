...
class CustomData(v.DataTable):
    ...
    def AddItem(self,time_start,time_end,stiction_cases,mean_mag_stic):
        ...
        return self
def UI_function(sdl_notebook_url):
    def pull_only_signals(url, grid=None):
        #login
        url_se='https://explore.seeq.com/login'
        spy.login(url=url_se, access_key='ntrBZhJVSu2a5Y0rPioSow', password='xBIOgM6Jlyion5qIVixhGNH8cBiosU',quiet=True,status=spy.Status(quiet=True))
    ...
    #   get the current path
        #path_compare_img =os.path.abspath(os.getcwd()) 
        #path_compare_img=path_compare_img.replace('dist','seeq/addons/stictiondetection/')
        #path_desk= path_compare_img                    #os.path.expanduser("~/Desktop") + '/'
        #print(path_compare_img)
        results=[]
        osci_index_final=[]
    ...
    container_results=v.Card(class_='',color='grey lighten-3',height='100%',children=[
        ...
        v.Row(class_='pt-4',no_gutters=True,children=[button_to_seeq]),
        v.Row(class_='pb-10',no_gutters=True,children=[button_clear_table])])
