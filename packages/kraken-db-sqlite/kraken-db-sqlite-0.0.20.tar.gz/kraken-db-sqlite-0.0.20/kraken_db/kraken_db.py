...
class Kraken_db:
    ...
    def update_id(self, old_id, new_id):
        ...
        """
        session.execute(update(User).where(DB_observation.record_id == old_id).values(record_id=new_id))
    ...
    def delete(self, observations):
        ...
        if not isinsance(observations, list:
            observations = [observations]

        for i in observations:
            if i.get('observation_id', None):
                session.execute(delete(DB_observation).where(DB_observation.observation_id == i.get('observation_id', None)))
