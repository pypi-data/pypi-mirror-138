...
class Connector:
    def __init__(self, connector_id) -> None:
        ...
        try:
            ...
            self.log_unhandled(f"Error with source code: {str(e)}")
        ...
        if getattr(self._cursor, "close", None):
            self._cursor.close()
    ...
    def log_info(self, message):
        ...
        try:
            ...
            self._exit_unsafe(f"Failed to log message: {str(e)}")
    ...
    def _history_finished(self):
        try:
            ...
            row = self._cursor.fetchone()
            if row:
                self.bytes += int(row.BytesTransferredNBR if row.BytesTransferredNBR else 0)
            else:
            ...
            if self.type == "source":
                ...
                self._cursor.execute("""
                    ...
                    SET
                        ...
                        ,RowInsertedNBR = ?
                    WHERE ETLJobHistoryID = ?
                """, (self._get_datetime_offset(), self.bytes, self.row_count, self._history_id))
            self._cursor.commit()
        except Exception as e:
            self.log_unhandled(f"Failed updating history: {str(e)}")

    def history_fail(self):
        try:
            self._cursor.execute("""
                UPDATE ETLJobHistory
                SET
                    ...
                    ,BytesTransferredNBR = ?
                WHERE ETLJobHistoryID = ?
            """, (self._get_datetime_offset(), self.bytes, self._history_id))
            self._cursor.commit()
        except Exception as e:
            self._exit_unsafe(f"Failed on updating history status: {str(e)}")
        if getattr(self._cursor, "close", None):
            self._cursor.close()

        # SEND FAIL MESSAGE (ONLY PRODUCTION)
        if environ['APP_ENV'] == 'production':
            requests.get(f"https://app.dataconnector.com/api/notification?job_id={self.job_id}&history_id={self.history_id}&key={environ['API_KEY_AUTH']}")
    ...
    def _load_ev(self):
            ...
            try:
                ...
                self._exit_unsafe(f"Unable to start job. Environment variable was not provided: {str(e)}")
