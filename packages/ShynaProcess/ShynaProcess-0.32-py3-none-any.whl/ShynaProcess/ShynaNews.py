...
import ssl
from ShynaProcess import ShynaRegexExtraction
...
class ShynaNews:
    ...
    url = ''
    news_item = {}
    s_reg = ShynaRegexExtraction.ShynaRegexExtraction()
        ...
        for row in entry:
            for _ in row.items():
                ...
                if row['description'] == '':
                    ...
                    self.news_item[row['title']] = self.s_reg.get_link_and_data_from_summary(row['description'])[1], row['id'], news_date, new_time
    ...
    def get_news_zee(self):
        ...
        for row in entry:
            for _ in row.items():
                ...
                self.news_item[row['title']] = self.s_reg.get_link_and_data_from_summary(row['summary'])[1], row['link'],news_date, news_time
        return self.news_item
