...
def stochastic_model_from(
    ...
    subhalo_centre_width:Optional[float]=None,
    subhalo_mass_at_200_log_uniform:bool=True,
) -> af.Collection:
    """
    ...
    This `log_likelihood_cap` is determined by sampling ~250 log likeilhood values from the original model, but where
    ...
    include_pixelization
        If `True` the `Pixelization`'s parameters are fitted for.
    include_regularization
        If `True` the `Regularization`'s parameters are fitted for.
