...
def multi_save_dataset_stations(nc_paths, block_length, block_length_factor: int = 10, compression='zstd', max_workers=3):
    """
    ...
    if max_workers <= 1:
        ...
        for nc_path in nc_paths1:
            new_paths0 = tu.processing.save_dataset_stations(nc_path, block_length * block_length_factor, compression=compression)
        ...
        with concurrent.futures.ProcessPoolExecutor(max_workers=max_workers, mp_context=mp.get_context("spawn")) as executor:
            futures = []
            for nc_path in nc_paths1:
                f = executor.submit(tu.processing.save_dataset_stations, nc_path, block_length * block_length_factor, compression=compression)
                futures.append(f)
...
def multi_merge_nc_remove(source_paths, by, max_workers=3):
    """
    ...
    with concurrent.futures.ProcessPoolExecutor(max_workers=max_workers, mp_context=mp.get_context("spawn")) as executor:
        ...
        for ds_id, paths in source_paths_dict.items():
            f = executor.submit(tu.data_io.merge_nc_remove, paths, by)
            futures.append(f)
        runs = concurrent.futures.wait(futures)
...
class Titan(object):
    ...
    def merge_nc_files(self, source_paths, by, max_workers=3):
        """
        ...
        new_paths = multi_merge_nc_remove(source_paths, by=by, max_workers=max_workers)
    ...
    def save_dataset_stations(self, source_paths, block_length, block_length_factor=10, compression='zstd', max_workers=3):
        """
