...
def save_dataset_stations(nc_path, block_length, compression='zstd'):
    """
    ...
    for c in chunks_list:
        ...
        if compression == 'zstd':
            ...
            obj1 = write_pkl_zstd(b.to_netcdf(), file_path)
            del obj1
        else:
...
def get_result_chunk_data(data, metadata, version_date, system_version=4):
    """

    """
    parameter = metadata['parameter']
    dataset_id = data[parameter].attrs['dataset_id']
    dict1 = {'chunk_id': data['chunk_id'].values.flatten()[0],
             'version_date': pd.Timestamp(version_date),
             'chunk_hash': data['chunk_hash'].values.flatten()[0],
...
def save_final_results(nc_path, metadata, version_date, system_version=4):
    ...
    for c in chunks_list:
        ...
        for b in sm_chunks_list:
            ...
            results_chunk_dict = get_result_chunk_data(d, metadata, version_date, system_version)
