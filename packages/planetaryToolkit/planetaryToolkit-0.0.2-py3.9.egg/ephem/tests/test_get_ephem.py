...
class TestNaif(unittest.TestCase):
    ...
    def test_lookup(self, test_names = ['Neptune', 'despina', 'PUCK'], test_ids = [899, 805, 715]):
...
class TestGetEphem(unittest.TestCase):
    ...
    def test_failcases(self,
        ...
        with self.assertRaises(ValueError):
            get_ephem.get_ephemerides('-999', obscode, tstart, tend, stepsize, quantities)
    ...
    def test_argparse(self,
                        ...
                        stepsize = '30 minutes', 
                        quantities = '1,3,4,8,9,12,13,14,15,17,19,20'):
        args = get_ephem.parse_arguments([code, obscode, tstart, tend, stepsize, quantities])
        self.assertEqual(args.code, code)
        self.assertEqual(args.obscode, obscode)
        self.assertEqual(args.tstart, tstart)
        self.assertEqual(args.tend, tend) # you get a list when nargs = 1, but just a string when nargs = '?'
