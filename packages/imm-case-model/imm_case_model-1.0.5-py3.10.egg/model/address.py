...
class Address(BaseModel):
    address_type:str    # this is the identifier variable 
    type_of_address: str # actually it is the prompt information
    po_box:Optional[str]
    unit:Optional[str]
    ...
    def __eq__(self,another):
        for k,v in self.__dict__.items():
            if k not in ['address_type','type_of_address'] and v != getattr(another,k,None):
                return False
        return True
...
class Addresses(object):
    ...
    def _specific_address(self,address_type):
        address=[address for address in self.addresses if address.address_type==address_type]
