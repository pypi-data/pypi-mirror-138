...
def resolve_feature(missing_feature_type, language='en', is_licensed=False,
    ...
    if '@' not in missing_feature_type:
        ...
        if is_licensed and is_trainable_pipe and missing_feature_type in NlpFeatureResolutions.default_HC_train_resolutions.keys():
            feature_resolution = copy(NlpFeatureResolutions.default_HC_resolutions[missing_feature_type])
            ...
            feature_resolution = copy(NlpFeatureResolutions.default_OCR_resolutions[missing_feature_type])
            ...
            feature_resolution = copy(NlpFeatureResolutions.default_OS_resolutions[missing_feature_type])
        ...
        nlu_component = copy(ComponentMap.os_components[jsl_anno_id])  # TODO <<<<<<<---- Test this??
...
def construct_trainable_component_from_identifier(nlu_ref, nlp_ref, authenticated=False) -> NluComponent:
    ...
    try:
        if anno_id in ComponentMap.os_components.keys():
            nlu_component = copy(ComponentMap.os_components[anno_id])
            ...
            nlu_component = copy(ComponentMap.hc_components[anno_id])
            return nlu_component.set_metadata(nlu_component.get_trainable_model(), nlu_ref, nlp_ref, 'xx', False,
                                              Licenses.hc)
        ...
        # if reference is not in namespace and not a component it will cause a unrecoverable crash
        raise ValueError(
...
def construct_component_from_pipe_identifier(language, nlp_ref, nlu_ref, path=None,
    ...
    for jsl_anno_object in iterable_stages:
        ...
        if anno_class_name in os_annos.keys():
            jsl_anno_id = os_annos[anno_class_name]
            nlu_component = copy(ComponentMap.os_components[jsl_anno_id])
            ...
            nlu_component = copy(ComponentMap.hc_components[jsl_anno_id])
            ...
            nlu_component = copy(ComponentMap.ocr_components[jsl_anno_id])
