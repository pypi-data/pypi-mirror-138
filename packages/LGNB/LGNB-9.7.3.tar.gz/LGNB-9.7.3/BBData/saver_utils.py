...
class AlphaUpdater(FeatureUpdater):
    ...
    def __init__(self, stk_univ="all", start_time=None, end_time=None, freq="Tday", not_save_primarykey=False):
        super(AlphaUpdater, self).__init__(stk_univ, start_time, end_time, freq, not_save_primarykey)

    @property
    def _uri_data(self):
        """Static feature file uri."""
        return os.path.join(data_path, "alphas", "{}", "{}.h5")

    @property
    def _uri_cfg(self):
        """Static feature file uri."""
        return os.path.join(data_path, "alphas", "{}", "{}_config.json")
