...
def _replace_missing_values(value: str):
    value = value.replace(f"{DELIMITER}{MISSING_VALUE}", f"{DELIMITER}{MISSING}")
    # handle missing value at begining of the row
    return value.replace(MISSING_VALUE, str(MISSING), 1) if value.startswith(MISSING_VALUE) else value
...
def _rewrite_csv_file_as_tab(filepath: str):
    with open(filepath, 'r', encoding=ENCODING) as fp:
        ...
        for row in reader:
            yield _replace_missing_values(DELIMITER.join(row))
