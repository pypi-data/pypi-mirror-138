...
### OSPF broken - asking for pyATS help .info doesn't exist ? Weird
...
def IOS_learned_stp(hostname, username, password, ip):
    ...
    # Create Testbed
        ...
        if learned_stp is not None:
            ...
            IOS_learned_stp_mstp_template = env.get_template('IOS_learned_mstp_rpvst.j2')
            loop_counter = 0
        # Render Templates
            for filetype in filetype_loop:
    ...
    # -------------------------
                    if loop_counter <= 3:
                        ...
                        with open(f"{ filename }_Learn Rapid Per VLAN Spanning Tree Mind Map.md", "w") as fh:
                            fh.write(parsed_output)               
                            fh.close()
                    with open(f"{ filename }_Rapid Per VLAN Spanning Tree.json", "w") as fh:
                        json.dump(learned_stp, fh, indent=4, sort_keys=True)
                        fh.close()
        ...
        # Render Templates
            if learned_stp['mstp']: 
                for filetype in filetype_loop:
                    parsed_output = IOS_learned_mstp_template.render(to_parse_stp=learned_stp['mstp'],filetype_loop=loop_counter)
                    loop_counter = loop_counter + 1
    ...
    # -------------------------
                    if loop_counter <= 3:
                        ...
                        with open(f"{ filename }_Learn Spanning Multiple Tree Mind Map.md", "w") as fh:
                            fh.write(parsed_output)               
                            fh.close()
                    with open(f"{ filename }_Spanning Tree Multiple.json", "w") as fh:
                        json.dump(learned_stp, fh, indent=4, sort_keys=True)
                        fh.close()                        
        return(learned_stp)
    except Exception as e:
