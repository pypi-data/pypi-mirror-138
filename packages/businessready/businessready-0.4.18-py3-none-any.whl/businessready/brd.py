...
def NXOS_all(hostname, username, password, ip):
    NXOS_learned_all(hostname, username, password, ip)
...
def NXOS_learned_all(hostname, username, password, ip):
    NXOS_learned_acl(hostname, username, password, ip)
    NXOS_learned_arp(hostname, username, password, ip)
    NXOS_learned_bgp(hostname, username, password, ip)
    NXOS_learned_hsrp(hostname, username, password, ip)
    NXOS_learned_interface(hostname, username, password, ip)
    NXOS_learned_ospf(hostname, username, password, ip)
    return("Learned All Functions")
...
def NXOS_learned_bgp(hostname, username, password, ip):
    try:
    # Create Testbed
        filename = hostname
        first_testbed = Testbed('dynamicallyCreatedTestbed')
        ...
        if learned_bgp != f"{ hostname } has no BGP to Learn":
            if "instance" in learned_bgp:
                NXOS_learned_bgp_template = env.get_template('NXOS_learned_bgp.j2')
                loop_counter = 0
        # Render Templates
                for filetype in filetype_loop:
                    parsed_output_statistics = NXOS_learned_bgp_template.render(to_parse_bgp=learned_bgp['instance'],filetype_loop=loop_counter)
                    loop_counter = loop_counter + 1
        # -------------------------
        # Save the files
        # -------------------------
                ...
                learned_bgp = f"{ hostname } has no BGP Instances"
        return(learned_bgp)
    except Exception as e:
        logging.exception(e)

def NXOS_learned_hsrp(hostname, username, password, ip):
    try:
    # Create Testbed
        filename = hostname
        first_testbed = Testbed('dynamicallyCreatedTestbed')
        ...
        if learned_hsrp != f"{ hostname } has no Interface to Learn":
            NXOS_learned_hsrp_template = env.get_template('NXOS_learned_hsrp.j2')
            loop_counter = 0
        # Render Templates
            for filetype in filetype_loop:
                parsed_output = NXOS_learned_hsrp_template.render(to_parse_hsrp=learned_hsrp,filetype_loop=loop_counter)
                loop_counter = loop_counter + 1
...
def NXOS_learned_interface(hostname, username, password, ip):
    try:
    # Create Testbed
        filename = hostname
        first_testbed = Testbed('dynamicallyCreatedTestbed')
        ...
        if learned_interface != f"{ hostname } has no Interface to Learn":
            NXOS_learned_interface_template = env.get_template('NXOS_learned_interface.j2')
            loop_counter = 0
        # Render Templates
            for filetype in filetype_loop:
                parsed_output = NXOS_learned_interface_template.render(to_parse_interface=learned_interface,filetype_loop=loop_counter)
                loop_counter = loop_counter + 1
...
def NXOS_learned_ospf(hostname, username, password, ip):
    try:
    # Create Testbed
        filename = hostname
        first_testbed = Testbed('dynamicallyCreatedTestbed')
        ...
        if learned_ospf != f"{ hostname } has no OSPF to Learn":
            NXOS_learned_interface_template = env.get_template('NXOS_learned_ospf.j2')
            loop_counter = 0
        # Render Templates
            for filetype in filetype_loop:
                parsed_output = NXOS_learned_interface_template.render(to_parse_ospf=learned_ospf,filetype_loop=loop_counter)
                loop_counter = loop_counter + 1
