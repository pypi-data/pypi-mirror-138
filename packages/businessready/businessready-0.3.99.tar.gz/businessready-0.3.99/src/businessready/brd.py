...
def IOS_learned_all(hostname, username, password, ip):
    ...
    IOS_learned_platform(hostname, username, password, ip)    
    ...
    # Create Testbed
        ...
        # Learn HSRP to JSON
            ...
            try:
                ...
                learned_hsrp = f"{ hostname } has no Interface to Learn"
...
def IOS_learned_platform(hostname, username, password, ip):
    try:
    # Create Testbed
        filename = hostname
        first_testbed = Testbed('dynamicallyCreatedTestbed')
        ...
        # Learn Platform to JSON
            ...
            try:
                learned_platform = device.learn("platform").info
            except:
                learned_platform = f"{ hostname } has no OSPF to Learn"

        # Pass to template 

        if learned_platform != f"{ hostname } has no OSPF to Learn":
            IOS_learned_platform_template = env.get_template('IOS_learned_platform.j2')
            loop_counter = 0
        # Render Templates
            for filetype in filetype_loop:
                parsed_output = IOS_learned_platform_template.render(to_parse_platform=learned_platform,filetype_loop=loop_counter)
                loop_counter = loop_counter + 1
    ...
    # -------------------------
                ...
                with open(f"{ filename }_Learn Platform.json", "w") as fh:
                    json.dump(learned_platform, fh, indent=4, sort_keys=True)
                    fh.close()
        return(learned_platform)
    except Exception as e:
        logging.exception(e)
