...
class Service:
    ...
    @staticmethod
    def async_requests(async_requests):
        # accepts of list of async_requests that are each wrapped by a lambda function
        # ex async_requests = [lambda: Service().async_get_users('123')]
        # you may also include dicts in the async_requests list that describe a syncronous request
        # ex async_requests = [{"request": Service().get_user, "args": ("123"), "kwargs": {}}]
        event_loop = asyncio.get_event_loop()
        return event_loop.run_until_complete(Service._async_requests(async_requests))

    @staticmethod
    async def _async_requests(async_requests):
        # this method will start all async_requests then await for them all to finish
        async with aiohttp.ClientSession() as async_session:
            Service.async_session = async_session
            futures = []

            for async_request in async_requests:
                if type(async_request) == dict:
                    # call synchronous methods as async and add them to the list of futures
                    futures.append(
                        asyncio.ensure_future(
                            Service.asyncify(async_request, async_session)
                        )
                    )
                else:
                    # call async methods and add them to the list of futures
                    futures.append(
                        asyncio.ensure_future(
                            async_request()
                        )
    ...
    @staticmethod
    async def asyncify(sync_request, async_session):
        # get the pieces of the request
        request = sync_request.get('request')
        request_args = tuple(sync_request.get('args'))
        request_kwargs = dict(sync_request.get('kwargs'))

        # unbind the synchronous invoke method
        # this is so the request method will not actually make an http call
        invoke = request.__self__.invoke
        request.__self__.invoke = lambda: None

        # synchronously call the request method
        # this will not actually send an http call since it was unbound above
        request(*request_args, **request_kwargs)

        # call async_invoke on the request methods object
        response = await request.__self__.async_invoke()

        # rebind the synchronous invoke
        request.__self__.invoke = invoke
    ...
    async def async_invoke(self):
        # this method makes the async request and returns a created Response object
        async with Service.async_session.request(
                self.request_method,
                self.request_url,
                params=self.request_parameters,
                headers=self.request_headers,
            ...
            async_response_data = await async_response.json()
            ...
            response = self.Response(
                ...
                cookies=dict(async_response.cookies),
