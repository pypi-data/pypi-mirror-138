...
class Sequential():
    ...
    def live_stats(self,ls_type, figure, F1=0, F2=1, n_row="limited"):
        ...
        if ls_type == 1:
            ...
            if len(self.train_inputs[0,:]) <= 2 and len(self.train_targets[0,:]) == 1:
                if len(self.train_inputs[:,0]) > 1000 and n_row=="limited":
                    X_set, y_set = self.train_inputs[:1000,:], self.train_targets[:1000,:]
                elif len(self.train_inputs[:,0]) > 1000 and n_row=="full":
                    X_set, y_set = self.train_inputs, self.train_targets
                elif (len(self.train_inputs[:,0]) <= 1000 and n_row=="full") or (len(self.train_inputs[:,0]) <= 1000 and n_row=="limited"):
                    X_set, y_set = self.train_inputs, self.train_targets
                else:
                    raise ValueError("invalid value for 'n_row'")
            ...
            if len(self.train_inputs[:,0]) > 1000 and n_row=="limited":
                plt.scatter(self.train_inputs[:1000,F1], self.train_inputs[:1000,F2], c=self.train_outputs[:1000,:], cmap="coolwarm", alpha=1)
            elif len(self.train_inputs[:,0]) > 1000 and n_row=="full":
                plt.scatter(self.train_inputs[:,F1], self.train_inputs[:,F2], c=self.train_outputs[:,:], cmap="coolwarm", alpha=1)
            elif (len(self.train_inputs[:,0]) <= 1000 and n_row=="full") or (len(self.train_inputs[:,0]) <= 1000 and n_row=="limited"):
                plt.scatter(self.train_inputs[:,F1], self.train_inputs[:,F2], c=self.train_outputs[:,:], cmap="coolwarm", alpha=1)
            else:
                raise ValueError("invalid value for 'n_row'")
    ...
    def fit(self, cost_fn, epochs = 1000, lr = 0.1, momentum = None, verbose = 1, live_stats = {"type": 0, "F1": 0, "F2": 1, "n_row": "limited"}):
        for n in [-1,0,1,2]:
            if type(live_stats) == dict:
                if live_stats["type"] == n:
                    break
                elif n == 2 and live_stats["type"] != n:
                    raise ValueError("unknown value for live_stats['type']")
            elif type(live_stats) == int:
                if live_stats == n:
                    break
        ...
        if type(live_stats) == dict:
            if live_stats["n_row"] != "full" and live_stats["n_row"] != "limited":
                raise ValueError("unknown value for live_stats['n_row']")
        ...
        if type(live_stats) == int:
            live_stats = {"type": live_stats, "F1": 0, "F2": 1, "n_row": "limited"}
        ...
        if verbose == 1:
            for epoch in tqdm(range(epochs)):
                ...
                if live_stats != 0:
                    if epoch % update == 0:
                        self.live_stats(ls_type = live_stats["type"], figure = figure, F1 = live_stats["F1"], F2 = live_stats["F2"], n_row = live_stats["n_row"])
