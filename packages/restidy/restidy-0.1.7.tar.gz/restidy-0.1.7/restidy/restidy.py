...
def main():
    ...
    join("db/gene_db_mapping.tsv")
    # print("Script path:", script)
    mapping_file = join("db/gene_db_mapping.tsv")
    ...
    print(output_file)
    df = pd.read_csv(input_file, sep='\t', names=['Strain', 'Resistance gene', 'Identity', 'Alignment Length/Gene Length',
                                                  'Coverage', 'Position in reference', 'Contig',  'Position in contig', 'Phenotype', 'Accession no.'])
