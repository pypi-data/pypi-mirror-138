...
class Brain:
    ...
    def init(self, mode):
        ...
        tf.autograph.set_verbosity(Config.get("NORMALIZATION_ENABLED"))
        self.model = load_model(mode)

    def predict(self, image):
        prediction = self.model.predict(image)
        result = {
            True: Config.get("RESULT_MAP")["WANDER"],
            False: Config.get("RESULT_MAP")["NORMAL"]
        }
        prediction = [prediction > Config.get("RESULT_THRESHOLD")]
