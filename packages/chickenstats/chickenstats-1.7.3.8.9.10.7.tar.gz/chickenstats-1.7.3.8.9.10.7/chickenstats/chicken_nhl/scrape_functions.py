...
def scrape_html_events(game_ids, _print = True, roster = pd.DataFrame(), game_data = pd.DataFrame(), session = None):
    ...
    for game_id_idx, game_id in enumerate(game_ids):
        ...
        for old_name, new_name in NEW_TEAMS_DICT.items():
            ...
            events_df.description = events_df.description.str.replace(old_name, new_name, regex = False)
...
def get_gamestats(pbp, roster, game_info):
    ...
    stats_dict = {x: 'sum' for x in stats_list}

    players = [x for x in game.columns if ('event_on' in x or 'opp_on' in x)]
    ...
    for player in players:
        ...
        group_list = ['season', 'game_id', 'game_date', 'event_team', 'opp_team', 'opp_goalie', 'own_goalie', player, 'period', 'strength_state']
    ...
    group_list = ['season', 'game_id', 'game_date', 'team', 'player', 'period', 'strength_state', 'opp_goalie', 'own_goalie',]
    ...
    stats_dict = {x: 'sum' for x in stats}
    ...
    for player in players:
        ...
        group_list = ['season', 'game_id', 'game_date', 'event_team', 'opp_team', player, 'period', 'strength_state', 'own_goalie', 'opp_goalie']
        ...
        if player == 'event_player_1_api_name':
            ...
            drop_list = [x for x in stats if x not in new_cols.keys()]
        ...
        if player == 'event_player_2_api_name':
            ...
            merge_list = ['season', 'game_id', 'game_date', 'team', 'opp_team', player, 'period', 'strength_state', 'own_goalie', 'opp_goalie']
    ...
    ind_stats = pd.DataFrame(columns = ['season', 'game_id', 'game_date', 'team', 'player','period', 'strength_state', 'own_goalie', 'opp_goalie'])

    for df in concat_list:

        merge_list = ['season', 'game_id', 'game_date', 'team', 'player','period', 'strength_state', 'own_goalie', 'opp_goalie']

        ind_stats = ind_stats.merge(df, left_on = merge_list, right_on = merge_list, how = 'outer').fillna(0)
        
    ## Zone starts
    
    zone_cols = ['season', 'game_id', 'game_date', 'event_team', 'period', 'strength_state', 'own_goalie', 'opp_goalie', 'OFF', 'DEF', 'NEU', 'OTF']
    ...
    for player in players:
        ...
        group_list = ['season', 'game_id', 'game_date', 'event_team', player, 'period', 'strength_state', 'own_goalie', 'opp_goalie']
    ...
    group_list = ['season', 'game_id', 'game_date', 'team', 'player', 'period', 'strength_state', 'own_goalie', 'opp_goalie']
    ...
    merge_list = ['season', 'game_id', 'game_date', 'team', 'player', 'period', 'strength_state', 'opp_goalie', 'own_goalie']
    ...
    columns = ['season', 'game_id', 'game_date', 'team', 'player','position', 'opp_team', 'period', 'strength_state',
...
def scrape_pbp(game_ids, gamestats = False, _print = True):
    ...
    for game_id_idx, game_id in enumerate(game_ids):
        ...
        pbp_df['opp_goalie'] = np.where(pbp_df.is_home == 1, pbp_df.away_goalie, pbp_df.home_goalie).fillna('EMPTY NET')

        pbp_df['own_goalie'] = np.where(pbp_df.is_home == 1, pbp_df.home_goalie, pbp_df.away_goalie).fillna('EMPTY NET')
