...
def _get_kwargs(
    *,
    client: Client,
    json_body: PoolV1GetGeographicDistributionRequest,
) -> Dict[str, Any]:
    url = "{}/api/v1/pool/get_geographic_distribution".format(client.base_url)
...
) -> Optional[Union[GoogleRpcStatus, PoolV1GetGeographicDistributionResponse]]:
    if response.status_code == 200:
        response_200 = PoolV1GetGeographicDistributionResponse.from_dict(
            response.json()
        )
...
def _build_response(
    *, response: httpx.Response
) -> Response[Union[GoogleRpcStatus, PoolV1GetGeographicDistributionResponse]]:
    return Response(
