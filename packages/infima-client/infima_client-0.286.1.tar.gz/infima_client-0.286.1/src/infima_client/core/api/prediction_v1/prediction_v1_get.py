...
def _get_kwargs(
    *,
    client: Client,
    json_body: PredictionV1GetRequest,
) -> Dict[str, Any]:
    url = "{}/api/v1/prediction/get".format(client.base_url)
...
) -> Optional[Union[GoogleRpcStatus, PredictionV1GetResponse]]:
    if response.status_code == 200:
        response_200 = PredictionV1GetResponse.from_dict(response.json())
...
def _build_response(
    *, response: httpx.Response
) -> Response[Union[GoogleRpcStatus, PredictionV1GetResponse]]:
    return Response(
...
def sync_detailed(
    *,
    ...
    """Fetch pool and cohort prepayment predictions for an as-of date, default to today.
