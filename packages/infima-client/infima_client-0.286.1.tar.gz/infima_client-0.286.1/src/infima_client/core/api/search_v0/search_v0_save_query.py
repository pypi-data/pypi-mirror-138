...
def _get_kwargs(
    *,
    client: Client,
    json_body: SearchV0SaveQueryRequest,
) -> Dict[str, Any]:
    url = "{}/api/v0/search/save_query".format(client.base_url)
...
) -> Optional[Union[GoogleRpcStatus, SearchV0SaveQueryResponse]]:
    if response.status_code == 200:
        response_200 = SearchV0SaveQueryResponse.from_dict(response.json())
...
def _build_response(
    *, response: httpx.Response
) -> Response[Union[GoogleRpcStatus, SearchV0SaveQueryResponse]]:
    return Response(
