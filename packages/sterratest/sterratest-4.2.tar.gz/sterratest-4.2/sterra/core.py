        ...
        if kwargs.get("only_usernames"):
            ...
            # else:
                ...
                if len(__a1List) != len(flist): # Export en part
                    value = exman(_).part(targlist, part_path)
                    try:
                        pid, file_path, _t = value
                    except ValueError:
                        pid, file_path = value
                    exporter(_, __a1List, file_path, Format="json")()
                    _.r(RateLimitError(f"Part file exported under id: {str(bold(pid))}"),cont=True)
                    _.p("Check this link for more -> https://github.com/novitae/sterraxcyl/wiki/Export#-parts",logo="Interrogation",ext=True)
