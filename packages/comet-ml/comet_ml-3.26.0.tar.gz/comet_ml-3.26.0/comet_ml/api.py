...
class APIExperiment(CommonExperiment):
    ...
    def download_tensorflow_folder(self, output_path="./", overwrite=False):
        """
        Download a tensorflow directory logged with
        `Experiment.log_tensorflow_folder(FOLDER)`.

        Args:
            folder: (str) the folder to download
            overwrite: (bool) if True, then overwrite any file that exists
            quiet: (bool) if True, then don't report on each file
        ...
        >>> api_experiment.download_tensorflow_folder("logs")
        ```
        """
