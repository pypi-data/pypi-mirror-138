...
def project_dev_vars(project_path: str = "") -> PdvType:
    ...
    elif tpl_projects and tpl_projects[0]['import_name'] == namespace_name + '_' + namespace_name:
...
def _register_project_templates(project_templates_packages_versions: List[str]):
    ...
    for package_sep_version in project_templates_packages_versions:
        ...
        import_name = '.'.join(pkg_name.split('_', maxsplit=1))
...
def _register_template_project(import_name: str, project_path: str = "", tpl_ver: str = "", tpl_root: str = "") -> str:
    ...
    cae.vpo(f"    - registered {import_name} package {tpl_ver} in {project_path} as template id '{key}'")

    REGISTERED_TPL_PROJECTS[key] = {
        'import_name': import_name,
        'pkg_path': os.path.join(project_path, *import_name.split('.')),
        'version': tpl_ver,
    }
...
def _renew_root_req_file(pdv: PdvType):
    ...
    root_prj_path = os.path.dirname(os.path.dirname(REGISTERED_TPL_PROJECTS.get(root_imp_name, {}).get('pkg_path', "")))
    if not root_prj_path:
        cae.po(f"   ## {root_imp_name} namespace root is not registered as template in {PPF(REGISTERED_TPL_PROJECTS)}")
        root_prj_path = os.path.join(os.path.dirname(pdv_str(pdv, 'project_path')), norm_name(root_imp_name))
...
def _template_projects(pdv: PdvType) -> List[RegisteredTemplateProject]:
    ...
    root_imp = namespace_name + '.' + namespace_name
    root_pkg = norm_name(root_imp)
    ...
    tpl_packages_versions = [_ for _ in dev_require if _ == root_pkg or _.startswith(tpl_pkg_name_prefix)]
    _register_project_templates(tpl_packages_versions)
    cae.vpo(f"  --- {project_name} registered {len(REGISTERED_TPL_PROJECTS)} template projects:")
    ...
    tpl_candis = [root_imp, TPL_IMPORT_NAME_PREFIX + norm_name(project_type), TPL_IMPORT_NAME_PREFIX + 'project']
...
def check_children_integrity(parent_pdv: PdvType, *children_pdv: PdvType):
    ...
    for chi_pdv in children_pdv:
        cae.dpo(f" ---  {pdv_str(chi_pdv, 'package_name')}  ---  {pdv_str(chi_pdv, 'project_desc')}")
...
def main():                                                                                 # pragma: no cover
    ...
    cae.add_option('namespaces', "namespaces to register (for templates)", ['ae_ae', 'aedev_aedev'], short_opt='s')
