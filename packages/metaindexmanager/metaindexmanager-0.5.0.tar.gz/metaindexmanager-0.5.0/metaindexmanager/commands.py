...
class SetCommand(Command):
    ...
    def completion_options(self, context, *args):
        logger.debug("Completion base: %s", args)
...
class RunIndexers(Command):
    ...
    def run_indexers(blocker, application, panel, path):
        ...
        results = metaindex.indexer.index_files(paths,
                                                ...
                                                fulltext=False,
                                                config=application.metaindexconf)
        if len(results) == 0:
            return
        ...
        for idx, result in enumerate(results):
            ...
            base = application.cache.get(lpath, False)
            if len(base) == 0:
