...
class NmfModel:
    def __init__(self, mat: Union[np.ndarray, pd.DataFrame], ncomp: int, nblocks: int = 1, name: str = "",
                 beta_loss: str = "frobenius", leverage: str = None, kernel: str = "linear", tol: float = 1e-6,
...
def generate_scree_plot(mat: Union[np.ndarray, pd.DataFrame], ncmin: int, ncmax: int, known_ncomp=0, sparsity=0.0,
                        do_entropies: bool = False, do_stabilities: bool = True) -> pd.DataFrame:
    """
    ...
    nmfmodel.set_sparsity(sparsity)
    n, f = mat.shape
