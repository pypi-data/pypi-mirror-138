...
def validate_instance_type(
    ...
    for instance_type in instance_type_list:
        if not isinstance(instance_type, str):
            raise TypeError(
                f"Instance type provided '{instance_type}' should be a string."
            )
...
def get_maximum_core_count(user_vm_types: list, all_vm_types: dict) -> int:
    """Get maxmimum core count from a list of vm types.

    This is a very basic function that returns the maximum of core coiled_accounts
    for a given list of vm types. The reason why this function exists is mostly so
    we can test this logic.

    Note that we are being defensive here, but the instances already been validated
    and the instances that the user specified will be in ``all_vm_types``. The
    values should also be set, although we need to give a default of 1 if the cores
    key isn't found because using 0 seems to trigger a declarative path in our
    backend.

    """
    max_core_count = max(
        [all_vm_types.get(instance, {}).get("cores", 1) for instance in user_vm_types]
    )
