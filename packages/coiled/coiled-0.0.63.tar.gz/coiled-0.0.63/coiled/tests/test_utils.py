...
def test_validate_instance_type_wrong_type(vm_types, api_instance_types_gcp):
    with pytest.raises(TypeError, match="should be a string"):
        validate_instance_type(
            instance_type_list=vm_types,
...
@pytest.mark.parametrize(
    "user_vm_types,expected",
    [
        (["t2d-standard-8"], 8),
        (["t2d-standard-32", "t2d-standard-8"], 32),
        (["non-existent"], 1),
    ],
)
def test_get_maximum_core_count(user_vm_types, expected, api_instance_types_gcp):
    max_cores = get_maximum_core_count(user_vm_types, api_instance_types_gcp)

    assert max_cores == expected
