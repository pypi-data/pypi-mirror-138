...
class Cluster(distributed.deploy.Cluster, Generic[IsAsynchronous]):
    ...
    async def _start(self):
        ...
        with console.status(
            ...
            if should_create:
                if self.worker_vm_types or self.scheduler_vm_types:
                    ...
                    if self.worker_vm_types:
                        ...
                        validate_instance_type(
                            instance_type_list=self.worker_vm_types,
                            allowed_instance_types=user_instance_types,
                            provider_name=provider_name,
                        )
                        self.worker_cpu = get_maximum_core_count(
                            self.worker_vm_types, user_instance_types
                        )

                    if self.scheduler_vm_types:
                        validate_instance_type(
                            instance_type_list=self.scheduler_vm_types,
                            allowed_instance_types=user_instance_types,
                            provider_name=provider_name,
                        )
                        self.scheduler_cpu = get_maximum_core_count(
                            self.scheduler_vm_types, user_instance_types
                        )
            ...
            if should_create and available_configs:
                try:
                    ...
                    if worker_error:
                        ...
                        console.print(msg)
                        return
                if len(self.scheduler_info["workers"]) < self._start_n_workers:
                    message = (
                        "Unable to get all the requested workers, please run the command "
                        "coiled.get_notifications(level='ERROR') for more information."
                    )
