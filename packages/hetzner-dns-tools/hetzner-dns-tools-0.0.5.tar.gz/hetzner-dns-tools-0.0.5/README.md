...
- After the project finishes building, press `Ctrl + C` to exit the logs, and open a bash shell in the same container: `docker exec -it hetzner-dns-tools bash`
  - You can exit the logs before the build completes. However, `hetzner-dns-tools` will not be available from the command line until the build is complete. It doesn't take too long.
