...
class CustomDateTimeRangeFilter(DateTimeRangeFilter):
    def queryset(self, request: WSGIRequest, queryset):
        if self.form.is_valid():
            log_time_gte = str(
                int(self.form.cleaned_data["logTime__range__gte"].timestamp())
            )
            log_time_lte = str(
                int(self.form.cleaned_data["logTime__range__lte"].timestamp())
            )
            if log_time_gte and log_time_lte:
                return queryset.filter(logTime__range=(log_time_gte, log_time_lte))
        return queryset
...
class PurchaseAdmin(admin.ModelAdmin):
    ...
    list_filter = (
        ...
        ("logTime", CustomDateTimeRangeFilter),
    )
    ...
    def time(self, obj):
        return datetime.fromtimestamp(int(obj.logTime)).strftime("%Y-%m-%d %H:%M:%S")
