...
class Guest(Client):
	...
	def infrastructure_user_add(self, nInfrastructureID, strUserEmail, objInfrastructurePermissions = None, bCreateUserIfNotExists = False):
	...
	def user_delegate_children(self, strUserID):
		...
		arrUsers = self.rpc("user_delegate_children", arrParams)
		for index in range(len(arrUsers)):
			arrUsers[index] = Deserializer.deserialize(arrUsers[index])
		return arrUsers
	...
	def user_delegate_parents(self, strUserID):
		...
		arrUsers = self.rpc("user_delegate_parents", arrParams)
		for index in range(len(arrUsers)):
			arrUsers[index] = Deserializer.deserialize(arrUsers[index])
		return arrUsers
