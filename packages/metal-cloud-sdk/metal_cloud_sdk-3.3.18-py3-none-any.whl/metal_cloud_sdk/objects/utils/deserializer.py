...
class Deserializer(object):
    ...
    def _recursive_decode(mxJSON):
        ...
        if isinstance(mxJSON, list):
            ...
            if "type" in mxJSON:
                # BSI-7279 Edge case: mxJSON["type"] has type JWT in the case of CookieLogin. We must swap back to CookieLogin
                responseType = mxJSON["type"]
                if responseType == "JWT":
                    responseType = "CookieLogin"

                module = importlib.import_module('metal_cloud_sdk.objects.' + responseType.lower())
                return Deserializer._decode(mxJSON, getattr(module, responseType))
