...
def _setup_env_for_multinode(required=False):
    if not _setup_env_for_multinode._setup_done:
        size = comm_size()
        rank = comm_rank()
        ...
        if hcl_type in SUPPORTED_HLS_TYPE:
            ...
            if is_hierarchical():
                os.environ["HLS1_MODULE_ID"] = str(
                    HLS_RANK_MODULE_BINDINGS[hcl_type][str(comm_local_rank())])
        ...
        _setup_env_for_multinode._setup_done = True


_setup_env_for_multinode._setup_done = False
...
def get_hw_module_id(rank, hls_type="HLS1"):
    if "HLS1_MODULE_ID_LIST" in os.environ:
        return os.environ["HLS1_MODULE_ID_LIST"][rank]
    local_rank = str(comm_local_rank())
