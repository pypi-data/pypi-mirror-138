...
class SynapseLoggerConfig:
    ...
    @property
    def config_ok(self):
        if self.CONFIG_ENV_VARIABLE_NAME not in os.environ or self.RANGE_ENV_VARIABLE_NAME not in os.environ:
            _log.warning("\n".join(
                ["Configuration for Habana Profiler is not provided in environment!",
                 f"Please set {self.CONFIG_ENV_VARIABLE_NAME} and {self.RANGE_ENV_VARIABLE_NAME} accordingly.",
                 "Example:",
                 f"\texport {self.CONFIG_ENV_VARIABLE_NAME}=hw",
                 f"\texport {self.RANGE_ENV_VARIABLE_NAME}=100-101"]))
            return False
        if not self.required:
            _log.warning("\n".join[
                "Neither 'hw' nor 'synapse' provider has been choosen.",
                f"Please set {self.CONFIG_ENV_VARIABLE_NAME}.",
                f"Available options (use comma for choosing multiple values):",
                f"\t hw",
                f"\t synapse"
            ])
            return False
