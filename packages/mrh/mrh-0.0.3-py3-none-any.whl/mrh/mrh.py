...
def cm(label_true,label_pred,print_note=True):
    ...
    normal_conf_mat: normalized confusion matrix
    ...
    if print_note:
        print('This Conf matrix has one extra row (NTL) & one extra column (NPL).')

    return conf_mat, normal_conf_mat


# ########################################################################### #
# Produce some statistics based on the 2D confusion matrix
# ########################################################################### #
def stats(conf_mat, print_binary_mat=True):
    '''
    Computes one-vs-rest confusion matrices for all classes of data
    ...
    divide = conf_mat.sum(axis=1, dtype='int64') # sum of each row
