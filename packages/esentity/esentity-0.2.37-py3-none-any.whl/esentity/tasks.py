...
def process_tg_message(domain, bot_id, message):
    ...
    if _data:
        ...
        if 'new_chat_title' in message or 'left_chat_member' in message or 'new_chat_member' in message:
            telegram_api(_jobs['token'], 'deleteMessage', json={'chat_id': message['message']['chat']['id'], 'message_id': mid})
    else:
