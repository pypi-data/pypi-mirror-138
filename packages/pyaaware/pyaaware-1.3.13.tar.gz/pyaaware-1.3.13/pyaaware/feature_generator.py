...
class FeatureGenerator:
    ...
    def execute_all(self, xf: np.ndarray, truth_in: (None, np.ndarray) = None) -> (np.ndarray, np.ndarray):
        ...
        if has_channels:
            feature = np.empty((channels, output_frames, self.num_bands, self.stride), dtype=np.single)
            truth = np.empty((channels, output_frames, self.num_classes), dtype=np.single)
        else:
            feature = np.empty((output_frames, self.num_bands, self.stride), dtype=np.single)
