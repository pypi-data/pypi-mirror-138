...
class FeatureGenerator:
    ...
    def execute_all(self, xf: np.ndarray, truth_in: (None, np.ndarray) = None) -> (np.ndarray, np.ndarray):
        ...
        if has_channels:
            feature = np.empty((self.num_bands, self.stride, channels, output_frames), dtype=np.single)
            truth = np.empty((self.num_classes, channels, output_frames), dtype=np.single)
        else:
            feature = np.empty((self.num_bands, self.stride, output_frames), dtype=np.single)
