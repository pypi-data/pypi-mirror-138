...
class ProcessingQueue(object):
    ...
    def setup_rogue(self):
        ...
        if len(result) > 0:
            result_dict = {k.decode() if isinstance(k, bytes) else k: v
                           for k, v in dict(zip(result[::2], result[1::2])).items()}
            event_dict = {k: v for k, v in result_dict.items() if k.startswith(HASH_EVENT_PREFIX + ':')}
            self.events = [Event.from_redis_string(raw_string=raw_string, redis_client=self.redis_client)
