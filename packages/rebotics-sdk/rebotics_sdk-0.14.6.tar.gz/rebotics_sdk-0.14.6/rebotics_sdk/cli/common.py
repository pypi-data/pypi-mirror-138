...
@click.option('--verification_code', prompt=True)
@pass_rebotics_context
def configure(ctx, user, password, host, verification_code):
    ...
    try:
        response = provider.token_auth(user, password, verification_code)
