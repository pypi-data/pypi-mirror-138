...
@LastEditTime: 2022-02-14 10:16:47
...
class TopBaseModel():
    ...
    def get_dead_date(self, user_nick, access_token, app_key, app_secret, is_log=False):
        ...
        try:
            ...
            if "article_user_subscribe" not in resp["vas_subscribe_get_response"]["article_user_subscribes"].keys():
                ...
                return invoke_result_data
            deadline_list = []
            for article_user_subscribe in resp["vas_subscribe_get_response"]["article_user_subscribes"]["article_user_subscribe"]:
                deadline_list.append(article_user_subscribe["deadline"])
            invoke_result_data.data = max(deadline_list)
            return invoke_result_data
        except Exception as ex:
            if self.context:
                self.context.logging_link_error(traceback.format_exc())
    ...
    def get_project_codes(self, user_nick, access_token,app_key, app_secret,is_filter_expire=False,is_log=False):
        """
        :description: 获取项目编码
        :param user_nick：用户昵称
        :param access_token：access_token
        :param app_key:app_key
        :param app_secret:app_secret
        :param is_filter_expire：是否过滤订购过期的项目编码
        :param is_log：是否记录返回信息
        :return list
        :last_editors: HuangJianYi
        """
        try:
            ...
            project_code_list = []
            for article_user_subscribe in resp["vas_subscribe_get_response"]["article_user_subscribes"]["article_user_subscribe"]:
                if is_filter_expire == True and article_user_subscribe["deadline"] < SevenHelper.get_now_datetime():
                    continue
                project_code_list.append(article_user_subscribe["item_code"])
            return ",".join(project_code_list)
        except Exception as ex:
            if self.context:
                self.context.logging_link_error(traceback.format_exc())
            elif self.logging_link_error:
    ...
    def change_crm_point(self, open_id, change_type, opt_type, quantity, access_token, app_key, app_secret, activity_id=0, activity_name="", is_log=False):
        ...
        try:
            ...
            if "record_id" not in resp["crm_point_change_response"]["results"].keys():
                invoke_result_data.success = False
