...
@LastEditTime: 2022-02-14 10:05:05
...
class GetHighPowerListHandler(ClientBaseHandler):
    ...
    def get_async(self):
        ...
        if len(custom_function_list) == 0:
            #获取项目编码
            project_codes = top_base_model.get_project_codes(store_user_nick, access_token)
            public_function_list = mp_base_model.get_public_function_list(project_codes)
            if len(public_function_list) > 0:
                config_data["function_config_list"] = public_function_list[0]["function_info_second_list"]
                config_data["skin_config_list"] = public_function_list[0]["skin_ids_second_list"]
        else:
            config_data["function_config_list"] = custom_function_list[0]["function_info_second_list"]
            config_data["skin_config_list"] = custom_function_list[0]["skin_ids_second_list"]
