    ...
    if col == 'MACD':
        ...
        def __calc(row):
            ...
            if period_len+26 >= row.name+1:
                ...
                ema_today = (row[col] * ((2/(period_len+1)))) +  (prev_ema * (1-(2/(period_len+1))))
                prev_ema = ema_today
                return ema_today
            else:
                ema_today = (row[col] * ((2/(period_len+1)))) + (ma * (1-(2/(period_len+1))))
                prev_ema = ema_today
                return ema_today
    else:
...
class Ichimoku:
    ...
    def calcPeriodMinMax(df, period_len): # Gets the min and max of period closing price
        arr_min = np.array(df[["open", "high", 'low', "close"]].rolling(period_len).min().rolling(4, axis=1).min().close)
        arr_max = np.array(df[["open", "high", 'low', "close"]].rolling(period_len).max().rolling(4, axis=1).max().close)
    ...
    def calcLeadingSpanA(df):
        periods = 26
        arr = list(df.apply(lambda row: (row.convline + row.baseline) / 2, axis=1).values)
    ...
    def calcConvBaseDivergence(df):
        arr = df.apply(lambda row: (row.convline - row.baseline) / row.baseline, axis=1).values
        return arr

    def calcLeadSpanDivergence(df):
        arr = df.apply(lambda row: (row.leadspan_a - row.leadspan_b)/row.leadspan_b, axis=1).values
        return arr
