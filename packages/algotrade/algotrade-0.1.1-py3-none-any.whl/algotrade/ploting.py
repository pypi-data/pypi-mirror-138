...
class Ichimoku:
    ...
    def _calc(self, indicator) -> None:
        ichimoku = indicator(self.df[["open", "high", 'low', "close"]].max(axis=1), self.df[["open", "high", 'low', "close"]].min(axis=1))
    ...
    def plot(self) -> None:
        ...
        plt.fill_between(
        self.leadspan_a.index, self.leadspan_a.values, self.leadspan_b.values, alpha=0.5, color="lightblue"
    )
...
class Macd:
    ...
    """
    def __init__(self, df : pd.DataFrame, timeframe_short=12, timeframe_long=26, timeframe_signal=9) -> None:
    ...
    def _calc(self, indicator, timeframe_short, timeframe_long, timeframe_signal):
        macd = indicator(self.df.close, timeframe_short, timeframe_long, timeframe_signal)
        self.macd = macd.macd()
