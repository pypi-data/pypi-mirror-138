...
def __get_result_buy(ticker, start_date, last_days, strategy): # function to screen last day of stock and test peformance
...
def screenStocks_Multiproccesed(ticker_list, strategy, last_days):
    ...
    with concurrent.futures.ProcessPoolExecutor() as executor:
        dates = {
            executor.submit(__get_result_buy, ticker, strategy=strategy, start_date=start_date, last_days=last_days): ticker
...
def main():
    ...
    print('-'*100)
    for stock in sorted(stocks_to_buy, key=lambda x:x[1].to_pydatetime(), reverse=True):
        print(stock)
