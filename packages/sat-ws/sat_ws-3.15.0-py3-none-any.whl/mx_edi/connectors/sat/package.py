...
class Package:
    ...
    def download(self, connector: SATConnector, process: bool = True):
        ...
        sizeof_raw = getsizeof(self.raw)
        _logger.debug("Downloaded package %s (%s bytes)MB", self.identifier, sizeof_raw / 1024)
        if process:
    ...
    def _process_download_response(self, response: Response = None):
        ...
        _logger.debug("Creating %s CFDI's from package %s", len(parsed["Content"]), self.identifier)
