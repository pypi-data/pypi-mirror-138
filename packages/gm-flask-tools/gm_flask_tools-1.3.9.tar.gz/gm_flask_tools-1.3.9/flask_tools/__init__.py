...
app.jobs = flask_jobs.JobScheduler(
    app,
    SERVER_HOST_URL=config.SERVER_HOST_URL if hasattr(config, 'SERVER_HOST_URL') else None  # if on linux, set to "https://mysite.com/"
)
...
''')
    ...
    if 'config.py' not in os.listdir(baseDir):
        ...
        with open(baseDir / 'config.py', mode='rt') as oldFile:
