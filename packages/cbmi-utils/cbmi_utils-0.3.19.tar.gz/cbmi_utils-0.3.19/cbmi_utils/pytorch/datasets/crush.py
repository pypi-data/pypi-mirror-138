...
class Crush(Dataset):
    def __init__(self, root: str, class_to_idx: dict, tissue='*', zoom: int = 0, transform: Optional[Callable] = None, target_transform: Optional[Callable] = None):
...
def crush_cg_crush_void_tumor(root='/data/ldap/histopathologic/original_read_only/Crush/Patches', zoom: int = 0, transform: Optional[Callable] = None, target_transform: Optional[Callable] = None):
    # Filter function
    class_to_idx = {
        "Crush": 0,
        "Crush-void": 0,
    ...
    return Crush(root, class_to_idx, tissue='CG', zoom=zoom, transform=transform, target_transform=target_transform)
