...
from optimization.initialization import Genes
from optimization.evaluation import FitnessFunction
from optimization.selection import (RankSelection,
...
from optimization.mating import MatingFunction
from optimization.reproduction import (KPointCrossover,
                                       LinearCombinationCrossover,
                                       FitnessProportionateAverage
                                       )

from optimization.mutation import (Boundary,
                                   Shrink
                                   )

from optimization.environment import (AdaptiveReproduction,
                                      Elitism,
                                      AdaptiveMutation
                                      )

from optimization.experimental.reproduction import AvergeReproduction
...
class GenesSearchCV:
    ...
    def __init__(self, n_gen: int, initialization_fn, fitness_fn, selection_fn, mating_fn, reproduction_fn, mutation_fn, adaptive_population=None, elitism=None, adaptive_mutation=None):
        ...
        if type(reproduction_fn) == (LinearCombinationCrossover and initialization_fn.all_str) or (FitnessProportionateAverage and initialization_fn.all_str):
            raise Exception('LinearCombinationCrossover and FitnessProportionateAverage only works if all hyperparameters in given searching space are numberic.')
