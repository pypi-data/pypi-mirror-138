...
class Rule(LoyalicosAPIClient):
    ...
    def post(self, member_id='', date_activity=None, wait_for_response=False):
        """
            Send Rule
        """
        if member_id != '':
...
class ValidityRule(LoyalicosAPIClient):
    """
        Extends API Client to handle Rules
    """
    def __init__(self, 
...
class ExchangeRule(LoyalicosAPIClient):
    """
        Extends API Client to handle Rules
    """
    def __init__(self, 
