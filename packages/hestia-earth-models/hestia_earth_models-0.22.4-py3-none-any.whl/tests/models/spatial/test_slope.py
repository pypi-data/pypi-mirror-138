...
from hestia_earth.models.spatial.slope import MODEL, TERM_ID, _should_run, run
...
def test_run_coordinates(*args):
    with open(f"{fixtures_path}/{MODEL}/coordinates.jsonld", encoding='utf-8') as f:
        site = json.load(f)
...
def test_run_boundary(*args):
    with open(f"{fixtures_path}/{MODEL}/boundary.jsonld", encoding='utf-8') as f:
        site = json.load(f)
