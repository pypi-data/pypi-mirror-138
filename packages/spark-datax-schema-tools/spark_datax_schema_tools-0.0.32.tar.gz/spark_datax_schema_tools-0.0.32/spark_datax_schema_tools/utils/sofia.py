def generate_sofia(spark=None,
    ...
    file_path_name2 = os.path.join(BASE_DIR, "utils", "files", "datum", "ObjectsThu Jan 27 2022.csv")
    ...
    if schema_write_or_read == "read":
        ...
        t.add_row(["sqlfilter", f"gf_cutoff_date=" + "{ODATE}"])
    else:
        t.add_row(["QueryParamas_paths", f"x_{text_schema.lower()}_hdfs_{table_name2}_0" + "_{ODATE}"])
        t.add_row(["sqlfilter", ""])

    t.add_row(["Parameter", "ODATE - STRING - information_date"])
