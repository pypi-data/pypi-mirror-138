...
def ddng_object_sofia(squad_name=None,
    ...
    file_path_name = os.path.join(BASE_DIR, "utils", "files", "sofia", "DDNG-O-32673_objects.xls")
...
def ddng_object_datum(squad_name=None,
                      ...
                      fields_partitions=None,
                      selected_columns=None):
    file_path_name = os.path.join(BASE_DIR, "utils", "files", "datum", "ObjectsThu Jan 27 2022.csv")
...
def ddng_fields_sofia(squad_name=None,
                      ...
                      selected_columns=None):
    file_path_name = os.path.join(BASE_DIR, "utils", "files", "sofia", "DDNG-F-31507_fields.xls")
    df = pd.read_excel(file_path_name)
    ...
    df5["LOGICAL_NAME_FIELD_SPA"] = df5["LOGICAL_NAME_FIELD_SPA"]
    df5["SIMPLE_FIELD_DESCRIPTION_SPA"] = df5["SIMPLE_FIELD_DESCRIPTION_SPA"]
    df5["LEVEL"] = df5["LEVEL"]
...
def ddng_fields_datum(squad_name=None,
                      ...
                      df_table_fields=None,
                      selected_columns=None):
    file_path_name = os.path.join(BASE_DIR, "utils", "files", "datum", "Fields_Sun Jan 09 2022.csv")
    df = pd.read_csv(file_path_name, sep=",")
    df.columns = [str(col).upper().strip().replace(" ", "_") for col in df.columns]
    df['PHYSICAL_NAME_FIELD'] = df['PHYSICAL_NAME_FIELD'].str.lower()
