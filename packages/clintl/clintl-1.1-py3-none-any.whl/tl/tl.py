...
middleground = 1
foreground = 2
colors = {"white": "#ffffff", "red": "#fbd8ea", "blue": "#c4e2fa", "yellow": "#fff4d6", "violet": "#dad0ef", "green": "#edf5dc", "turquoise": "#d4f3f6"}
...
def alignment_solution(time_points, reference, metrics, xpadding=0, xnudge=0):
    (textwidth_function, textheight_function, date_position_function, month_width, text_height, lineheight, min_scale_date, max_scale_date) = metrics
...
def bottom_alignment_solution(top_solution, metrics, xpadding=0, xnudge=0):
    (textwidth_function, textheight_function, date_position_function, month_width, text_height, lineheight, min_scale_date, max_scale_date) = metrics
...
def render_thread(thread, metrics, style, xoffset, yoffset=0):
    (textwidth_function, textheight_function, date_position_function, month_width, text_height, lineheight, min_scale_date, max_scale_date) = metrics
    (lwd, xpadding, ypadding, threadoffset, linescaling) = style
    ...
    if "timepoints" in thread.keys():
        ...
        top_alignments = alignment_solution(thread_tps, reference, metrics, xpadding=datepadding, xnudge=xnudge)
        bottom_alignments = bottom_alignment_solution(top_alignments, metrics)  
...
def render_block(block, header_width, metrics, style, xoffset, yoffset=0):
    ...
    for thd in block["threads"]:
        out = add_rendering(out, render_thread, thd, metrics, style, xoffset)
    out[-2] += ypadding
    y_bottom = out[-2]
...
def render_tl(tl, fontsize=14, font="Arial", monthwidth=1, today=False, autowidth=False, datelines=False, grid="", linescaling=1, mindate=0, maxdate=0):
    ...
    if autowidth:
        month_width = (1200 * .96-xoffset) / len(month_between(min_scale_date, max_scale_date))
    ...
    # render blocks
    for b in tl["blocks"]:
        render_out = add_rendering(render_out, render_block, b, header_width, metrics, style, xoffset)
...
def main(
    ...
    # rendering
    try:
        ([svg_out, y, max_x]) = render_tl(tl, fontsize=fontsize, monthwidth=monthwidth, today=today, autowidth=autowidth, datelines=datelines, grid=grid, linescaling=linescaling*1.2, mindate=min_scale_date, maxdate=max_scale_date)
