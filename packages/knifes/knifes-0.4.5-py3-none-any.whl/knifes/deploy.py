...
def script_main(proc_name, chdir, logdir):
    args = sys.argv[1:]
    if not args:
        print_help()
    elif args[0] == 'reload':
        ...
        install_modules(chdir, proc_name)
    elif args[0] == 'log':
...
def start_gunicorn(project_dir, project_name, log_dir):
    cmd = '{}{}_venv/bin/gunicorn -c {}gunicorn_{}_conf.py {}.wsgi:application'.format(project_dir, project_name, project_dir, project_name, project_name)
    exec_shell(cmd)
    time.sleep(1)
