...
def main_script(proc_name, chdir, logdir):
    args = sys.argv[1:]
    if not args:
        print_help()
    elif args[0] == 'reload':
        ...
        start_gunicorn(chdir, logdir, proc_name)
    else:
        print_help()
...
def start_gunicorn(project_dir, log_dir, project_name):
    """
    调用前先在project目录下创建venv环境,如:
    /root/.pyenv/versions/3.7.2/bin/python -m venv venv
    """
    cmd = '{}venv/bin/gunicorn -c {}gunicorn_{}_conf.py {}.wsgi:application'.format(project_dir, project_dir, project_name, project_name)
    # 判断是否已启动
