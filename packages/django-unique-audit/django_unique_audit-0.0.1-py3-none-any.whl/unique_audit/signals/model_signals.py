...
from unique_audit.middleware.unique_audit import get_current_request, \
    get_current_user
from unique_audit.models import CRUDEvent
from unique_audit.settings import REGISTERED_CLASSES, UNREGISTERED_CLASSES, \
    WATCH_MODEL_EVENTS, CRUD_DIFFERENCE_CALLBACKS, LOGGING_BACKEND, \
    DATABASE_ALIAS
from unique_audit.utils import get_m2m_field_name, model_delta
    ...
    try:
        with transaction.atomic(using=using):
            if not should_audit(instance):
                return False
            try:
                object_json_repr = serializers.serialize("python", [instance])
            except Exception:
            ...
            if not created:
                ...
                if not delta and getattr(settings, "DJANGO_UNIQUE_AUDIT_CRUD_EVENT_NO_CHANGED_FIELDS_SKIP", False):
                    return False
            ...
            if create_crud_event and not created:
                ...
                def crud_flow():
                    try:
                        # atomicity based on the unique_audit database alias
                        with transaction.atomic(using=DATABASE_ALIAS):
                            crud_event = audit_logger.crud({
                                ...
                                'user_pk_as_string': str(user.pk) if user else None
                            })
                    except Exception as e:
                        try:
                            logger.exception(
                                "unique audit had a pre_save exception on CRUDEvent creation. instance: {}, instance pk: {}".format(
        ...
        logger.exception('unique audit had a pre-save exception.')
...
def post_save(sender, instance, created, raw, using, update_fields, **kwargs):
    ...
    try:
            ...
            if create_crud_event and created:
                ...
                def crud_flow():
                    try:
                        ...
                        try:
                            logger.exception(
                                "unique audit had a post_save exception on CRUDEvent creation. instance: {}, instance pk: {}".format(
                                    instance, instance.pk))
                        except Exception:
                            pass
                if getattr(settings, "TEST", False):
        ...
        logger.exception('unique audit had a post-save exception.')
...
def m2m_changed(sender, instance, action, reverse, model, pk_set, using, **kwargs):
    ...
    try:
        with transaction.atomic(using=using):
            ...
            def crud_flow():
                try:
                    ...
                    try:
                        logger.exception(
                            "unique audit had a m2m_changed exception on CRUDEvent creation. instance: {}, instance pk: {}".format(
                                instance, instance.pk))
                    except Exception:
                        pass
        ...
        logger.exception('unique audit had an m2m-changed exception.')
...
def post_delete(sender, instance, using, **kwargs):
    ...
    try:
        with transaction.atomic(using=using):
            ...
            def crud_flow():
                try:
                    ...
                    try:
                        logger.exception(
                            "unique audit had a post_delete exception on CRUDEvent creation. instance: {}, instance pk: {}".format(
                                instance, instance.pk))
                    except Exception:
                        pass
        ...
        logger.exception('unique audit had a post-delete exception.')


if WATCH_MODEL_EVENTS:
    signals.post_save.connect(post_save, dispatch_uid='unique_audit_signals_post_save')
    signals.pre_save.connect(pre_save, dispatch_uid='unique_audit_signals_pre_save')
    signals.m2m_changed.connect(m2m_changed, dispatch_uid='unique_audit_signals_m2m_changed')
    signals.post_delete.connect(post_delete, dispatch_uid='unique_audit_signals_post_delete')
