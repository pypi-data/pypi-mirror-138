...
"""
  ...
  def json_to_fasta_variants(json_filename):
  	with open(json_filename) as json_file:
  		data = json.load(json_file)
  		# Generate fasta file with the WT & variants
  		filename = "{}.fasta".format(data["metadata"]["name"])
  ...
  def json_preds_to_csv_singleseq(self, json_data):
      # Function to save the predictions in the json file to a txt-like file
      filename = json_data["id"].split("/")[-1].split(".")[0]
      with open('results/{}_b2btools_preds.txt'.format(filename), 'w+') as predfile:		# outfile here
  ...
  def json_preds_to_csv_msa(self, json_data, id):
        # Function to save the predictions in the json file to a txt-like file
        with open('results/{}_b2btools_preds.txt'.format(id), 'w+') as predfile:		# outfile here
