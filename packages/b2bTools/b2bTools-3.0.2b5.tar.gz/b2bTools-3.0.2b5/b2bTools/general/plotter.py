...
biophys_dict = {1:'backbone', 2:'sidechain', 3: 'coil',
				4: 'sheet', 5: 'helix', 6: 'earlyFolding',
				7: 'disoMine'}

def plot_results(json_file_names, msa_like, mutation):
	if msa_like == 0: # To plot in MSA-distrib like
		colors = ['blue', 'orange']
		...
		for biophys_data in biophys_dict.values():
			for file, col in zip(json_file_names, colors):
				with open(file) as json_file:
					...
					for n in range(len(data['results'][biophys_data]['median'])):
						if data['results'][biophys_data]['median'][n] == None \
								or data['results'][biophys_data]['firstQuartile'][n] == None \
								or data['results'][biophys_data]['thirdQuartile'][n] == None:
							none_idx.append(n)
						...
						try:
							...
							if len(none_idx) == 1:
								...
								range_list.append((none_idx[0] + 1, len(
									data['results'][biophys_data]['median'])))
					...
					if range_list:
						for tuple in range_list:
							...
							firstq = \
								data['results'][biophys_data]['firstQuartile'][
								tuple[0]:tuple[1]]
							thirdq = \
								data['results'][biophys_data]['thirdQuartile'][
								tuple[0]:tuple[1]]
							bottom = \
								data['results'][biophys_data]['bottomOutlier'][
								tuple[0]:tuple[1]]
							top = \
								data['results'][biophys_data]['topOutlier'][tuple[0]:tuple[1]]
							plt.fill_between(
					...
					# When there aren't None values in the data
						...
						x = np.arange(0, len(data['results'][biophys_data]['median']), 1)
						firstq = data['results'][biophys_data]['firstQuartile']
						thirdq = data['results'][biophys_data]['thirdQuartile']
						bottom = data['results'][biophys_data]['bottomOutlier']
				...
				if mutation:
					print(mutation)
					plt.plot(mutation['results'][biophys_data], linewidth=0.5, color='red')

			plt.axis([0, len(data['results'][biophys_data]['median']), 0, 1.1])
	...
	if msa_like == 1: # To plot single level
	...
	if msa_like == 2: # To plot when reading a json input
		...
		with open(json_file_names) as json_file:
			...
			for biophys_data in biophys_dict.values():
				for result_id in data['results'].keys():
					if result_id != "sequence":
						plt.plot(data['results'][result_id][biophys_data], label = result_id)
						plt.legend()
						plt.title(result_id)
						plt.ylabel(biophys_data)
						plt.xlabel('Residue position')
				...
				if opt == 1:
					filename = json_file_names.split(".")[0] + '_' + biophys_data + '.png'
					plt.savefig("results/" + filename)
					print('plot succesfully saved in results/{}'.format(filename))
					plt.close()
