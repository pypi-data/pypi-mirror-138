...
class Lima:
    def __init__(self, langs: str = "fre,eng", pipes: str = "main,deepud",
        ...
        self.analyzer = aymaralima.lima.LimaAnalyzer(
            langs, pipes, aymaralima.__path__[-1], user_config_path,
            user_resources_path, ",".join([f"{k}:{v}" for k, v in meta.items()]))

    def analyzeText(self,
                    text: str,
                    lang: str = "eng",
                    pipeline: str = None,
                    meta: Dict[str, str] = {}) -> pyconll.unit.conll.Conll:
        if pipeline is None:
