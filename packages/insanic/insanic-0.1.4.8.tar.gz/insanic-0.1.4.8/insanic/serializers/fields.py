...
class BooleanField(Field):
    async def serialize_value(self, value: Any) -> bool:
        if value is None:
            return None
...
class IntegerField(Field):
    async def serialize_value(self, value: Any) -> int:
        if value is None:
            return None
...
class FloatField(Field):
    async def serialize_value(self, value: Any) -> float:
        if value is None:
            return None
...
class StringField(Field):
    async def serialize_value(self, value: Any) -> str:
        if value is None:
            return None
...
class DateField(Field):
    def __init__(self, date_format='%Y-%m-%d', **kwargs: Any) -> None:
        self.date_format = date_format
        super().__init__(**kwargs)

    # @TODO: Check if value a proper date?
    async def serialize_value(self, value: date | None) -> str:
        if value is None:
            return None
