...
class t2react(object):
    ...
    def run(self, save_filename='', incon_filename='', runlocation='', simulator='AUTOUGH2_2',
        ...
        if self.filename:
            ...
            if (self.type == 'AUTOUGH2'):
                ...
                if runlocation: os.chdir(runlocation)
                ...
                simulation = Popen(cmd, stdin=infile, stdout=outfile)
                running = True
                while running:
                    time.sleep(10)
                    running, proper_end = self.check_end()
                    if proper_end == True and running == False:
                        simulation.terminate()
                    elif proper_end == False:
                        break

    def check_end(self):
        running_flag = True
        proper_end = True
        with open('iter.out', 'rb') as f:
            ...
            try:
                ...
                if tolerance < 0.0001:
                    running_flag = False
                    return running_flag, proper_end
            except:
                proper_end = False
            # time_lapse = self.parameter['tstop'] - sim_duration
            return running_flag, proper_end
        # infile = open('iter.out', 'r')
        # print(time.ctime())
    ...
    def read(self, filename='', meshfilename='', runlocation=None):
    ...
    def write(self, filename='', meshfilename='', runlocation='',
        ...
        """
        if runlocation:
            isdir = os.path.isdir(runlocation)
            if isdir is False:
                os.mkdir(runlocation)
            os.chdir(runlocation)
        if filename: self.filename = filename
        if self.filename == '': self.filename = 't2react.dat'
    ...
    def transfer_rocktypes_from(self, source, mapping):
        ...
        t2react object, using the specified block mapping."""
        from copy import deepcopy
    ...
    def transfer_generators_from(self, source, sourcegeo, geo,
        ...
        """Transfers generators from another t2react object, using the
    ...
    def transfer_from(self, source, sourcegeo, geo,
        ...
        initial conditions from another t2react object (without
    ...
    def effective_incons(self, incons=None):
        ...
        default initial conditions and INCON and INDOM data blocks in the t2react object.
    ...
    def json(self, geo, mesh_filename, atmos_volume=1.e25, incons=None,
        ...
        """Returns a Waiwera JSON dictionary representing the t2react object
