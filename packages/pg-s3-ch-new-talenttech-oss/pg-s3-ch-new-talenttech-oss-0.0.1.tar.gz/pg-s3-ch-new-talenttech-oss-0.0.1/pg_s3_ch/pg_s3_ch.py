...
class PGS3CH:
    ...
    def extract_to_s3(self):
        ...
        for id in range(0, end):
            ...
            data = fa.getvalue()
            file_name = '{entity_name}_{range_from}.csv'.format(entity_name=self.entity_name, range_from=range_from)
    ...
    def s3_to_temp(self):
        ...
        sql = """
            ...
            SELECT 
                {schema_insert}, NOW()
            FROM s3("{endpoint_url}/{bucket}/{upload_path}/{entity_name}_*.csv", 
                    '{S3_ACCESS_KEY}',
                    '{S3_ACCESS_SECRET}',
                    'CSV', 
                    '{schema}'
