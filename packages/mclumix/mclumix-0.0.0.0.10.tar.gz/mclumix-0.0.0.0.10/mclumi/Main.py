...
def main():
    ...
    parser.add_argument(
        "tool",
        type=str,
        help='trim, dedup_basic, dedup_pos, dedup_gene, dedup_sc, dc_by_cnt',
    )
    parser.add_argument(
        "--read_structure", "-rs",
    ...
    if args.tool == 'dechimeric':
        ...
        if args.m == None:
            ...
            raise ValueError
        # cmd = 'python ' + fpnf + ' -h'
        cmd = 'python ' + fpnf + ' -m ' + args.m + ' -ibam ' + args.ibam + ' -tcthres ' + str(args.tcthres) + ' -obam ' + args.obam + ' -obam ' + args.obam_c + ' -issv ' + str(args.issv) + ' -vb ' + str(args.vb)
        # print(cmd)
        s = subprocess.Popen(cmd, shell=True)
        s.communicate()