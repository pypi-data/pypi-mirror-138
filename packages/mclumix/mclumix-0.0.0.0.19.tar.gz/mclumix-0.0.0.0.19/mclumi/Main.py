...
def main():
    ...
    if args.tool == 'dedup_gene':
        ...
        if args.gist == None:
            ...
            raise ValueError
        # cmd = 'python ' + fpnf + ' -h'
        cmd = 'python ' + fpnf + ' -m ' + args.m + ' -ibam ' + args.ibam + ' -ed ' + str(args.ed) + ' -gt ' + str(args.gt) + ' -gist ' + str(args.gist) + ' -fthres ' + str(args.fthres) + ' -infv ' + str(args.infv) + ' -expv ' + str(args.expv) + ' -itern ' + str(args.itern) + ' -obam ' + args.obam + ' -odsum ' + args.odsum + ' -issv ' + str(args.issv) + ' -vb ' + str(args.vb)
        # print(cmd)
        s = subprocess.Popen(cmd, shell=True)
        s.communicate()
