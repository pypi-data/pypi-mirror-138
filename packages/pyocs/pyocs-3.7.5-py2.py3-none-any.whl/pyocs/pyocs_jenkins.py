...
class PyocsJenkins:
    ...
    def create_jenkins_job(self, model_id, build_cmd_param_dict):
        ...
        for key in compile_param:
            if key in self.jenkins_param_dict:
                compile_param[self.jenkins_param_dict[key]] = compile_param.pop(key)

        master = self.get_jenkins_master(compile_param["projectName"], compile_param["branchName"])
        self._create_jenkins_job(compile_param=compile_param, master=master)
