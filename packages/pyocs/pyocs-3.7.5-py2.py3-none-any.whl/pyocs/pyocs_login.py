...
class PyocsLogin:
    ...
    def get_account_from_json_file():
        ...
        try:
            ...
            if "userCn" not in account_dict:
                if account_dict["Username"] == "ocs_tsc":
                    account_dict["userCn"] = "公共账号"
                else:
                    token = requests.get(
                    ...
                    ret = requests.get(
                        "https://itapis.cvte.com/wuli-service/user/search/" + account_dict["Username"],
                        headers=headers)
                    for item in json.loads(ret.text)["data"]:
                        if item["alias"] == account_dict["Username"]:
                            account_dict["userCn"] = item['userCn']
                            break
            return account_dict
