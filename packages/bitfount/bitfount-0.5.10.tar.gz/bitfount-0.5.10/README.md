...
```python
adult = bf.DataSource(
    data="https://bitfount-hosted-downloads.s3.eu-west-2.amazonaws.com/adult.csv",
    ignore_cols=['fnlwgt'],
)
```
...
```python
schema = bf.BitfountSchema(adult, force_stype={'categorical':["TARGET","workclass","marital-status","occupation","relationship","ra\
ce","native-country","gender","education"] })
```
...
adult.data = processor.transform(adult.data)
schema.add_datasource_features(adult)
```
