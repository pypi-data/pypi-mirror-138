...
# Hub-related JSON.
class _AccessRequestResponseJSONBase(TypedDict, total=True):
    """Guaranteed present contents of response JSON for access requests."""

    accessJSON: str
    modellerPublicKeyPem: str
    status: str


class _AccessRequestResponseJSON(_AccessRequestResponseJSONBase, total=False):
    """Response JSON for access requests."""

    signedPermission: str


class _AccessJSONBase(TypedDict, total=True):
    """Guaranteed present contents of an accessJSON."""

    protocol: str
    modellerIdPUserID: str


class _AccessJSON(_AccessJSONBase, total=False):
    """Contents of a signed access request."""

    # Optional key. Not provided if it's yet approved.
    status: str
    # Optional key. Not provided if user hasn't set a public key.
    modellerPublicKey: str
