...
def _mock_authentication_code(username: str, password: str, token_dir: Path) -> str:
    ...
    return inspect.cleandoc(
        f"""from unittest.mock import patch
            ...
            from bitfount.hub.api import _STAGING_IDP_URL, _STAGING_AM_URL
            ...
            user_storage_path = Path('{str(token_dir)}')/"pods"
            oidc_patcher = patch(
                "{oidc_webbrowser_import}",
                side_effect=partial(
                    oidc_flow,
...
def _perform_notebook_replacements(
    ...
    if nb_num in (3, 5, 9):
        ...
        if nb_num == 5:
            ...
            # second_pod_name = f"{run_user}/{second_adult_pod_name}"
            nb = _replace_in_notebook_source_code(
                ADULT_YAML_POD_NAME,
                second_adult_pod_name,
                nb,
...
class TestTutorials:
    ...
    @pytest.mark.skip(
        "This is tested in the tutorials yaml config so can be skipped here."
    )
    @pytest.mark.parametrize(
        "fixed_notebooks, authentication_user",
        [(2, MODELLER_USERNAME)],
    ...
    def test_tutorial_4(
        ...
        assert Path("part_4_model.pt").exists()
    ...
    def test_tutorial_7(
        ...
        assert Path("part_7_model.pt").exists()
    ...
    def test_tutorial_8(
        ...
        assert Path("part_8_model.pt").exists()
