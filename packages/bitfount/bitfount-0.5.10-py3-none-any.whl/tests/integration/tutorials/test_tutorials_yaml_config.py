...
- `"bitfount.federated.transport.identity_verification.oidc.webbrowser.open_new_tab`: we
patch the webbrowser opening call that is made in the OIDC authentication flow
...
we don't block the Modeller's responses to the OIDC challenges.
"""
import asyncio
...
from tests.utils.helper import tutorial_yaml_test
...
def _run_pod(
    ...
    """
    try:
        tutorial_file: str = f"tests/integration/tutorials/tutorials_yaml_config/{TUTORIALS[tutorial_num]}.md"  # noqa: B950
...
def _perform_notebook_replacements(
    ...
    if nb_num in (1, 3, 6, 7):
        ...
        nb = _replace_in_notebook_source_code("bitfount/adult", adult_pod_name, nb)
        if nb_num == 3:
...
class TestTutorials:
    ...
    def fixed_notebooks(
        ...
        for nb_num in nb_nums:
            nb = jupytext.read(
                f"tests/integration/tutorials/tutorials_yaml_config/{TUTORIALS[nb_num]}.md"  # noqa: B950
            )
