...
class _FeatureDict(TypedDict, total=False):
    """Typed dictionary for the features in a BitfountSchema.

    `total=False` indicates that the keys don't all need to be present. But the keys
    that are present need to be one of the ones listed below.
    """

    categorical: Dict[str, CategoricalRecord]
    continuous: Dict[str, ContinuousRecord]
    image: Dict[str, ImageRecord]
    text: Dict[str, TextRecord]


class DataPathModifiers(TypedDict, total=False):
