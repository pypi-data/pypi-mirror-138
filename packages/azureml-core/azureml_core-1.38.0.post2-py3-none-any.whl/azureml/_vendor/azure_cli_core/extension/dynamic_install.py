...
def _get_extension_command_tree(cli_ctx):
    from azureml._vendor.azure_cli_core._session import EXT_CMD_TREE
    import os
    ...
    if not EXT_CMD_TREE.data:
        import posixpath
        import requests
        from azureml._vendor.azure_cli_core.util import should_disable_connection_verify
        try:
...
def _check_value_in_extensions(cli_ctx, parser, args, no_prompt):  # pylint: disable=too-many-statements, too-many-locals
    ...
    from azureml._vendor.azure_cli_core.azclierror import NoTTYError
    exit_code = 2
    ...
    from azureml._vendor.azure_cli_core.extension import get_extension, ExtensionNotInstalledException
    from azureml._vendor.azure_cli_core.extension._resolve import resolve_from_index, NoExtensionCandidatesError
    try:
    ...
    print_error = True
    if install_ext:
        from azureml._vendor.azure_cli_core.extension.operations import add_extension
