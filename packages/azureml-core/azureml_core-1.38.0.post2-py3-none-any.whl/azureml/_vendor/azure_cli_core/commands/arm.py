...
from azureml._vendor.azure_cli_core import EXCLUDED_PARAMS
from azureml._vendor.azure_cli_core.commands import LongRunningOperation
from azureml._vendor.azure_cli_core.commands.client_factory import get_mgmt_service_client
from azureml._vendor.azure_cli_core.commands.events import EVENT_INVOKER_PRE_LOAD_ARGUMENTS
from azureml._vendor.azure_cli_core.commands.validators import IterateValue
from azureml._vendor.azure_cli_core.util import shell_safe_json_parse, get_command_type_kwarg
...
def raise_subdivision_deployment_error(error_message, error_code=None):
    from azureml._vendor.azure_cli_core.azclierror import InvalidTemplateError, DeploymentError
...
def register_global_subscription_argument(cli_ctx):
    ...
    def add_subscription_parameter(_, **kwargs):
        ...
        from azureml._vendor.azure_cli_core._completers import get_subscription_id_list
...
def show_exception_handler(ex):
    if getattr(getattr(ex, 'response', ex), 'status_code', None) == 404:
        import sys
        from azureml._vendor.azure_cli_core.azlogging import CommandLoggerContext
        from azureml._vendor.azure_cli_core.azclierror import ResourceNotFoundError
...
def get_arm_resource_by_id(cli_ctx, arm_id, api_version=None):
    ...
    if not api_version:
        ...
        if not rt:
            from azureml._vendor.azure_cli_core.parser import IncorrectUsageError
