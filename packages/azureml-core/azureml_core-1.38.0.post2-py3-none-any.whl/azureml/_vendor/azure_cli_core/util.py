...
def handle_exception(ex):  # pylint: disable=too-many-locals, too-many-statements, too-many-branches
    ...
    import azureml._vendor.azure_cli_core.azclierror as azclierror
    import traceback
...
def get_installed_cli_distributions():
    ...
    from azureml._vendor.azure_cli_core import __version__ as azure_cli_core_version
...
def get_cached_latest_versions(versions=None):
    ...
    import datetime
    from azureml._vendor.azure_cli_core._session import VERSIONS
...
def get_az_version_string(use_cache=False):  # pylint: disable=too-many-statements
    from azureml._vendor.azure_cli_core.extension import get_extensions, EXTENSIONS_DIR, DEV_EXTENSION_SOURCES, EXTENSIONS_SYS_DIR
    import io
    output = io.StringIO()
    versions = _get_local_versions()
...
def get_az_version_json():
    from azureml._vendor.azure_cli_core.extension import get_extensions
    versions = {'extensions': {}}
...
def get_az_user_agent():
    ...
    from azureml._vendor.azure_cli_core import __version__ as core_version

    agents = ["AZURECLI/{}".format(core_version)]

    from azureml._vendor.azure_cli_core._environment import _ENV_AZ_INSTALLER
...
def handle_version_update():
    ...
    try:
        from azureml._vendor.azure_cli_core._session import VERSIONS
        from packaging.version import parse  # pylint: disable=import-error,no-name-in-module
        from azureml._vendor.azure_cli_core import __version__
        if not VERSIONS['versions']:
            ...
            from azureml._vendor.azure_cli_core.cloud import refresh_known_clouds
