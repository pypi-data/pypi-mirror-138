...
HPUDeviceContext::HPUDeviceContext(int device_id) : device_id_(device_id) {
  ...
  // This is because HCL for Gaudi2 requires that HCL_Init() must be called
  // before creating the collective stream.
}
...
HPUDeviceContext::~HPUDeviceContext() {
  ...
  if (collective_stream_ != nullptr) {
    hccl_status = hcclxReleaseCollectiveStream(collective_stream_);
    HCCL_OP_ASSERT(hccl_status == hcclSuccess);
  }

  if (d2h_stream_ != nullptr) {
    hccl_status = hcclxReleaseCopyStream(d2h_stream_);
    HCCL_OP_ASSERT(hccl_status == hcclSuccess);
  }
  if (h2d_stream_ != nullptr) {
    hccl_status = hcclxReleaseCopyStream(h2d_stream_);
    HCCL_OP_ASSERT(hccl_status == hcclSuccess);
  }

  if (d2d_stream_ != nullptr) {
    hccl_status = hcclxReleaseCopyStream(d2d_stream_);
    HCCL_OP_ASSERT(hccl_status == hcclSuccess);
  }

  hccl_status = hcclxCloseDevice(device_id_);
}

hcclStream_t HPUDeviceContext::collective_stream() {
  if (collective_stream_ == nullptr) {
...
hcclStream_t HPUDeviceContext::d2h_stream() {
  if (d2h_stream_ == nullptr) {
...
hcclStream_t HPUDeviceContext::h2d_stream() {
  if (h2d_stream_ == nullptr) {
    const hcclResult_t hccl_status = hcclxAcquireCopyStream(
...
hcclStream_t HPUDeviceContext::d2d_stream() {
  if (d2d_stream_ == nullptr) {
    const hcclResult_t hccl_status = hcclxAcquireCopyStream(
...
void HCCLOpContext::InitDeviceQueue(
    const std::vector<TensorTableEntry>& entries, hcclStream_t initial_stream) {
  LOG(TRACE) << "Entry " << __PRETTY_FUNCTION__;
  HCCL_OP_ASSERT(nullptr != my_device_);
...
HCCLOpContext::FinalizeDeviceQueue(std::vector<TensorTableEntry>& entries,
  ...
  const hcclStream_t stream_to_submit{current_stream_};
  if (stream_to_submit) {
    ...
    hcclxSubmitEvents(stream_to_submit, output_addresses.data(),
                      ...
                      new EventSubmissionFinalizer{
                          [on_finalize, &timeline, entries]() mutable {
                            if (on_finalize) {
                              on_finalize();
                            };
                            for (auto& e : entries) {
...
Status HCCLAllgather::Execute(std::vector<TensorTableEntry>& entries,
  ...
  if (same_shape) {
    ...
    hcclResult_t hccl_result = hcclAllGather(
        send_buffer, recv_buffer, fusion_data.GetRecvCount(0), hcclChar,
        *op_context_.hccl_comm_, op_context_.collective_stream());
    HCCL_OP_ASSERT(hcclSuccess == hccl_result);
    ...
    LOG(WARNING)
        << "HCCLAllgather for tensors with different shapes (between ranks) is "
           "inefficient.";
    ...
    for (int rank_idx = 0; rank_idx < comm_size; rank_idx++) {
      ...
      hcclResult_t hccl_result = hcclBroadcast(
          send_buff, recv_buff, fusion_data.GetRecvCount(rank_idx), hcclChar,
          rank_idx, *op_context_.hccl_comm_, op_context_.collective_stream());
      HCCL_OP_ASSERT(hcclSuccess == hccl_result);
