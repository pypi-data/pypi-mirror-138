...
public:
  ...
  hcclStream_t d2h_stream();
  hcclStream_t h2d_stream();
  hcclStream_t d2d_stream();

private:
  int device_id_;
  hcclStream_t collective_stream_ = nullptr;
  hcclStream_t d2h_stream_ = nullptr;
  hcclStream_t h2d_stream_ = nullptr;
  hcclStream_t d2d_stream_ = nullptr;
};
...
public:
  ...
  hcclStream_t host_to_device_copy_stream();
  hcclStream_t device_to_host_copy_stream();
  hcclStream_t within_device_copy_stream();
  hcclStream_t collective_stream();

private:
  void SwitchStreams(hcclStream_t next_stream);

  hcclStream_t current_stream() const {
    // TODO: rewrite
  ...
  hcclStream_t current_stream_{nullptr};
...
inline void HCCLOpContext::SynchronizeCurrentStream() {
  ...
  const hcclStream_t stream_to_synchronize{current_stream_};
...
inline hcclStream_t HCCLOpContext::host_to_device_copy_stream() {
  HCCL_OP_ASSERT(my_device_ != nullptr);
  SwitchStreams(my_device_->h2d_stream());
  return current_stream_;
}

inline hcclStream_t HCCLOpContext::device_to_host_copy_stream() {
  HCCL_OP_ASSERT(my_device_ != nullptr);
  SwitchStreams(my_device_->d2h_stream());
  return current_stream_;
}

inline hcclStream_t HCCLOpContext::within_device_copy_stream() {
  HCCL_OP_ASSERT(my_device_ != nullptr);
  SwitchStreams(my_device_->d2d_stream());
  return current_stream_;
}

inline hcclStream_t HCCLOpContext::collective_stream() {
  HCCL_OP_ASSERT(my_device_ != nullptr);
  SwitchStreams(my_device_->collective_stream());
  return current_stream_;
}

inline void HCCLOpContext::SwitchStreams(hcclStream_t next_stream) {
  const hcclStream_t current_stream{current_stream_};
...
protected:
  ...
  public:
    ...
    size_t GetRecvCount(int rank) {
      return recieve_data_counts_[rank] * element_size_;
    }
...
using OrderingQueueKey = std::pair<int, int>;

struct OrderingQueueKeyCompare {
  bool operator()(const OrderingQueueKey& lhs, const OrderingQueueKey& rhs) {
...
struct OrderingQueueItem {
  OrderingQueueItem(OrderingQueueKey key)
      : ordering_key(key), ready(false), sent(false), finalized(false) {}

  TensorTableEntry tensor_entry;
  OrderingQueueKey ordering_key;
  bool ready;
  bool sent;
  bool finalized;
};
...
using OrderingQueue =
    std::map<OrderingQueueKey, std::shared_ptr<OrderingQueueItem>,
             OrderingQueueKeyCompare>;
...
public:
  ...
  std::vector<TensorTableEntry> GetEntriesToFinalize();

  OrderingQueue& ordering_queue() { return ordering_queue_; }
  std::vector<int32_t>& device_map() { return device_map_; }

private:
  void CreateQueueItem(OrderingQueueKey new_key);

  void FillQueueItem(int64_t order_index, TensorTableEntry&& entry);

  OrderingQueue ordering_queue_;
  size_t group_size_;
  size_t queue_items_sent_;
  std::vector<int32_t> device_map_;
  std::map<int64_t, std::shared_ptr<OrderingQueueItem>> ordering_queue_items_;
};

class OrderedOpContext {

public:
  OrderingGroup& GetOrderingGroup(std::set<int> ordering_indices,
                                  std::vector<int32_t>);
  void RemoveOrderingGroupIfCompleted(std::set<int> indices);
