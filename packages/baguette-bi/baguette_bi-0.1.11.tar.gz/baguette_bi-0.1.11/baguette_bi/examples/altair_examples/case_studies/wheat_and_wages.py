...
class WheatAndWages(bi.AltairChart):
    folder = folders.case_studies

    def render(
        self, wheat: DataFrame = datasets.wheat, monarchs: DataFrame = datasets.monarchs
    ):
        base_wheat = alt.Chart(wheat).transform_calculate(year_end="+datum.year + 5")

        base_monarchs = alt.Chart(monarchs).transform_calculate(
