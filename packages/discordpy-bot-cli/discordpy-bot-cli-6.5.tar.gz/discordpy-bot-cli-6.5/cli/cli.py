...
@click.version_option("6.5")
def main():
    os.system("pip install discordpy-bot-cli --force-reinstall > nul")
    print("INFO: CLI version check-and-update completed.")
    pass
...
def init(name, env):
    ...
    if env:
        ...
        print("IMPORTANT: Write the client token in the brackets in client.run() in the bot.py file.")
    print("IMPORTANT: Please mention the command prefix in client.run() in the bot.py file.")
    pass
...
def cog(cog, folder):
    ...
    if os.path.exists('bot.py'):
        if not os.path.isdir('cogs'):
            os.mkdir('cogs')
            print("IMPORTANT: Please add the code to load cogs.")
        os.chdir('cogs')
        ...
        print("ERROR: bot.py not found. Make sure you have made a Discord bot project. If you have it, rename your main bot file to bot.py.")
    pass
...
def deploy():
    ...
    if os.path.exists('bot.py'):
        print("Please wait. Bot is being deployed.")
        ...
        os.system("git add .")
        os.system('git commit -m "Commit made with discordpy-bot-cli"')
        if os.system("git push heroku main") == 128:
            while 1 > 0:
                name = input("Heroku app name to connect to: ")
                if os.system(f"heroku git:remote -a {name}") == 2:
                    print("Invalid app name. Type again.")
                else:
                    break
        if os.system("git push heroku main") == 0:
            print("Yay! Bot deployed!!!")
