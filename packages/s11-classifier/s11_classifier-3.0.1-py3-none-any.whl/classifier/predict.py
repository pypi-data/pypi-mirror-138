...
def prediction_class_probabilities(predicted, valid, meta, config_dict, window):
    ...
    write_tifs(os.path.join(config_dict['tmp_dir'], 'probability'),
               window,
               meta['dst_proba_meta'],
               result_proba)
...
def prediction_all_probabilities(
    ...
    for internal_label in range(predicted.shape[1]):
        ...
        write_tifs(
            os.path.join(config_dict['tmp_dir'], str(labels[internal_label])),
            window,
            meta['dst_proba_meta'],
            result_proba)
...
def do_prediction_proba(data, valid, model_dict, config_dict, meta, window):
...
def gather_data_for_prediction(rasters, meta, config_dict, window):
...
def prediction(window, rasters, model_dict, meta, config_dict):
    ...
    if config_dict['app_rasters_are_timeseries']:
        data, valid = \
            gather_data_for_prediction_ts(rasters, meta, config_dict,  window)
    else:
        data, valid = \
            gather_data_for_prediction(rasters, meta, config_dict, window)
    if not valid.any():
        return

    if not config_dict['app_algorithm'] in \
            US_ALGORITHMS:
    ...
    write_tifs(os.path.join(config_dict['tmp_dir'], 'classification'),
               window,
               meta['dst_meta'],
               result)
