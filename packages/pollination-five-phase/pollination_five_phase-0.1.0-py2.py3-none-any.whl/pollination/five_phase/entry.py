...
from .direct_sunlight.entry import DirectSunlightEntryPoint


@dataclass
class RecipeEntryPoint(DAG):
    """Five phase entry point."""
    ...
    def create_octrees(
        ...
        sunpath=generate_sunpath._outputs.sunpath, phase='5'
    ):
        ...
        return [
            ...
            {
                'from': CreateOctrees()._outputs.five_phase_info
            }
        ]
    ...
    @task(
        template=DirectSunlightEntryPoint,
        loop=create_octrees._outputs.five_phase_info,
        needs=[
            _create_rad_folder, create_octrees, split_grid_folder,
            generate_sunpath
        ],
        sub_folder='calcs/5_phase',
        sub_paths={
            'octree': '{{item.octree_direct_sun}}'
        }
    )
    def calculate_five_phase_matrix(
        self,
        identifier='{{item.identifier}}',
        radiance_parameters=radiance_parameters,
        sensor_grids_info=split_grid_folder._outputs.sensor_grids,
        ...
        sun_modifiers=generate_sunpath._outputs.sun_modifiers,
        bsdf_folder=_create_rad_folder._outputs.bsdf_folder,
        results_folder='../../calcs/5_phase/results'
    ):
        pass
    ...
    @task(
        ...
        sub_folder='calcs/3_phase/total',
        sub_paths={
            'octree': '__three_phase__.oct'
        }
    )
    ...
    @task(
        ...
        needs=[
            _create_rad_folder, create_octrees,
            create_direct_sky, create_sky_dome,
            prepare_three_phase
        ],
        sub_folder='calcs/3_phase/direct',
        sub_paths={
            'octree': '__three_phase__.oct'
        }
    )
    def calculate_three_phase_matrix_direct(
        self,
        model_folder=_create_rad_folder._outputs.model_folder,
        grouped_apertures=prepare_three_phase._outputs.grouped_apertures_info,
        grouped_apertures_folder=prepare_three_phase._outputs.grouped_apertures_folder,
        ...
        octree=create_octrees._outputs.scene_folder,
        sky_dome=create_sky_dome._outputs.sky_dome,
        sky_matrix=create_direct_sky._outputs.sky_matrix,
        bsdf_folder=_create_rad_folder._outputs.bsdf_folder,
        multiplication_options=''
    ):
        pass
    ...
    #     loop=calculate_three_phase_matrix_direct._outputs.multiplication_info,
    #     needs=[
    #         calculate_three_phase_matrix_direct,
    #         calculate_three_phase_matrix_total,
    #         calculate_five_phase_matrix
    #     ]
    # )
    # def add_remove_five_phase_matrix(
    #     self,
    #     direct_sky_matrix='calcs/3_phase/direct/results/{{item.identifier}}.ill',
    #     total_sky_matrix='calcs/3_phase/total/results/{{item.identifier}}.ill',
    #     sunlight_matrix='calcs/5_phase/results/{{item.state_id}}/{{item.grid_id}}.ill',
    #     conversion='47.4 119.9 11.6'
    # ):
    #     return [
    #         {
    #             'from': AddRemoveSkyMatrix()._outputs.results_file,
    #             'to': 'results/5_phase/{{item.identifier}}.ill'
    #         }
    #     ]
