...
def bulk_get_keys(cache, pattern, chunk_size=5000):
    ...
    cursor = '0'
    while cursor != 0:
        cursor, keys = cache.scan(cursor=cursor,
                                  match=pattern,
                                  count=chunk_size)
        if keys:
            allkeys.extend(keys)
...
def write_records_atomic(cache, records,
                         keygenerator,
                         spec,
                         expiry=7*86400):
    """
    ...
    #
    if target not in ['map', 'map-serialized',
                      'elements', 'elements-serialized']:
            ...
            if spec['target'] in ['elements', 'elements-serialized']:
                for col in list(towrite.keys()):
                    ...
                    if target == 'elements-serialized':
                        ...
                        raise Exception(f"Unknown element type {type(value)} for {colkey}")
...
def write_df_dict(cache, df,
    ...
    return write_records_atomic(cache, records,
                                keygenerator,
                                spec,
                                expiry)
