...
class MetricsBase(Compute):
    ...
    def get_datasets(self, profile, specs):
        ...
        for source in sources:
            ...
            try:
                ...
                logger.exception(f"[{name}] post-processing failed",
                                 extra={
                                     'transform': self.name
                                 })
    ...
    def get_dataset_generic(self, source):
        ...
        if ((not hasattr(self, 'read_data')) or
            (not hasattr(self, 'get_dataset'))):
            raise Exception(" get_dataset_raw expects read_data and get_dataset methods")
        ...
        if cache:
            ...
            if os.path.exists(cachefile):
                logger.debug("Read cached {}".format(name),
                             extra={
                                 'transform': self.name
                             })
        ...
        df, metadata = datasetobj.read_data(start_date,
                                            end_date,
