...
class CloudwatchProcessor(ProcessorBase):
    ...
    def get_streams(self, spec):
        ...
        while iteration < max_iterations:
            try:
                ...
                streams = [s for s in streams if (s['firstEventTimestamp']/1000) >= cutoff]
                ...
                logger.exception(f"Failed {log_group}",
                                 extra={
                                     'transform': self.name,
                                     'data': msg
                                 })
        ...
        logger.debug(f"[{log_group}] {len(allstreams)} streams",
                     extra={
                         'transform': self.name,
                         'data': msg
                     })
    ...
    def get_logs(self, spec, stream):
        ...
        """
       {
        ...
        msg += f"[Stream] Last event: {lastevent_timestamp} {type(lastevent_timestamp)} (" + ("Match" if (db_lastevent == lastevent_timestamp) else "No match") +")\n"
        msg += f"[Force] {force}\n"

        # Exists and valid..
        if ((db_lastevent is not None) and
            (db_lastevent == lastevent_timestamp) and
            (not force)):
            msg += "Skipping. Updated"
            logger.debug(f"[{log_stream[-32:]}] Skipping",
                         extra={
                         'transform': self.name,
                         'data': msg
                     })
        ...
        while iteration < max_iterations:
            try:
                ...
                msg += f"Iteration: {iteration} new {len(events)} Total {len(allevents)}\n"

            except:
                logger.exception(f"Failed {log_stream[-32:]}",
                                 extra={
                                     'transform': self.name,
                                 })
                raise

        logger.debug(f"[{log_stream[-32:]}] {len(allevents)} events",
                     extra={
                         'transform': self.name,
                         'data': msg
                     })
    ...
    def collapse_events(self, spec, stream, allevents):
        ...
        try:
            ...
            for event in cleaned:
                ...
                if ((lastevent is not None) and
                    (set(tags) == set(lastevent['metadata']['tags'])) and
            ...
            logger.debug(f"[{log_stream[-32:]}] processed",
                         extra={
                             'transform': self.name,
                             'data': json.dumps(stats,indent=4)
                         })
        except:
            logger.exception(f"[{log_stream[-32:]}] failed",
                         extra={
                             'transform': self.name,
                             'data': json.dumps(stats,indent=4)
                         })
    ...
    def run(self, spec):
        ...
        logger.debug(f"Processing {spec['name']}",
                     extra={
                         'data': str(spec)
                     })
        ...
        logger.debug(f"Completed {spec['name']}",
                     extra={
                         'data': str(spec)
                     })
...
class S3DBTProcessor(ProcessorBase):
    ...
    def process_one_file(self, spec, loggroup, f):
        ...
        for line in lines:
            ...
            try:
                ...
                if ((logstream is None) or ((data is not None) and ('streamname' in data))):
                    ...
                    lastevent = time.mktime(start_timestamp.timetuple())*1e3 + \
                ...
                try:
                    ...
                    ts = ts.replace(year = start_timestamp.year,
    ...
    def run(self, spec):
        ...
        for idx, f in enumerate(files):
            ...
            logger.debug(f"[{log_group}] [{idx}] Processed",
                         extra={
                             'data': msg
                         })
        ...
        logger.debug("Tagged and merged",
                         extra={
                             'data': msg
                         })
        #for event in filtered:
