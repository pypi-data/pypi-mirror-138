import traceback, lzma, io, struct, platform, os, time, zstandard
from PIL import Image 
lzham_supported = False
try:
	import lzham 
except:
	if platform.system() != "Windows":
		print("[*] installing a missing module: lzham")
		os.system("pip install pylzham")
		lzham_supported = True
class memory:
...
def join_image(img, p):
    ...
    for j in range(ha):
        for h in range(_w % a):
            ...
            x += 1
def bytes2image(config, directory=""):
	pixels = []
	info = config["info"]
	name = config["name"]
	...
	del info
	print(f"[*] saving {name}")
	base.save(directory+name)
	del name
	del base
	return True
class pycat:
	def decodeSC(file, use_disk=False,directory=""):
		m = memory(b'')
		m.mem = open(file, "rb")
		temp = open(file, "rb")
		use_lzham = False
		...
		try:
			...
			if use_lzma:
				...
				if lzham_supported:
					...
					raise Exception("lzham not supported on this os")
				del x
				del y
		except:
		...
		if decompressed:
			...
			try:
				if not use_disk:
					...
					s = open(directory+"temp.sb","wb")
					s.write(decompressed)
					s.close()
					s = open(directory+"temp.sb","rb")
					sc = memory(b'')
					sc.mem.close()
					sc.mem = s
				del decompressed
				...
				while(True):
					try:
						...
						if info[3] > 0 and info[4] > 0:
							...
							try:
								if not bytes2image(img, directory):
									index -= 1
								print()
							except:
