...
def generate_romfs_img(romfs_dir, dst_img_name):
    ...
    return subprocess.call([exe, '-d', dir, '-f', dst, '-a', '64'])
...
#             bflb_utils.printf(e)
    ...
    def bl60x_mfg_flasher_eflash_loader_cfg(self,
        ...
        if d0fw is True and pt_parcel['fw_d0_addr'] is not None:   
            d_files.append("chips/" + chipname + "/img_create_iot/d0fw.bin")
            d_addrs.append(hex(pt_parcel['fw_d0_addr'])[2:])   
        
        if imtb is True and pt_parcel['imtb_addr'] is not None:       
...
#                               bl_factory_params_file_prefix + suffix)
        ...
        if config["check_box"]['d0_download'] is True:
            ...
            if d0_bin is not None and len(d0_bin) > 0:
                try:
                    shutil.copyfile(d0_bin, os.path.join(chip_path, chipname, "img_create_iot", "d0fw.bin"))
                except Exception as e:
                    bflb_utils.printf(e)
            d0_bin_d = True
        ...
        return self.bl60x_mfg_flasher_eflash_loader_cfg(chipname, chiptype, bin_d, boot2_d, ro_params_d,
                                                   pt_parcel, media_bin_d, mfg_d, d0_bin_d, imtb_bin_d)
