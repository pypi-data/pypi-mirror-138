...
class AWSClient(object):
    ...
    def get_iam_role_credentials(self, client_name=None,
        ...
        if(client_name is not None):
            role_name = f"SDRC-IAMRole-DL-Client-{client_name}"
        try:
    ...
    def get_client_idws(self, client_name, job_id):
        ...
        food_idw_prefix = (
            f'aqueduct/client_databases/{client_name}/jobs/{job_id}/' +
        ...
        try:
            ...
            self._logger.info("Health Resources IDW file found.")
        except:
            self._logger.error(
                "Couldn't find IDW files associated with that client/job_id")
            return
        health_resource_idw_csv = self.s3_client.get_object(
            Bucket='sdrc-s3-datalake', Key=health_resource_idw_key)
    ...
    def load_s3_idws(self, client_db, client_name, job_id,
        ...
        """
        food_idw_df, hr_idw_df = self.get_client_idws(client_name, job_id)
