...
def export():
    ...
    def wrapped(_fn):
        ...
        # TODO: somehow, fix a strange bug in validating args using 'issubclass(...)'

        _args_valid = len(tuple(filter(lambda x: not hasattr(x, 'nml_type'),
                                       _args))) > 1 if _args else True  # cause empty
