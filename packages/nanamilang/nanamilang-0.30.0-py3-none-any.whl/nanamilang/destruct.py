...
InteropType = Py3Object or Py3Inst
...
class Destructuring:
    ...
    # Destructuring({:keys [a b]}).destruct(Py3Inst)
    # -> {'a': Py3Object, 'b': Py3Object}

    # Destructuring({:keys [a b]}).destruct(Py3Object)
    # -> {'a': Py3Object, 'b': Py3Object}
    ...
    def destruct(self,
                 any_valid_data_type: (Base
                                       or InteropType
                                       or Collection)) -> dict:
        """Destruct given data type, then return environment dict"""

        if isinstance(self._target_token_or_form, Token):
            any_valid_data_type: Base
            return self._single_token_destructure(any_valid_data_type)
        if (isinstance(self._target_token_or_form, list) and
                isinstance(any_valid_data_type, Collection)):
            any_valid_data_type: Collection
            return self._collection_form_destructure(any_valid_data_type)
        if (isinstance(self._target_token_or_form, list) and
                isinstance(any_valid_data_type, (Py3Object, Py3Inst))):
            any_valid_data_type: Py3Object or Py3Inst
            return self._python3_interop_destructure(any_valid_data_type)
        return {}  # <------------------ return an empty dictionary otherwise

    def _vector_form_destructure(self, vector: Vector) -> dict:
        """When the left and the right forms both are Vectors"""
        ...
        for idx, idn in enumerate(
            ...
            item_index = IntegerNumber(idx)  # <- cast it to IntegerNumber
            environ[idn.dt().origin()] = vector.get(item_index, self._nil)

        return environ

    def _hashmap_to_hashmap_form_destructure(self,
                                             hashmap: HashMap) -> dict:
        """When the left form is a HashMap and the right form is a HashMap"""
    ...
    def _hashmap_to_interop_form_destructure(self,
                                             interop: InteropType) -> dict:
        """When the left form is a HashMap and the right form is an Interop"""
    ...
    def _single_token_destructure(self, any_valid_data_type: Base) -> dict:
        """When the left form is a token and the right form any valid"""
    ...
    def _collection_form_destructure(self, collection: Collection) -> dict:
        """When the left form is a form and the right form is a Collection"""

        if (NML_M_FORM_IS_A_VECTOR(self._target_token_or_form) and
                isinstance(collection, Vector)):
            return self._vector_form_destructure(collection)
        if (NML_M_FORM_IS_A_HASHMAP(self._target_token_or_form) and
                isinstance(collection, HashMap)):
            return self._hashmap_to_hashmap_form_destructure(collection)

        return {}  # <------------------- return an empty dictionary otherwise

    def _python3_interop_destructure(self, interop: InteropType) -> dict:
        """When the left form is a HashMap and the right form is an Interop"""

        if NML_M_FORM_IS_A_HASHMAP(self._target_token_or_form):
            return self._hashmap_to_interop_form_destructure(interop)
    ...
    def _keys_destruct(self, lst: list,
                       hashmap_or_interop: (HashMap or InteropType)) -> dict:
