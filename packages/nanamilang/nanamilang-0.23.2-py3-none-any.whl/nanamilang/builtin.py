...
class BuiltinMacros:
    ...
    def fn_macro(tree: list, local_env: dict, _, eval_function) -> list or Token:
        ...
        parameters = []
        for parameter_token_or_form in NML_M_ITERATE_AS_PAIRS(parameters_form):
            if isinstance(parameter_token_or_form, list):
                NML_M_ASSERT_FORM_IS_A_VECTOR(parameter_token_or_form,
                                              'fn: at the moment, only Vector can be destructured')
                right = []
                for x in NML_M_ITERATE_AS_PAIRS(parameter_token_or_form):
                ...
                assert parameter_token_or_form.type() == Token.Identifier, \
                                                         'fn: param name needs to be an Identifier'
                parameters.append((parameter_token_or_form.dt().origin(), None))

        fn_name = name_token.dt().origin() if name_token else randstr()

        fni = fn.Fn(body_token_or_form, local_env, eval_function, fn_name, parameters)
    ...
    def defn_macro(tree: list, local_env: dict, module_env, eval_function) -> list or Token:
        ...
        for parameter_token_or_form in NML_M_ITERATE_AS_PAIRS(parameters_form):
            if isinstance(parameter_token_or_form, list):
                NML_M_ASSERT_FORM_IS_A_VECTOR(parameter_token_or_form, 'defn: can only destructure a Vector')
                right = []
                for x in NML_M_ITERATE_AS_PAIRS(parameter_token_or_form):
                    assert x.type() == Token.Identifier,         'defn: param name needs to be an Identifier'
                    right.append(x.dt().origin())
                parameters.append((f'{randstr()}', right))
            else:
                assert parameter_token_or_form.type() == Token.Identifier, \
                                                                 'defn: param name needs to be an Identifier'
                parameters.append((parameter_token_or_form.dt().origin(), None))

        fn_name = name_token.dt().origin()

        fni = fn.Fn(body_token_or_form, local_env, eval_function, fn_name, parameters, spec_list)
        ...
        for [idn_token_or_form, dtype_token_or_form] in NML_M_ITERATE_AS_PAIRS(bindings_form, 2):
            ...
            if isinstance(idn_token_or_form, list):
                if NML_M_FORM_IS_A_VECTOR(idn_token_or_form):
                    assert isinstance(evaluated, datatypes.Vector),           'let: right-side needs to be a Vector'
                    NML_M_ASSERT_COLL_FORM_ITEMS_COUNT_IS(
                        idn_token_or_form,
                        evaluated.count().reference(),   'let: lengths of the vectors from both sides do not match')
                    ...
                    assert isinstance(evaluated, datatypes.HashMap),         'let: right-side needs to be a HashMap'
                    _ = tuple(NML_M_ITERATE_AS_PAIRS(idn_token_or_form, 0))
                    assert len(_) == 2,       'let: HashMap form incorrect arity: expected exactly: Keyword, Vector'
                    assert isinstance(_[0].dt(), datatypes.Keyword),  'let: destructuring kind Keyword was expected'
                    NML_M_ASSERT_FORM_IS_A_VECTOR(_[1],                   'let: Vector of Identifiers was expected')
                    if _[0].dt().reference() == 'strs':
                        for lt_idn in NML_M_ITERATE_AS_PAIRS(_[1]):
                            assert lt_idn.type() == Token.Identifier, 'let: the left-side needs to be an Identifier'
                            local_env[lt_idn.dt().origin()] = evaluated.get(datatypes.String(lt_idn.dt().origin()))
                    elif _[0].dt().reference() == 'keys':
                        for lt_idn in NML_M_ITERATE_AS_PAIRS(_[1]):
                            assert lt_idn.type() == Token.Identifier, 'let: the left-side needs to be an Identifier'
                            local_env[lt_idn.dt().origin()] = evaluated.get(datatypes.Keyword(lt_idn.dt().origin()))
                    else:
                        raise AssertionError('let: at the moment, we can only take strings and keys from a HashMap')
                else:
                    raise AssertionError('let: at the moment, we can only destructure HashMaps and Vectors structs')
            else:
...
class BuiltinFunctions:
    ...
    @meta({'spec': [[Spec.ArityIs, 2],
           ...
           'docstring': 'Returns collection element by the key (map), index (vec) or element (set)'})
    def get_func(args: List[datatypes.Base]) -> datatypes.Base:
        """
        Builtin 'get' function implementation

        :param args: incoming 'get' function arguments (target collection and key, index or element).
        :return: datatypes.Base
        """
        ...
        return collection.get(by, default=datatypes.Nil('nil'))
        # Sadly, each Collection has support fot the default value, but Spec.validate() does not support vary arg len
    ...
    def map_func(args: List[datatypes.Base]) -> datatypes.Vector or datatypes.NException:
        ...
        for element in collection.items():
            ...
            if isinstance(res, datatypes.NException):
                return res  # <---------------- if we ran into NException instance, we need to propagate it
            _.append(res)
