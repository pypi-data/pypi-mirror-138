...
class InstanceEvents(event.Events):
    ...
    * the :class:`_orm.Mapper` class itself indicates listening for all
    ...
    @classmethod
    @util.preload_module("saorm14.orm")
    def _accept_with(cls, target):
        ...
        if isinstance(target, instrumentation.ClassManager):
            ...
            util.warn_deprecated(
                "The `saorm14.orm.mapper()` symbol is deprecated and "
                "will be removed in a future release. For the mapper-wide "
                "event target, use the 'saorm14.orm.Mapper' class.",
                "2.0",
            )
...
class MapperEvents(event.Events):
    ...
    * the :class:`_orm.Mapper` class itself indicates listening for all
    ...
    :param retval=False: when True, the user-defined event function
       ...
       * ``saorm14.orm.interfaces.EXT_CONTINUE`` - continue event
         processing normally.
       * ``saorm14.orm.interfaces.EXT_STOP`` - cancel all subsequent
...
class AttributeEvents(event.Events):
    ...
    def bulk_replace(self, target, values, initiator):
        ...
        incoming initiator::
            ...
            from saorm14.orm.attributes import OP_BULK_REPLACE
