...
:class:`~saorm14.orm.session.Session` within its transactional context.
This is most easily accomplished using the
:meth:`~.Session.execute` method, which returns a
:class:`~saorm14.engine.CursorResult` in the same manner as an
...
The current :class:`~saorm14.engine.Connection` held by the
:class:`~saorm14.orm.session.Session` is accessible using the
...
::
    ...
    from saorm14.sql import Update, Delete
    from saorm14.orm import Session, sessionmaker
    import random
