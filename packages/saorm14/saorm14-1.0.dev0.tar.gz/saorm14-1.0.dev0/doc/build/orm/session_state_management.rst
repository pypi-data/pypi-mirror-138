...
The :class:`~saorm14.orm.session.Session` itself acts somewhat like a
...
out of scope from within the :class:`~saorm14.orm.session.Session` as well
...
* An application which reads an object structure from a file and wishes to
  ...
  and the :class:`~saorm14.orm.session.Session` will
...
A surprise would occur if we said this::
    ...
    >>> a1 = session.merge(a1)
    >>> session.commit()
    saorm14.orm.exc.FlushError: New instance <Address at 0x1298f50>
...
is a quick way to check::
    ...
    >>> a1.user
    >>> a1.__dict__
    {'_sa_instance_state': <saorm14.orm.state.InstanceState object at 0x1298d10>,
        'user_id': 1,
        'id': 1,
        'user': None}
