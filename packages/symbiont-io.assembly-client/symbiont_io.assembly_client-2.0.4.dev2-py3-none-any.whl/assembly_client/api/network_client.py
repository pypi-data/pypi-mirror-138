...
class NetworkClient:
    ...
    def __getitem__(self, key_alias):
        ...
        """
        return KeyAliasClient(self, self.node_sessions[self.alias_locations[key_alias]], key_alias)
...
class KeyAliasClient():
    ...
    def __getattr__(self, contract_name):
        return ContractClient(self.network_client, self.session, self.key_alias, contract_name)
...
class ContractClient():
    ...
    def __getitem__(self, version):
        ...
        if len(s) == 1:
            ...
            raise Exception("Cannot parse {}, it needs to take a form `l-v.v.v` or `v.v.v`".format(version))
