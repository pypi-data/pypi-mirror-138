...
#             setattr(self, '_id', str(uuid4()))

#     def load(self, props) -> None:
#         """ sets props on the document instance """
#         for p in props:
#             if p == '_id':
#                 if not self._is_uuid(props.get(p)):
#                     raise InvalidDocumentID(f"{props.get(p)} is not a valid uuid")
#             setattr(self, p, props.get(p))

#     @property
#     def as_dictionary(self) -> dict:
#         """ returns a dictionary representation of the current document instance """
...
class Collection:
    ...
    def _statement_to_sql(self, key, op, val) -> str:
        ...
        valid_ops = ['lt', 'gt', 'lte', 'gte', 'eq', 'sw', 'ew', 'in']
        statement = None
