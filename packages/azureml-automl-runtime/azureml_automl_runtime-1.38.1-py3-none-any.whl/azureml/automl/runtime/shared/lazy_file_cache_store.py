...
class CachedValueMetadata:
    ...
    def __init__(self, local_path: str, remote_path: str, type: str) -> None:
        """
        CacheValue constructor.

        :param key: Key of the item in the cache
        :param local_path: path of the cached item on local system.
        :param remote_path: path of the cached item in remote cache store.
        :type: supported type for cached value
        """
        if type not in CachedValueMetadata.SUPPORTED_TYPES:
            msg = "Unsupported type value {}.".format(type)
...
class FileCachedValueMetadata(CachedValueMetadata):
    ...
    def update_local_path(self, key: str, parent_local_path: str) -> None:
        ...
        self.local_path = ".".join([
            os.path.join(parent_local_path, *self._get_qfn(key).split("/")),
            self.ext])
    ...
    def update_remote_path(self, key: str, parent_remote_path: str) -> None:
        ...
        self.remote_path = ".".join(
            [(pathlib.Path(parent_remote_path).joinpath(
                self._get_qfn(key))).as_posix(), self.ext])
...
class DirCachedValueMetadata(CachedValueMetadata):
    ...
    def __init__(self, key: str, local_path: str, remote_path: str = "", **kwargs: Any) -> None:
        ...
        if "dir_items" in kwargs:
            # To be used during json deserilization.
            # dir_items should not be send as parameter.
    ...
    def update_local_path(self, key: str, parent_local_path: str) -> None:
        ...
        for k, v in self.dir_items.items():
            v.update_local_path(k, self.local_path)
    ...
    def update_remote_path(self, key: str, parent_remote_path: str) -> None:
        ...
        for k, v in self.dir_items.items():
            v.update_remote_path(k, self.remote_path)
...
class _JSONUtils(json.JSONEncoder):
    ...
    def decode(jdict):
        ...
        if jdict.get('type') == CachedValueMetadata.TYPE_DIR:
            return DirCachedValueMetadata(**jdict)
        elif jdict.get('type') == CachedValueMetadata.TYPE_FILE:
...
class LazyFileCacheStore(CacheStore):
            ...
            for key in keys:
                try:
                    ...
                    for k in derived_keys:
                        ...
                        if item is not None and item.type == CachedValueMetadata.TYPE_FILE:
    ...
    def _is_valid_key(self, key: str) -> bool:
        ...
        for str in key.split("/"):
            ...
            if item is None:
                ...
                item = self.cache_items.get(key_to_check + "/")
            if item is not None and item.type == CachedValueMetadata.TYPE_DIR:
    ...
    def remove(self, key: str) -> None:
        ...
        # if to_remove.get('type') == 'dir':
        # shutil.rmtree(to_remove.get('local_path'))
        if to_remove.type == CachedValueMetadata.TYPE_DIR:
            shutil.rmtree(to_remove.local_path)
        else:
            # os.remove(os.path.join(self._root, to_remove.get('local_path')))
    ...
    def load(self) -> None:
        ...
        with self.log_activity():
            ...
            for entry in os.scandir(self._indices_dir_path):
                if entry.name == self._index_file_name:
                    continue
