        ...
        try:
            with self.log_activity():
                ...
                if len(local_files) > 0:  # we have some files already downloaded locally
                    run_files = self._get_keys_available_on_run()
                    # calculate the delta b/w what's available locally and what's currently on the Run
                    files_to_skip = run_files.intersection(local_files)
                    if len(files_to_skip) == len(local_files):
                        logger.info("flush: No new files were added to the cache store, skip uploading any data.")
                        return
                    if len(files_to_skip) > 0:
                        logger.info(f"flush: Uploading {len(files_to_skip)} files.")
                        for f in files_to_skip:
                            f_name = "{}/{}".format(self._artifact_path, f)
                            f_output_path = pathlib.Path(local_path, f_name)
                            f_output_path.unlink()
                self._run.upload_folder(name=self._artifact_path, path=self._root)
        except Exception as ex:
            logging_utilities.log_traceback(ex, logger, is_critical=False)
