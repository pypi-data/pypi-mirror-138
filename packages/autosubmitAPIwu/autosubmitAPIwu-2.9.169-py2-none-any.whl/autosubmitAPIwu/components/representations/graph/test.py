...
class TestGraph(unittest.TestCase):
  def setUp(self):
    self.test_graph = GraphRepresentation("a29z", Layout.STANDARD)           
  
  def tearDown(self):
    del self.test_graph
  
  def test_add_normal_edges(self):
    graph = GraphRepresentation("a0yh", Layout.STANDARD)    
  ...
  def test_wrong_layout(self):
    with self.assertRaises(ValueError):
      graph = GraphRepresentation("a29z", "otherlayout")
      graph.perform_calculations()
  
  def test_calculate_average_post_time(self):
    graph = GraphRepresentation("a29z", Layout.STANDARD)
  ...
  def test_grouped_by_date_member_dict(self):
    graph = GraphRepresentation("a29z", Layout.STANDARD, GroupedBy.DATE_MEMBER)
    graph.perform_calculations()
  ...
  def test_grouped_by_status(self):
    graph = GraphRepresentation("a29z", Layout.STANDARD, GroupedBy.STATUS)
    graph.perform_calculations()
    groups = graph._get_grouped_by_status_dict()
    self.assertTrue(len(groups) > 0)

  def test_grouped_by_wrong_parameter(self):
    graph = GraphRepresentation("a29z", Layout.STANDARD, "NONE")
    with self.assertRaises(ValueError):
      graph.perform_calculations()
