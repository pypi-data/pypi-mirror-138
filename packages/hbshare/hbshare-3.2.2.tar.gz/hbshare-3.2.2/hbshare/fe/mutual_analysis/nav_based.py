...
def get_benchmark_ret():
    ...
    return  benchmark_ret[['scenario','hb1y','tjyf']]
...
if __name__ == '__main__':
    ...
    # sql="select distinct(jjdm) from st_fund.t_st_gm_nhb where  rq1n!=99999  "
    # nav_jjdm_list=hbdb.db2df(sql, db='funduser')
    #
    # sql = "select distinct jjdm from st_fund.t_st_gm_jjxx where  cpfl='2' and (jjfl='1' or (jjfl='3' and (ejfl='35' or ejfl='37'))) "
    # stock_jjdm_list = hbdb.db2df(sql, db='funduser')
    # stock_jjdm_list=pd.merge(stock_jjdm_list,nav_jjdm_list,how='inner',on='jjdm')['jjdm'].tolist()
    ...
    saved_df.to_csv('scenario_ret.csv',index=False)
