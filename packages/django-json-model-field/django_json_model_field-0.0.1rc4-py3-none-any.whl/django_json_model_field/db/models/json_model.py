...
class JSONModel(metaclass=JSONModelBase):
    ...
    def get_prep_value(self) -> dict:
        ...
        self.full_clean()
        # Using each field's get_prep_value in turn will help ensure that the individual field values are JSON
        # serializable. This might not be a guarantee, but it is less likely that a value that is suitable to be stored
        # in a DB column would not be serializable.
        return {field.name: field.get_prep_value(getattr(self, field.name)) for field in self._meta.fields}

    def get_db_prep_value(self, connection: BaseDatabaseWrapper) -> dict:
        """
        ...
        # serializable. This might not be a guarantee, but it is less likely that a value that is suitable to be stored
        # in a DB column would not be serializable.
        return {field.name: field.get_db_prep_value(getattr(self, field.name), connection) for field in self._meta.fields}
    ...
    def _init_fields(self, initial: dict) -> None:
        ...
        for field in self._meta.fields:
            setattr(self, field.name, initial.get(field.name, field.get_default()))
