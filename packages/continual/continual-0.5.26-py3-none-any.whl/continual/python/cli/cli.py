...
def push_dbt(
    ...
    try:
        ...
        typer.secho("Using Continual environment '%s'\n" % (env.name), fg="green")
    except Exception as e:
        typer.secho(
    ...
    if env.data_store.type == "":
        web_ui_project_envs_url = f"{utils.get_endpoint()}/{env_name}/environments"
        typer.secho(
            f"Continual environment '{env_name}' does not have a data warehouse configured.",
            fg="red",
        )
        typer.secho(
