...
def evaluate(mixdb: dict,
    ...
    for mi in range(num_mixtures):
        ni = int(mixdb['mixtures'][mi]['noise_file_index'])
        ti = int(mixdb['mixtures'][mi]['target_file_index'])

        # Get mixture config parameters target gain (and target SNR above)
        taugi = int(mixdb['mixtures'][mi]['target_augmentation_index'])
        tsnr = str(mixdb['target_augmentations'][taugi]['snr'])

        if 'gain' in mixdb['target_augmentations'][taugi].keys():
            tgain = float(mixdb['target_augmentations'][taugi]['gain'])
        else:
            tgain = 1
        ...
        tridx = int(mixdb['mixtures'][mi]['truth_index']) - 1
        ...
        if mi == num_mixtures - 1:
            ...
            mxend = mixdb['mixtures'][mi + 1]['i_sample_offset']
            # out frames are possibly decimated/stride-combined
            fend = mixdb['mixtures'][mi + 1]['o_frame_offset']
            # in frames are transform
            ifend = mixdb['mixtures'][mi + 1]['i_frame_offset']

        # Select subset of waveforms, if present
        mxbegin = mixdb['mixtures'][mi]['i_sample_offset']
        if mixture is not None:
            mx = mixture[mxbegin:mxend] / data_scale
        else:
            mx = []

        if noise is not None:
            nt = noise[mxbegin:mxend] / data_scale
            # noise level in dB
        ...
        if target is not None:
            tt = target[mxbegin:mxend] / data_scale
            # target level in dB
        ...
        segsnr = segsnr[mixdb['mixtures'][mi]['i_frame_offset']:ifend]
        ...
        truth = truth[mixdb['mixtures'][mi]['o_frame_offset']:fend, ]

        if predict is not None:
            nno = predict[mixdb['mixtures'][mi]['o_frame_offset']:fend]
        ...
        if mi + 1 in plot_number:
            ...
            segsnrex = np.reshape(np.tile(10 * np.log10(segsnr + np.finfo(float).eps), [samples_per_transform, 1]),
                                  ...
                                  order='F')
            feat = feature[mixdb['mixtures'][mi]['o_frame_offset']:fend, ]
