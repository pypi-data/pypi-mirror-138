...
class ClickhouseCubeBackend(CubeBackend):
    ...
    def _prepare_db_query(
        ...
        settings_part = ", ".join(f"{k} = {v}" for k, v in self.default_settings.items())
        if settings_part:
    ...
    def _init_table(self, cube: Type[Cube]):
        ...
        for idx in meta.indexes:
            indexes.append(
                f"INDEX {idx.name} ({idx.expression}) TYPE {idx.type} GRANULARITY {idx.granularity}"
            )
        idx_part = ", ".join(indexes)
        if idx_part:
            field_part += ", " + idx_part
        # sorting key
