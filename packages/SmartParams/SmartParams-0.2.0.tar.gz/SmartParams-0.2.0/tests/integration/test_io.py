...
class TestSave(UnitCase):
    def test_save__yaml(self) -> None:
        data = dict(key='value')
        with tempfile.TemporaryDirectory() as temp_dir:
            path = Path(temp_dir, 'config', 'params.yaml')

            save(data=data, path=path)

            self.assertTrue(path.exists())
            self.assertEqual('key: value\n', path.read_text())

    def test_save__unknown(self) -> None:
        data = dict(key='value')
        path = Path('config', 'params.exe')

        self.assertRaises(ValueError, save, data=data, path=path)
