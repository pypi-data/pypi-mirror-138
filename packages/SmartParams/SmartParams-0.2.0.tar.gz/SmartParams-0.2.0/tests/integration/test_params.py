...
class TestSmartRunCase(UnitCase):
    ...
    def test_run__dump_with_path(self) -> None:
        with tempfile.NamedTemporaryFile(mode='w+', suffix='.yaml') as file:
            self.smart.run(path=Path(file.name), dump=True)

            self.assertEqual("arg1: str???\narg2: 10\n", file.read())

    @patch('smartparams.cli.sys', Mock(argv=[]))
    def test_run__dump_with_directory(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            expected = Path(directory, 'params.yaml')

            self.smart.run(path=Path(directory), dump=True)
    ...
    @patch('smartparams.cli.sys', Mock(argv=[]))
    @patch('smartparams.smart.print')
    def test_run__keys_with_path(self, print_mock: Mock) -> None:
        with tempfile.NamedTemporaryFile(mode='w+', suffix='.yaml') as file:
            file.write("arg1: string\n")
            file.seek(0)

            self.smart.run(path=Path(file.name), keys=True)

            print_mock.assert_called_once_with("- arg1\n- arg2\n")

    @patch('smartparams.cli.sys', Mock(argv=[]))
    @patch('smartparams.smart.print')
    def test_run__keys_without_path(self, print_mock: Mock) -> None:
        self.smart.run(path=None, keys=True)

        print_mock.assert_called_once_with("- arg2\n")
