...
from smartparams.utils import get_hints, get_name, get_nested, import_class
...
class Smart(Generic[_T]):
    ...
    def run(
        ...
        dump: bool = False,
        keys: bool = False,
        print_format: str = 'yaml',
    ) -> 'Smart':
        if path and path.is_dir():
            path = path.joinpath(self.default_params_filename)
        ...
        if args.dump:
            representation = self._representation(self._class, skip_default=bool(self._params))
            self.update(representation, override=False)

            if args.path:
                self.save(args.path)
            else:
                print(to_string(self.params, args.print_format))
        else:
            if args.keys:
                print(to_string(self.flatten_keys(), args.print_format))
    ...
    def _register_aliases(cls, aliases: Mapping[str, str]) -> None:
        for origin, alias in aliases.items():
            if origin in cls._aliases:
                warnings.warn(f"Override origin '{origin}'.")
                cls._origins.pop(cls._aliases.pop(origin))

            if alias in cls._origins:
                warnings.warn(f"Override alias '{alias}'.")
    ...
    def _update_from_smart(self, smart: 'Smart', override: bool) -> None:
        flatten_params = {key: self.pop(key) for key in self.flatten_keys()}

        for key in smart.flatten_keys():
            self.set(key, smart.get(key))
            if override:
                flatten_params.pop(key, None)
