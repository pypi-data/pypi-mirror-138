...
def view_model(model, filenames, rows, cols, template):
    ...
    def get_filters(image, layer, filter):
        ...
        layer = model.layers[layer]
        filters = tf.keras.Model(inputs=model.inputs, outputs=layer.output).predict(
            np.asarray([image])
        )
        lo = filter * rows * cols
        hi = min(lo + rows * cols, layer.output.shape[-1])
        return [filters[0][..., f] for f in range(lo, hi)] + [
    ...
    prev_image, images, next_image = prev_dropdown_next(
        "Images", [(filename, f) for f, filename in enumerate(filenames)], on_image
    )
    #
    def on_layer(change):
        filters.value = 0
    ...
    return VBox(
        [
            HBox([images, prev_image, next_image]),
            HBox([layers, prev_layer, next_layer]),
            HBox([filters, prev_filter, next_filter]),
            fig,
        ]
    )
