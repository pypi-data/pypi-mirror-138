...
class SVGRectangle(SVGPrimitive):
    def __init__(self, xy: Point, wh: Size, r=0, *args, **kwargs):
        super().__init__(*args, **kwargs)

        self.xy = xy
        self.wh = wh
        self.magic_h = 0.551915
        self.r = r
    ...
    def from_xml(_, x: minidom.Element):
        ...
        if x.hasAttribute("rx"):
            r = float(x.getAttribute("rx"))
        else:
            r=0

        return SVGRectangle(xy, wh, r=r, fill=fill)

    def to_path(self):
        print('##############to rectangle path')
        if self.r==0:
            print('in zero')
            ...
            commands = []
            #0,0附近的弧开始
            controlp_l = self.magic_h * self.r
            #c1
            p0, p1, p2, p3 = self.xy+Point(0,self.r), self.xy+Point(0,self.r-controlp_l),\
                             self.xy + Point(self.r-controlp_l, 0), self.xy+Point(self.r, 0)
            commands.append(SVGCommandBezier(p0, p1, p2, p3))
            #l1
            p0, p1 = self.xy+Point(self.r,0), self.xy + self.wh.xproj()-Point(self.r, 0)
            commands.append(SVGCommandLine(p0, p1))
            #c2
            p0, p1, p2, p3 = self.xy + self.wh.xproj()-Point(self.r,0), self.xy + self.wh.xproj() - Point(self.r - controlp_l,0),\
                             self.xy + self.wh.xproj() + Point(0, self.r - controlp_l), self.xy + self.wh.xproj() + Point(0, self.r)
            commands.append(SVGCommandBezier(p0, p1, p2, p3))
            #l2
            p0, p1 = self.xy + self.wh.xproj() + Point(0, self.r), self.xy + self.wh - Point(0, self.r)
            commands.append(SVGCommandLine(p0, p1))
            #c3
            p0, p1, p2, p3 = self.xy + self.wh - Point(0, self.r), self.xy + self.wh - Point(0, self.r - controlp_l), \
                             self.xy + self.wh - Point(self.r - controlp_l, 0), self.xy + self.wh - Point(self.r, 0)
            commands.append(SVGCommandBezier(p0, p1, p2, p3))
            #l3
            p0, p1 = self.xy + self.wh - Point(self.r, 0), self.xy + self.wh.yproj() + Point(self.r, 0)
            commands.append(SVGCommandLine(p0, p1))
            #c4
            p0, p1, p2, p3 = self.xy + self.wh.yproj() + Point(self.r, 0), self.xy + self.wh.yproj() + Point(self.r - controlp_l, 0), \
                             self.xy + self.wh.yproj() - Point(0, self.r - controlp_l), self.xy + self.wh.yproj() - Point(0, self.r)
            commands.append(SVGCommandBezier(p0, p1, p2, p3))
            #l4
            p0, p1 = self.xy + self.wh.yproj() - Point(0, self.r), self.xy + Point(0, self.r)
            commands.append(SVGCommandLine(p0, p1))
            return SVGPath(commands, closed=True).to_group(fill=self.fill)
