...
def get_hubspot_deals_all_fields(API_KEY, 
    ...
    deals_all_fields = deals_all_fields.reset_index(drop = True, inplace = True)
...
def get_hubspot_contact(API_KEY,
    ...
    start_date = (date.today()-td(days=n_days)).isoformat()
    ...
    while i >0 :
        ...
        for dd in d1:
            ...
            try:
                start_date = dt.strptime(start_date,'%Y-%m-%dT%H:%M:%S.%f')
            except:
