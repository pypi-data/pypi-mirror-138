...
def categorical_focal_loss(alpha, gamma=2.,label_smoothing=0.0):
    """
    ...
    def categorical_focal_loss_fixed(y_true, y_pred):
        ...
        shrink = (1.0-label_smoothing_v)
        y_true = label_smoothing_v/classes_v + y_true*shrink

        # Calculate Cross Entropy
        cross_entropy = -y_true * K.log(y_pred)

        # Calculate Focal Loss
        loss = alpha_v * K.pow(1 - y_pred, gamma) * cross_entropy
...
def loss_adapter(loss_function, factor=1, expand=False, from_logits=False):
    ...
    def loss(y_true, y_pred):
        ...
        if expand_:
            new_height = int(tf.math.round(
                tf.shape(y_pred,  out_type=tf.float32)[1] * scale))
            new_width = int(tf.math.round(
                tf.shape(y_pred,  out_type=tf.float32)[2] * scale))
            y_pred = tf.image.resize(
                y_pred, [new_height, new_width], method='nearest')
        else:
            new_height = int(tf.math.round(
                tf.shape(y_pred,  out_type=tf.float32)[1]))
            new_width = int(tf.math.round(
                tf.shape(y_pred,  out_type=tf.float32)[2]))
