...
def focal_categorical_crossentropy(gamma, n_classes, label_smoothing=0.0):
    ...
    def focal_categorical_crossentropy_(target, pred):
        return - ((1.0-label_smoothing_v)*(K.sum(target*tf.pow((1.0-pred), gamma_v)*K.log(pred + K.epsilon()))) + (label_smoothing_v/(n_classes_v-1))*(K.sum((1.0-target)*tf.pow((pred), gamma_v)*K.log(((1.0 - pred) + K.epsilon())))))
