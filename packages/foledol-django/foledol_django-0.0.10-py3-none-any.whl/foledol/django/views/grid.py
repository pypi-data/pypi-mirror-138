...
from foledol.django.models import Grid
...
class ColumnSubTable(Table):
    def __init__(self, rows, read_only=False):
        super().__init__(rows, [
            ...
            TableColumn('criteria', "Critère", method='criteria_as_str'),
...
def grid_form(request, grid, read_only=False):
    ...
    if columns:
        ...
        sort = get_param(request, context, 'sort', default_sort) if path != ':/grids' else default_sort
...
def grid_rows(grid, rows, fields):
    for column in grid.column_set.all():
        ...
        if field and field.external and not column.hidden:
            rows = rows.annotate(**{field.key: F(field.key)})

    for column in grid.column_set.all():
        rows = column.filter(rows)
