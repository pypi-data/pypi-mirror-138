...
class SeismicCube(PetrelObject):
    ...
    def set_value(self, value: float) -> bool:
        ...
        if not ok:
            raise exceptions.PythonToolException("Could not set seismic volume")
        return ok
    ...
    def extent(self) -> primitives.Extent:
        ...
        Traces are indexed by `i` and `j`, with `k` specifying the sample number in the trace.
    ...
    def layer(self, k: int) -> Chunk:
        ...
        Raises:
            ValueError: if the cube does not have the column specified
        """
    ...
    def column(self, i: int, j: int) -> Chunk:
        ...
        Args:
            i (int): the index in the i-direction
            j (int): the index in the j-direction
    ...
    def chunk(self, 
        ...
        Args:
            irange (tuple(int, int)): Inclusive range of i-values, or () or None for all
            jrange (tuple(int, int)): Inclusive range of j-values, or () or None for all
            krange (tuple(int, int)): Inclusive range of k-values, or () or None for allchunks
    ...
    def all(self) -> Chunk:
        ...
        Please note that this can be an expensive operation in time and memory depending on the size of cube.
    ...
    def columns(self, irange: typing.Tuple[int, int] = None, jrange: typing.Tuple[int, int] = None) -> typing.Iterator[Chunk]:
        ...
        Returns a generator of column
        :class:`cegalprizm.pythontool.Chunk` objects covering the
        `irange` and `jrange` passed.

        Args:

          irange (iterable): the range of i-values to generate columns for.  If `None`,
                             generate for all i-values

          jrange (iterable): the range of j-values to generate columns
                             for.  If `None`, generate for all j-values.
    ...
    def indices(self, x: float, y: float, z: float) -> primitives.Indices:
        ...
        Returns:
            ...
            cegalprizm.pythontool.Indices: An object representing
    ...
    def annotation(self, i: int, j: int, k: int=0) -> primitives.Annotation:
        ...
        Returns:
            ...
            cegalprizm.pythontool.Annotation: an object
            representing the annotations at these indices

        Raises:
            ValueError: if the annotations are beyond the valid range
        """
    ...
    def annotation_indices(self, inline: int, crossline: int, samplenumber: int = 1) -> primitives.Indices:
        """The indices of a particular inline/crossline/samplenumber
    ...
    def has_same_parent(self, other: "SeismicCube") -> bool:
        ...
        Args:
            other (cegalprizm.pythontool.SeismicCube): the other seismic cube
    ...
    def positions_to_ijks(self, positions: typing.Tuple[typing.List[float], typing.List[float], typing.List[float]])\
        ...
        """Converts a 3-tuple with xyzs to ijk

        Args:
        positions: 3-tuple with lists representing x, y and time/depth coordinates.

        Returns:
            3-tuple with lists representing the closest i, j and k indices to each coordinate.
        """
        [x, y, z] = positions
        lst_is = []
        lst_js = []
    ...
    def ijks_to_positions(self, indices: typing.Tuple[typing.List[float], typing.List[float], typing.List[float]])\
        -> typing.Tuple[typing.List[float], typing.List[float], typing.List[float]]:
        """Converts a 3-tuple with ijks to xyzs.


        Args:
        indices: 3-tuple with lists with i, j and k indices.

        Returns:
            3-tuple with lists representing the x, y and time/depth coordinates of the indices.
        """
        extent = self.extent
...
class SeismicLine(PetrelObject):
    ...
    def columns(self, jrange: typing.Iterable[int] = None) -> typing.Iterable[chunk.Chunk]:
        ...
        Raises:
            ValueError: if the j-index is invalid.
    ...
    def has_same_parent(self, other: "SeismicLine") -> bool:
        ...
        Args:
            other (cegalprizm.pythontool.SeismicLine): the other seismic line
