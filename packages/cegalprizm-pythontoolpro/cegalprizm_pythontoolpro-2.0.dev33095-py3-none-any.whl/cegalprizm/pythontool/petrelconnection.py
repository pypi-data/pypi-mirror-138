...
class PetrelConnection:
    ...
    by default Python Tool Pro will try to connect to a running Hub server and will create a default Petrel context (petrel_ctx). This will work if 1 Petrel connector (1 Petrel instace) is connected to the Hub server. If multiple Petrel connectors are available you need to define the Petrel context to ensure you connect to the correct Petrel instance.
    If no Hub connection can be established Python Tool Pro will try to establish a connection directly with Petrel. This only works when Python Tool Pro and Petrel are running on the same machine. In this case the argument 'host' will be set to 'localhost' and 'port' will read the portnumber
    from a 'session' file in the users AppData. The 'session' file is created or updated on Petrel start-up. It can be updated manually with the portnumber stated in Marina.


    Args:
        petrel_ctx(): 
        port (int, optional): port number for communication with server. Reads portnumber from file in users AppData. This can be overwritten manually.
        host (str, optional): Name of host. Defaults to 'localhost'.
        use_mmap (bool, optional): Memory mapping is disabled after version 1.2. This is optional to ensure backwards compatibility. Defaults to False.
        allow_experimental (bool, optional): Enable experimental methods to be called. Defaults to False
        enable_history (bool, optional): Petrel data object history is updated when changed from cegalprizm.Python Tool Pro. Defaults to True.
    """
    _keystone = None
    _no_keystone = None
    _keystone_url = None
    ...
    def __repr__(self) -> str:
        ...
        return 'version: {0}\nhost: {1}\nport: {2}\nis open: {3}'.format(package_version, host, port, opened)
    ...
    def _init_array_transfer_mode(self):
        self._preferred_subchunk_size_bytes = self.preferred_streamed_unit_bytes
    
    @property
    def preferred_streamed_unit_bytes(self) -> int:
        """When streaming large amounts of data to and from Petrel, the data is chopped
        into smaller units. This is the approximate maximum size of each unit.
        By default the value is 2MB = 2 * 1024 * 1024 = 2097152.

        Returns:
            int: Number of bytes in each streamed unit of data.
        """
        return self._preferred_streamed_unit_bytes

    @preferred_streamed_unit_bytes.setter
    def preferred_streamed_unit_bytes(self, value: int):
        """When streaming large amounts of data to and from Petrel, the data is chopped
        into smaller units. This is the approximate maximum size of each unit.
        By default the value is 2 MB = 2097152 bytes.

        It should be no reason for changing this value. A value close to 4MB or larger will
        make the streaming fail. A very small value will dramatically increase the time to
        stream data.

        Args:
            value (int): Number of bytes in each streamed unit of data.
        """
        self._preferred_streamed_unit_bytes = value
        self._init_array_transfer_mode()

    @property
    def streamed_server_chunk_buffer_MB(self) -> int:
        """When streaming large amounts of data from the client script to Petrel,
        the streamed data are buffered on the server before written to the Petrel domain objects.
        This speeds up streaming of large data objects.
        This property is the size of the server's buffer and defaults to 100(MB).

        It is a potential for speeding up streaming from client script to server by increasing 
        this number, but to a cost of memory usage on the server. Thus, the value may be 
        increased if the server has sufficient RAM available or decreased (even to 0), if the
        server has very limited memory available.

        Returns:
            int: Size of the server's chunk data buffer in mega bytes.
        """
        return self._streamed_server_chunk_buffer_MB

    @streamed_server_chunk_buffer_MB.setter
    def streamed_server_chunk_buffer_MB(self, value: int):
        """When streaming large amounts of data from the client script to Petrel,
        the streamed data are buffered on the server before written to the Petrel domain objects.
        This speeds up streaming of large data objects.
        This property is the size of the server's buffer and defaults to 100(MB).
        ...
        Returns:
            int: Size of the server's chunk data buffer in mega bytes. Minimum 0 and maximum 1024.
        """
        max_limit = 1024
        if not isinstance(value, int) or value < 0 or value > max_limit:
            raise PythonToolException(f'''Size of chunk buffer on server when uploading data must be set to an integer between 0 and {max_limit} (MB)''')

        self._streamed_server_chunk_buffer_MB = value

    def _set_report(self, report):
        self._report = report.message
        self._was_success = report.value

# TODO Implement mechanism an _all_ requests not returning values before uncommenting these
#    def get_report(self): 
#        return self._report
...
#        return self._was_success
    ...
    def import_workflows(self, projectPath: str, workflowNames: typing.List[str]) -> typing.List[Workflow]:
        ...
        Args:
           projectPath(str): the file path to the Petrel project
           workflowNames(list of str): the Petrel name[s] of the workflow[s] to be imported
        """
    ...
    def get_current_project_name(self) -> str:
        """The name of the currently active Petrel project.

        Returns:
            string: Name of the currently avtive Petrel project.
        """
        self._opened_test()
        request = petrelinterface_pb2.EmptyRequest()
    ...
    def get_petrelobjects_by_guids(self, guids: typing.List[str]) -> typing.List[object]:
        ...
        Args:
            List[str]: A list of GUIDs as strings of the objects to be returned

        Returns: 
            List[object]: The objects for the given GUIDs. If GUID  does not exist in Petrel project 'None' is returned.
        """
        self._opened_test()
    ...
    def get_petrel_project_units(self) -> typing.Dict[str, str]:
        ...
        Returns: 
            dict[str, str]: A dictionary with the storage units and their unit symbols
        """
        self._opened_test()
        request = petrelinterface_pb2.EmptyRequest()
    ...
    def append_scriptname_to_history(self, path_to_append: str) -> str:
        ...
        Must be set after open() and should not be changed later in the script/notebook.
        
        Example:

        from cegalprizm.pythontool.petrelconnection import PetrelConnection, make_connection
        petrel = PetrelConnection(port=40129, enable_history=False, allow_experimental=True)
        petrel.open()
    ...
    def _get_well(self, path):
        """Get a well in Petrel as a Borehole by its path or GUID as printed by method show_wells.

        Args:
            path (string): Either the path or GUID of the well.

        Returns:
            Well: Reference to a well (borehole) object in the current Petrel project.
        """
        self._opened_test()
        request = petrelinterface_pb2.PetrelObjectRequest(node_path = path)
