...
class Well(PetrelObject):
    ...
    def logs_dataframe(self, 
        ...
        Args:
        global_well_logs:
            a single WellLog, DiscreteWellLog, GlobalWellLog, or DiscreteGlobalWellLogs or a list of either of them
        
        discrete_data_as (str): Default value is 'string'
    ...
    def md_to_xytime(self, md: typing.List[float])\
        ...
        Args:
            time: List with MD values

        Returns:
            3-tuple with lists representing the X, Y and Z (Time) coordinates of the MD values.
        """               
        lst_xs = []
        lst_ys = []
        lst_zs = []
    ...
    @experimental_method
    def md_to_xydepth(self, depths: typing.List[float])\
            -> typing.Tuple[typing.List[float], typing.List[float], typing.List[float]]:
        """Converts a list of MD values to X, Y and Z (depth)

        Args:
            depths: List with MD values

        Returns:
            3-tuple with lists representing the X, Y and Z (depth) coordinates of the MD values.
        ...
        lst_zs = []
        n = 1000
        for i in range(0, len(depths), n):
            data = self._borehole_object_link.GetTvdPosition(depths[i:i+n])
            lst_xs.append(data[0])
            lst_ys.append(data[1])
            lst_zs.append(data[2])
        d = ([x for xs in lst_xs for x in xs ], 
...
# Subclass Well just to make an alias called Borehole for backward compatibility
class Borehole(Well):
    def __init__(self, petrel_object_link):
        super(Borehole, self).__init__(petrel_object_link)
