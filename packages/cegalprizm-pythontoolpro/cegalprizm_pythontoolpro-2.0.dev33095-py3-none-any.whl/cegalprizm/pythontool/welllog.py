...
class LogSample(object):
    ...
    def __init__(self, welllog, md: float, x, y, z, twt:float, tvd:float, value:float, is_discrete):
    ...
    def _is_missing_value(self, v):
        if self._is_discrete:
            return v == _system().Int32.MaxValue
        else:
    ...
    @property
    def md(self) -> float:
        """The measured depth of the sample

        Returns:
            The measured depth of the sample in units consistent with Petrel
    ...
    @property
    def twt(self) -> float:
        """The two-way-time 'depth' of the sample
    ...
    @property
    def tvd(self) -> float:
        """The true vertical depth of the sample

        Returns:
            The true vertical depth of the sample in units consistent with Petrel
    ...
    def value(self, v: float):
        ...
        if self._welllog is not None:
            if self._welllog.readonly:
                raise exceptions.PythonToolException("Log is readonly")
            # TODO: consider disallowing this as it's too easy to write an n^2 loop
            self._welllog._petrel_object_link.SetSample(self._md, v2)  # slow!
        self._value = v
...
class LogSamples(object):
    ...
    **Example**:
    ...
    def __init__(self, welllog):
        self._welllog = welllog
        self._samples = None

    def _load_sample_cache(self) -> typing.List[LogSample] :
        is_discrete = isinstance(self._welllog, DiscreteWellLog)
        cs_samples = self._welllog._petrel_object_link.Samples()
        if cs_samples is None:
            cs_samples = []
        self._samples = [
            LogSample(
                self._welllog, s.Md, s.X, s.Y, s.ZMd, s.ZTwt, s.ZTvd, s.Value, is_discrete
            )
            for s in cs_samples
        ]
    ...
    def __getitem__(self, idx: int) -> LogSample:
        if self._samples is None:
            self._samples = self._load_sample_cache()
    ...
    def __str__(self) -> str:
        if self._samples is None:
            self._samples = self._load_sample_cache()
        return "LogSamples(welllog=%s, count=%s)" % (self._welllog, len(self._samples))

    def clone(self) -> "LogSamples":
        """Creates a copy of this `LogSamples` object

        Creates a 'disconnected' copy of this `LogSamples`.  A
        'disconnected' `LogSamples` has no parent `WellLog`, so any
        values assigned to it are not transfered to Petrel
        immediately.  This can greatly improve performance.
        ...
        **Example**:
        ...
        .. code-block:: python
          ...
          temp_samples = mylog.samples.clone()
          # do the calculation on a disconnected object for speed
          for s in temp_samples:
              s.value = s.value / 100

          # assign all samples back to Petrel in one go
          mylog.samples = temp_samples
        """
        if self._samples is None:
            self._samples = self._load_sample_cache()
        ...
        disconnected_log_samples = LogSamples(None)
        disconnected_log_samples._samples = disconnected_samples
        return disconnected_log_samples
...
class WellLog(PetrelObject):
    ...
    @property
    def borehole(self) -> typing.Optional["Well"]:
        """This is an alias for well.
        """
        return self.well
    ...
    def set_values(self, mds: typing.List[float], values: typing.List[typing.Optional[float]]) -> None:
        ...
        self.samples._load_sample_cache()
...
class GlobalWellLog(PetrelObject):
    ...
    def log(self, well_name: str) -> WellLog:
        ...
        Args: 
            well_name (string): The Petrel name of the well
    ...
    @property
    def logs(self) -> "Logs":
        """The logs in this Global Well Log

        Returns:
            An iterable collection of `cegalprizm.pythontool.WellLog` objects
        """
...
class DiscreteGlobalWellLog(PetrelObject):
    ...
    @property
    def logs(self) -> "Logs":
        """The discrete logs in this Global Well Log

        Returns:
            An iterable collection of `cegalprizm.pythontool.DiscreteWellLog` objects
        """
        if self._logs is None:
            self._logs = Logs(self)
        return self._logs
...
class DiscreteWellLog(PetrelObject):
    ...
    @property
    def borehole(self) -> "borehole.Well":
        """This is an alias for well
        """
        return self.well
...
class Logs(object):
    ...
    def __init__(self, parent):
        ...
        from cegalprizm.pythontool import WellLog, DiscreteWellLog, Borehole, Well

        if isinstance(parent, Borehole) or isinstance(parent, Well):
            ...
            raise TypeError(
                "Parent must be Well, Borehole, GlobalWellLog or DiscreteGlobalWellLog"
            )
    ...
    def __getitem__(self, key: str) -> typing.Union[WellLog, DiscreteWellLog]:
        ...
        def key_extractor_well_name(log):
            return log.borehole.petrel_name
