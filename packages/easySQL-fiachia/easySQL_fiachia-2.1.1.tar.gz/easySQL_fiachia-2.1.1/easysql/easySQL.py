...
class DataSql:
    ...
    def add(self, data_from_dataSQL):
        ...
        :param data_from_dataSQL: [DataSql]
        :return:
        """
    ...
    def is_empty(self):
        return not (self.attribute or self.value_list or self.data_list)
...
class EasySql:
    ...
    def add(self, table_name, is_commit=True, data_from_dataSql=None, **kwargs):
        """
        添加数据

        :param table_name: [str] 数据表名
        :param is_commit: [bool] 是否立即提交
        :param data_from_dataSql: [DataSql/None] 添加整个数据集中的数据，与kwargs手动输入数据集冲突，不要同时使用
    ...
    def add_list_old(self, table_name, data_list_long=None, **kwargs):
        temp = 0
        for key in kwargs.keys():
