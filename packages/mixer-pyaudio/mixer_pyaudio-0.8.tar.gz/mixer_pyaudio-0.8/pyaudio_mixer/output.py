...
class OutputTrack:
    ...
    `reserved` : bool
        The mixer class has a property called `available_output_track` and it returns all output tracks that are not playing. You can set this parameter to True to tell the mixer that this track is always unavailable regardless of whether it is playing or not. Defaults to False.
    """
