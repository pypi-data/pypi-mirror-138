...
from nlp_tools.tasks.base_model import BaseModel
    ...
    def evaluate(self,
        ...
        if should_round:
            ...
            if self.processor.output_dim != 1:
                raise ValueError("Evaluate with round fuction only accept 1D output")
            y_pred = [round_func(i) for i in y_pred]
            report = metrics.classification_report(y_data,
                                                   y_pred,
                                                   digits=digits)
            report_dict = metrics.classification_report(y_data,
                                                        y_pred,
                                                        output_dict=True,
                                                        digits=digits)
            print(report)
