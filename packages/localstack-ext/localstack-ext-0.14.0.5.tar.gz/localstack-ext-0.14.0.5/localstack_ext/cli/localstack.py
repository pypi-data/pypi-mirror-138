...
class ProCliPlugin(LocalstackCliPlugin):
 ...
 def should_load(self):
  ...
  return wBNtE if e else wBNtP
 def is_active(self):
  return self.should_load()
 def attach(self,cli:LocalstackCli)->wBNtz:
...
@click.option("--revert",is_flag=wBNtE,help="Revert systemd-resolved settings for the docker interface")
def cmd_dns_systemd(revert:wBNto):
...
def _cpvcs_initialized(pod_name:wBNtY)->wBNto:
...
@click.option("-n","--name",help="Name of the cloud pod",required=wBNtE)
def cmd_cpvcs_init(name:wBNtY):
...
@click.option("-n","--name",help="Name of the cloud pod",required=wBNtE)
@click.option("-r","--remote",help="Whether the Pod should also be deleted remotely.",is_flag=wBNtE,default=wBNtP)
def cmd_cpvcs_delete(name:wBNtY,remote:wBNto):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=wBNtE)
def cmd_cpvcs_register_remote(name:wBNtY):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Current Name of the cloud pod",required=wBNtE)
@click.option("-nn","--new-name",help="New name of the cloud pod",required=wBNtE)
def cmd_cpvcs_rename(name:wBNtY,new_name:wBNtY):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-n","--name",help="Name of the cloud pod",required=wBNtE)
def cmd_cpvcs_commit(message:wBNtY,name:wBNtY):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("--squash",is_flag=wBNtE,help="Squashes commits together, so only the latest commit is stored in the revision graph")
@click.option("--three-way",is_flag=wBNtE,default=wBNtP,help="")
@click.option("-m","--message",help="Add a comment describing the version")
@click.option("-n","--name",help="Name of the cloud pod",required=wBNtE)
def cmd_cpvcs_push(squash:wBNto,message:wBNtY,name:wBNtY,three_way:wBNto):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-n","--name",help="Name of the cloud pod",required=wBNtE)
@click.option("-v","--version",type=wBNtX)
@click.option("-m","--message",required=wBNtP)
def cmd_cpvcs_push_overwrite(version:wBNtX,message:wBNtY,name:wBNtY):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-v","--version",default="-1",type=wBNtX,help="Loads the state of the specified version - Most recent one by default")
@click.option("--reset",is_flag=wBNtE,default=wBNtP,help="Will reset the application state before injecting")
@click.option("-n","--name",help="Name of the cloud pod",required=wBNtE)
def cmd_cpvcs_inject(version:wBNtX,reset:wBNto,name:wBNtY):
 from localstack_ext.bootstrap import pods_client
...
@click.option("--inject/--no-inject",default=wBNtE,help="Whether the latest version of the pulled pod should be injected")
@click.option("--reset/--no-reset",default=wBNtE,help="Whether the current application state should be reset after the pod has been pulled")
@click.option("-n","--name",help="Name of the cloud pod",required=wBNtE)
@click.option("--lazy/--eager",default=wBNtE,help="Will only fetch references to existing versions, i.e. version state is only downloaded when required")
@cpvcs.command(name="pull",help="Injects the state from a version into the application runtime")
def cmd_cpvcs_pull(name:wBNtY,inject:wBNto,reset:wBNto,lazy:wBNto):
 from localstack_ext.bootstrap import pods_client
...
@click.option("--remote","-r",is_flag=wBNtE,default=wBNtP)
def cmd_cpvcs_list_pods(remote:wBNto):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=wBNtE)
def cmd_cpvcs_versions(name:wBNtY):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",required=wBNtE,type=wBNtX)
@click.option("-n","--name",help="Name of the cloud pod",required=wBNtE)
def cmd_cpvcs_version_info(version:wBNtX,name:wBNtY):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",type=wBNtX,default=-1,help="Latest version by default")
@click.option("-n","--name",help="Name of the cloud pod",required=wBNtE)
def cmd_cpvcs_version_metamodel(version:wBNtX,name:wBNtY):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",required=wBNtE,type=wBNtX,help="The version the state should be set to")
@click.option("--inject/--no-inject",default=wBNtE,help="Whether the state should be directly injected into the application runtime after changing version")
@click.option("--reset/--no-reset",default=wBNtE,help="Whether the current application state should be reset before changing version")
@click.option("--commit-before",is_flag=wBNtP,help="Whether the current application state should be commited to the currently selected version before changing version")
@click.option("-n","--name",help="Name of the cloud pod",required=wBNtE)
def cmd_cpvcs_set_version(version:wBNtX,inject:wBNto,reset:wBNto,commit_before:wBNto,name:wBNtY):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=wBNtE)
def cmd_cpvcs_commits(version:wBNtX,name:wBNtY):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("--version","-v",required=wBNtE)
@click.option("--commit","-c",required=wBNtE)
@click.option("-n","--name",help="Name of the cloud pod",required=wBNtE)
def cmd_cpvcs_commit_diff(version:wBNtX,commit:wBNtX,name:wBNtY):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
def cmd_pod_list():
 ...
 from localstack_ext.bootstrap import pods_client
 try:
  result=pods_client.list_pods(wBNtz)
  status.stop()
...
def print_table(columns:Dict[wBNtY,wBNtY],rows:List[Dict[wBNtY,Any]],formatters:Dict[wBNtY,Callable[[Any],wBNtY]]=wBNtz):
 from rich.table import Table
 if formatters is wBNtz:
  formatters=wBNtj()
 t=Table()
...
class ConsoleLogger(logging.Logger):
 def __init__(self,name):
  wBNtJ(ConsoleLogger,self).__init__(name)
 def info(self,msg:Any,*args:Any,**kwargs:Any)->wBNtz:
  console.print(msg%args)
 def warning(self,msg:Any,*args:Any,**kwargs:Any)->wBNtz:
  console.print("[red]Warning:[/red] ",msg%args)
 def error(self,msg:Any,*args:Any,**kwargs:Any)->wBNtz:
  console.print("[red]Error:[/red] ",msg%args)
 def exception(self,msg:Any,*args:Any,**kwargs:Any)->wBNtz:
  console.print("[red]Error:[/red] ",msg%args)
  console.print_exception()
