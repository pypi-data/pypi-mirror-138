...
class CloudFrontOriginAccessIdentity(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return result and result[0]or CcaOd
 @CcaOk
...
class CloudFrontFunction(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  return CcaOf(CloudFrontFunction,self).get_cfn_attribute(attribute_name)
 @CcaOY
...
class CloudFrontDistribution(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  origins=origins if CcaOI(origins,CcaOj)else origins.get("Items")
  origins_to_create=CcaOt([orig["DomainName"]for orig in origins])
  aliases=props.get("Aliases",[])
  aliases=aliases if CcaOI(aliases,CcaOj)else aliases.get("Items")
  aliases_to_create=CcaOt(aliases)
  for distr in distrs:
   origins=CcaOt([orig.get("DomainName")for orig in distr.get("Origins",{})["Items"]])
   aliases=CcaOt(distr.get("Aliases",{}).get("Items",[]))
 ...
 def get_deploy_templates():
  def lambda_get_distribution_config(params,**kwargs):
   ...
   config["IsIPV6Enabled"]=config.pop("IPV6Enabled",CcaOT)
   config["Enabled"]=CcaOV(config.get("Enabled")).lower()=="true"
   cert=config["ViewerCertificate"]=config.get("ViewerCertificate",{})
   custom_errors=config.get("CustomErrorResponses",[])
   custom_errors=(custom_errors.get("Items",[])if CcaOI(custom_errors,CcaOE)else custom_errors)
   for item in custom_errors:
    item["ResponseCode"]=CcaOV(item.get("ResponseCode"))
   cache=config["DefaultCacheBehavior"]
   cache["TrustedSigners"]=cache.get("TrustedSigners")or{"Quantity":0,"Enabled":CcaOT}
   cache["MinTTL"]=cache.get("MinTTL")or 600
   forwarded=cache["ForwardedValues"]=cache.get("ForwardedValues",{})
   forwarded["QueryString"]=CcaOV(forwarded.get("QueryString")).lower()=="true"
   ...
   def convert_list(key,parent=CcaOd):
    parent=config if parent is CcaOd else parent
    items=parent.get(key)or[]
    if CcaOI(items,CcaOE)and "Items" in items and "Quantity" in items:
     return
    parent[key]={"Quantity":CcaOu(items),"Items":items}
   cache_behavs=config.get("CacheBehaviors",[])
   cache_behavs=(cache_behavs.get("Items",[])if CcaOI(cache_behavs,CcaOE)else cache_behavs)
