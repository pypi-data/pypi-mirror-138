...
class SESTemplate(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return(template or[niRWz])[0]
 def get_physical_resource_id(self,attribute=niRWz,**kwargs):
 ...
 def update_resource(self,new_resource,stack_name,resources):
  ...
  if niRWa(template.get(attr,"")==new_template.get(attr,"")for attr in["SubjectPart","TextPart","HtmlPart"]):
   return
...
class SESReceiptRuleSet(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute):
  return niRWg(SESReceiptRuleSet,self).get_cfn_attribute(attribute)
 @niRWL
 def add_defaults(resource,stack_name:niRWK):
...
class SESReceiptRule(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute):
  return niRWg(SESReceiptRule,self).get_cfn_attribute(attribute)
 @niRWm
