...
class EC2ElasticIP(GenericBaseModel):
 ...
 def cloudformation_type():
  return "AWS::EC2::EIP"
 def get_physical_resource_id(self,attribute=hFAXd,**kwargs):
...
class SecurityGroupInOrEgress(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  for perm in perms:
   if hFAXU(perm["IpProtocol"])!=hFAXU(rp["IpProtocol"]):
    continue
 ...
 @hFAXT
 def get_vpc_and_name_for_security_group(cls,group_id:hFAXU)->Tuple[hFAXU,hFAXU]:
  client=aws_stack.connect_to_service("ec2")
  groups=client.describe_security_groups(GroupIds=[group_id])["SecurityGroups"]
  vpc_id=groups and groups[0].get("VpcId")or hFAXd
  group_name=groups and groups[0]["GroupName"]or hFAXd
 ...
 def get_deploy_templates(cls):
  def create_params(params,**kwargs):
   ...
   source_group_id=result.pop("SourceSecurityGroupId",hFAXd)
   vpc_id=hFAXd
   ...
   dst_group_id=result.pop("DestinationSecurityGroupId",hFAXd)
   ...
   if result.get("IpProtocol"):
    result["IpProtocol"]=hFAXU(result.get("IpProtocol"))
   description=result.pop("Description",hFAXd)
   cidr_ipv6=result.pop("CidrIpv6",hFAXd)
