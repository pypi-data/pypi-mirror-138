...
class StateFileLocator(abc.ABC):
 location="_none_"
 active_instance=rWyGx
 @abc.abstractmethod
 def get_state_file_location_by_key(self,key:rWyGP)->rWyGP:
  pass
 @rWyGz
 def get(cls,requested_file_locator:rWyGP):
...
class ObjectStorageProvider(abc.ABC):
 ...
 active_instance=rWyGx
 @rWyGz
 def get(cls,state_file_locator:rWyGP,requested_storage:rWyGP,serializers:Dict[rWyGP,CPVCSSerializer]):
  ...
  return cls.active_instance
 def __init__(self,state_file_locator:StateFileLocator,serializers:Dict[rWyGP,CPVCSSerializer]):
 ...
 def get_state_file_location_by_key(self,key:rWyGP)->rWyGP:
  pass
 @abc.abstractmethod
 def get_version_by_key(self,key:rWyGP)->Version:
  pass
 @abc.abstractmethod
 def get_revision_by_key(self,key:rWyGP)->Revision:
  pass
 @abc.abstractmethod
 def get_revision_or_version_by_key(self,key:rWyGP)->Union[Revision,Version]:
  pass
 @abc.abstractmethod
 def get_delta_file_by_key(self,key:rWyGP)->rWyGP:
  pass
 @abc.abstractmethod
 def upsert_objects(self,*args:CPVCSObj)->List[rWyGP]:
  pass
 @abc.abstractmethod
 def update_revision_key(self,old_key:rWyGP,new_key:rWyGP,referenced_by_version:rWyGP=rWyGx):
  pass
 @abc.abstractmethod
 def version_exists(self,key:rWyGP)->rWyGX:
  pass
 @abc.abstractmethod
 def merge_remote_into_local_version(self,remote_location:rWyGP,key:rWyGP):
  pass
 @abc.abstractmethod
 def _update_key(self,old_key:rWyGP,new_key:rWyGP,base_path:rWyGP)->rWyGX:
  pass
 @abc.abstractmethod
 def _serialize(self,*args:CPVCSObj)->List[rWyGP]:
  pass
 @abc.abstractmethod
 def _deserialize(self,key_serializer:Dict[rWyGP,rWyGP],remote_location:rWyGP=rWyGx)->List[CPVCSObj]:
  pass
class ObjectStorageLocal(ObjectStorageProvider):
 ...
 def _serialize(self,*args:CPVCSObj)->List[rWyGP]:
  ...
  for cpvcs_obj in args:
   if rWyGF(cpvcs_obj,Version):
    hash_refs.append(self.serializers["version"].store_obj(cpvcs_obj))
   elif rWyGF(cpvcs_obj,Revision):
    hash_refs.append(self.serializers["revision"].store_obj(cpvcs_obj))
  return hash_refs
 def _deserialize(self,key_serializer:Dict[rWyGP,rWyGP],remote_location:rWyGP=rWyGx)->List[Union[Revision,Version]]:
