...
CAPTURE_METAMODEL_SEPARATELY=cjLvh
...
class MetamodelDelta(SubtypesInstanceManager):
 def create_delta_for_state_files(self,sf1:StateFileRef,sf2:StateFileRef):
  ...
  if not sf1:
   backend2=load_persisted_object(config_context.get_obj_file_path(sf2.hash_ref))
   clazz=cjLvk(backend2)
   backend1=_infer_backend_init(clazz,sf2)
  else:
   backend1=load_persisted_object(config_context.get_obj_file_path(sf1.hash_ref))
   clazz=cjLvk(backend1)
   backend2=_infer_backend_init(clazz,sf1)
  diff=self.create_delta_json(backend1,backend2)
  return diff
 def create_delta_log(self,state_from:Set[StateFileRef],state_to:Set[StateFileRef])->cjLvq:
  ...
  def _create_sf_lookup(state_files:Set[StateFileRef])->Dict[cjLvq,StateFileRef]:
  ...
  for state_file_from in state_files_from_regular:
   ...
   if state_file_from.any_congruence(state_files_to_regular):
    ...
    diff_json=self.create_delta_for_state_files(state_file_from,cjLvg)
  ...
  for state_files_to in state_files_to_lookup.values():
   ...
   diff_json=self.create_delta_for_state_files(cjLvg,state_files_to)
...
class CommitMetamodelUtils:
 @cjLvW
 def create_metadata_archive(cls,version:Version,delete_reference:bool=cjLvh,overwrite:bool=cjLvh,metamodels_file:cjLvq=cjLvg):
 ...
 def get_metamodel_delta(cls,prev_metamodel:Dict,this_metamodel:Dict)->Dict:
  ...
  return result
 @cjLvW
 def create_delta_metamodel_file(cls,version:cjLvn,revision:cjLvn,store_to_zip=cjLvh)->Tuple[Dict,Dict]:
 ...
 @cjLvW
 def reconstruct_metamodel(cls,version:cjLvn,revision:cjLvn=cjLvg)->Dict:
  result={}
  for rev in cjLvT(1,revision+1):
  ...
  return result
 @cjLvW
 def get_version_metamodel(cls,version:cjLvn,revision:cjLvn=cjLvg)->Dict:
 ...
 @cjLvW
 def get_commit_diff(cls,version_no:cjLvn,commit_no:cjLvn)->Optional[Dict]:
  ...
  return result
def _infer_backend_init(clazz:Type,sf:StateFileRef)->cjLvf:
 if cjLvP(clazz,cjLvd):
  return{}
 constructor=cjLvl(clazz,"__init__",cjLvg)
...
def _create_metamodel_helper(obj,width=25,visited:set=cjLvg):
 if obj is cjLvg:
  return obj
