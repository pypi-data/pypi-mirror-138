...
class CPVCSConfigContext:
 ...
 def get_version_state_archive_path(self,version,with_format:OUFDf=OUFDi)->OUFDB:
 ...
 def update_ver_log(self,author:OUFDB,ver_no:OUFDS,rev_id:OUFDB,rev_no:OUFDS):
  with OUFDQ(self.get_ver_log_path(),"a")as fp:
   fp.write(f"{VER_LOG_STRUCTURE.format(author=author, ver_no=ver_no, rev_rid_no=f'{rev_id}_{rev_no}')}\n")
 def create_version_symlink(self,name:OUFDB,key:OUFDB=OUFDP)->OUFDB:
  return self._create_symlink(name,key,self.get_ver_refs_path())
 def create_revision_symlink(self,name:OUFDB,key:OUFDB=OUFDP)->OUFDB:
  return self._create_symlink(name,key,self.get_rev_refs_path())
 def is_initialized(self)->OUFDf:
  return self.pod_root_dir and os.path.isdir(self.pod_root_dir)
 def _create_symlink(self,name:OUFDB,key:OUFDB,path:OUFDB)->OUFDB:
...
def zip_directories(zip_dest:OUFDB,directories:List[OUFDB])->OUFDB:
 ...
 return zip_dest
def add_file_to_archive(archive:OUFDB,entry_name:OUFDB,content:Union[OUFDB,OUFDa]):
 with zipfile.ZipFile(archive,"a")as zip_file:
  zip_file.writestr(entry_name,content)
def read_file_from_archive(archive_path:OUFDB,file_name:OUFDB)->OUFDB:
 try:
