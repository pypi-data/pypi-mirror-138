  ...
  try:
   if not self._check_if_available():
    try:
     ...
     if wCHjc(e,PackageInstallationException):
      raise
     else:
      suffix=(f" ({self.log_package_name})." if self.log_package_name is not wCHjk else ".")
   ...
   if raise_on_error:
    ...
    if self.logger.isEnabledFor(logging.DEBUG):
     ...
     self.logger.error(e)
 def _check_if_available(self)->wCHjO:
  raise wCHjR
 def _prepare_installation(self)->wCHjk:
  pass
 def _install_package(self)->wCHjk:
  raise wCHjR
 def _post_process(self)->wCHjk:
  pass
 def _verify_installation(self)->wCHjk:
  if not self._check_if_available():
   package_info=(f" of {self.log_package_name}" if self.log_package_name is not wCHjk else "")
...
class MultiPackageInstaller(PackageInstaller):
 def __init__(self,log_package_name:Optional[wCHjs],package_installers:Union[PackageInstaller,List[PackageInstaller]]):
  wCHjp(MultiPackageInstaller,self).__init__(log_package_name=log_package_name)
  self.package_installers=(package_installers if wCHjc(package_installers,List)else[package_installers])
 def install(self,raise_on_error:wCHjO=wCHjG):
...
class OSSpecificPackageInstaller(MultiPackageInstaller):
 def __init__(self,debian_installers:PackageInstaller,redhat_installers:PackageInstaller):
  ...
  wCHjp(OSSpecificPackageInstaller,self).__init__(log_package_name=self.__class__.__name__,package_installers=package_installers)
 def install(self,raise_on_error:wCHjO=wCHjG):
  try:
   if not in_docker():
    ...
    wCHjp(OSSpecificPackageInstaller,self).install(raise_on_error=raise_on_error)
  except PackageInstallationException as e:
   if raise_on_error:
    raise
   else:
...
class DebianPackageInstaller(PackageInstaller,ABC):
 def __init__(self,package_name:wCHjs):
  wCHjp(DebianPackageInstaller,self).__init__(log_package_name=package_name)
 def _install_os_packages(self,packages:Union[wCHjs,List[wCHjs]]):
  packages=packages if wCHjc(packages,List)else[packages]
  with INSTALL_LOCK:
   run(["apt-get","update"])
   run(["apt-get","install","-y","--no-install-recommends"]+packages)
class RedHatPackageInstaller(PackageInstaller,ABC):
 def __init__(self,package_name:wCHjs):
  wCHjp(RedHatPackageInstaller,self).__init__(log_package_name=package_name)
 def _install_os_packages(self,packages:Union[wCHjs,List[wCHjs]]):
  packages=packages if wCHjc(packages,List)else[packages]
  with INSTALL_LOCK:
...
class DebianPostgres11Installer(DebianPackageInstaller):
 def __init__(self):
  wCHjp(DebianPostgres11Installer,self).__init__("postgres11")
 def _check_if_available(self)->wCHjO:
...
class RedHatPostgres11Installer(RedHatPackageInstaller):
 def __init__(self):
  wCHjp(RedHatPostgres11Installer,self).__init__("postgres11")
 def _check_if_available(self)->wCHjO:
  return is_command_available("psql")
 def _prepare_installation(self)->wCHjk:
...
class DebianPlPythonInstaller(DebianPackageInstaller):
 def __init__(self):
  wCHjp(DebianPlPythonInstaller,self).__init__("plpython3")
 def _check_if_available(self)->wCHjO:
...
class RedHatPlPythonInstaller(RedHatPackageInstaller):
 def __init__(self):
  wCHjp(RedHatPlPythonInstaller,self).__init__("plpython3")
 def _check_if_available(self)->wCHjO:
...
class DebianMariaDBInstaller(DebianPackageInstaller):
 def __init__(self):
  wCHjp(DebianMariaDBInstaller,self).__init__("MariaDB")
 def _check_if_available(self)->wCHjO:
...
class RedHatMariaDBInstaller(RedHatPackageInstaller):
 def __init__(self):
  wCHjp(RedHatMariaDBInstaller,self).__init__("MariaDB")
 def _check_if_available(self)->wCHjO:
  return is_command_available("mysqld")
 def _install_package(self)->wCHjk:
...
class DebianTimescaleDBInstaller(DebianPackageInstaller):
 def __init__(self):
  wCHjp(DebianTimescaleDBInstaller,self).__init__("timescaledb")
 def _check_if_available(self)->wCHjO:
...
class RedHatTimescaleDBInstaller(RedHatPackageInstaller):
 def __init__(self):
  wCHjp(RedHatTimescaleDBInstaller,self).__init__("timescaledb")
 def _check_if_available(self)->wCHjO:
...
class DebianRedisInstaller(DebianPackageInstaller):
 def __init__(self):
  wCHjp(DebianRedisInstaller,self).__init__("Redis")
 def _check_if_available(self)->wCHjO:
...
class RedHatRedisInstaller(RedHatPackageInstaller):
 def __init__(self):
  wCHjp(RedHatRedisInstaller,self).__init__("Redis")
 def _check_if_available(self)->wCHjO:
  return is_command_available("redis-server")
 def _install_package(self)->wCHjk:
...
class DebianMosquittoInstaller(DebianPackageInstaller):
 def __init__(self):
  wCHjp(DebianMosquittoInstaller,self).__init__("Mosquitto")
 def _check_if_available(self)->wCHjO:
...
class RedHatMosquittoInstaller(RedHatPackageInstaller):
 def __init__(self):
  wCHjp(RedHatMosquittoInstaller,self).__init__("Mosquitto")
 def _check_if_available(self)->wCHjO:
  return is_command_available("mosquitto")
 def _install_package(self)->wCHjk:
