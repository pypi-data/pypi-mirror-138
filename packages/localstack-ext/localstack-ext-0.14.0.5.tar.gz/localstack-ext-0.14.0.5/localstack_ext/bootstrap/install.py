...
def install_libs():
 ...
 postgres_installer.install(raise_on_error=HCUVc)
 timescaledb_installer.install(raise_on_error=HCUVc)
 redis_installer.install(raise_on_error=HCUVc)
 mosquitto_installer.install(raise_on_error=HCUVc)
...
def setup_ssl_cert():
 ...
 try:
  ...
  try:
   ...
   raise
def download_github_artifact(url:HCUVE,target_file:HCUVE,timeout:int=HCUVG):
 def do_download(url,print_error=HCUVc):
  try:
