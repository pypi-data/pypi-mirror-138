...
SENT_EMAILS=[]
EMAIL_BLACKLIST=rfBwm()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",rfBwb(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=rfBwO,from_name=rfBwO,smtp_host=rfBwO,smtp_user=rfBwO,smtp_pass=rfBwO,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if rfBwL(recipients,rfBwn)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=rfBwO):
 ...
 result["From"]=formataddr((rfBwb(Header(from_name,"utf-8")),from_email))
 return result
