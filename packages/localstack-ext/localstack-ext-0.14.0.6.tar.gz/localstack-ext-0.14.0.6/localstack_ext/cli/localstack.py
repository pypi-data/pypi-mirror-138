...
class ProCliPlugin(LocalstackCliPlugin):
 ...
 def should_load(self):
  ...
  return VBItd if e else VBItl
 def is_active(self):
  return self.should_load()
 def attach(self,cli:LocalstackCli)->VBItT:
...
@click.option("--revert",is_flag=VBItd,help="Revert systemd-resolved settings for the docker interface")
def cmd_dns_systemd(revert:VBItx):
...
def _cpvcs_initialized(pod_name:VBItS)->VBItx:
...
@click.option("-n","--name",help="Name of the cloud pod",required=VBItd)
def cmd_cpvcs_init(name:VBItS):
...
@click.option("-n","--name",help="Name of the cloud pod",required=VBItd)
@click.option("-r","--remote",help="Whether the Pod should also be deleted remotely.",is_flag=VBItd,default=VBItl)
def cmd_cpvcs_delete(name:VBItS,remote:VBItx):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=VBItd)
def cmd_cpvcs_register_remote(name:VBItS):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Current Name of the cloud pod",required=VBItd)
@click.option("-nn","--new-name",help="New name of the cloud pod",required=VBItd)
def cmd_cpvcs_rename(name:VBItS,new_name:VBItS):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-n","--name",help="Name of the cloud pod",required=VBItd)
def cmd_cpvcs_commit(message:VBItS,name:VBItS):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("--squash",is_flag=VBItd,help="Squashes commits together, so only the latest commit is stored in the revision graph")
@click.option("--three-way",is_flag=VBItd,default=VBItl,help="")
@click.option("-m","--message",help="Add a comment describing the version")
@click.option("-n","--name",help="Name of the cloud pod",required=VBItd)
def cmd_cpvcs_push(squash:VBItx,message:VBItS,name:VBItS,three_way:VBItx):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-n","--name",help="Name of the cloud pod",required=VBItd)
@click.option("-v","--version",type=VBItv)
@click.option("-m","--message",required=VBItl)
def cmd_cpvcs_push_overwrite(version:VBItv,message:VBItS,name:VBItS):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-v","--version",default="-1",type=VBItv,help="Loads the state of the specified version - Most recent one by default")
@click.option("--reset",is_flag=VBItd,default=VBItl,help="Will reset the application state before injecting")
@click.option("-n","--name",help="Name of the cloud pod",required=VBItd)
def cmd_cpvcs_inject(version:VBItv,reset:VBItx,name:VBItS):
 from localstack_ext.bootstrap import pods_client
...
@click.option("--inject/--no-inject",default=VBItd,help="Whether the latest version of the pulled pod should be injected")
@click.option("--reset/--no-reset",default=VBItd,help="Whether the current application state should be reset after the pod has been pulled")
@click.option("-n","--name",help="Name of the cloud pod",required=VBItd)
@click.option("--lazy/--eager",default=VBItd,help="Will only fetch references to existing versions, i.e. version state is only downloaded when required")
@cpvcs.command(name="pull",help="Injects the state from a version into the application runtime")
def cmd_cpvcs_pull(name:VBItS,inject:VBItx,reset:VBItx,lazy:VBItx):
 from localstack_ext.bootstrap import pods_client
...
@click.option("--remote","-r",is_flag=VBItd,default=VBItl)
def cmd_cpvcs_list_pods(remote:VBItx):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=VBItd)
def cmd_cpvcs_versions(name:VBItS):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",required=VBItd,type=VBItv)
@click.option("-n","--name",help="Name of the cloud pod",required=VBItd)
def cmd_cpvcs_version_info(version:VBItv,name:VBItS):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",type=VBItv,default=-1,help="Latest version by default")
@click.option("-n","--name",help="Name of the cloud pod",required=VBItd)
def cmd_cpvcs_version_metamodel(version:VBItv,name:VBItS):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",required=VBItd,type=VBItv,help="The version the state should be set to")
@click.option("--inject/--no-inject",default=VBItd,help="Whether the state should be directly injected into the application runtime after changing version")
@click.option("--reset/--no-reset",default=VBItd,help="Whether the current application state should be reset before changing version")
@click.option("--commit-before",is_flag=VBItl,help="Whether the current application state should be commited to the currently selected version before changing version")
@click.option("-n","--name",help="Name of the cloud pod",required=VBItd)
def cmd_cpvcs_set_version(version:VBItv,inject:VBItx,reset:VBItx,commit_before:VBItx,name:VBItS):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=VBItd)
def cmd_cpvcs_commits(version:VBItv,name:VBItS):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("--version","-v",required=VBItd)
@click.option("--commit","-c",required=VBItd)
@click.option("-n","--name",help="Name of the cloud pod",required=VBItd)
def cmd_cpvcs_commit_diff(version:VBItv,commit:VBItv,name:VBItS):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
def cmd_pod_list():
 ...
 from localstack_ext.bootstrap import pods_client
 try:
  result=pods_client.list_pods(VBItT)
  status.stop()
...
def print_table(columns:Dict[VBItS,VBItS],rows:List[Dict[VBItS,Any]],formatters:Dict[VBItS,Callable[[Any],VBItS]]=VBItT):
 from rich.table import Table
 if formatters is VBItT:
  formatters=VBIte()
 t=Table()
...
class ConsoleLogger(logging.Logger):
 def __init__(self,name):
  VBItX(ConsoleLogger,self).__init__(name)
 def info(self,msg:Any,*args:Any,**kwargs:Any)->VBItT:
  console.print(msg%args)
 def warning(self,msg:Any,*args:Any,**kwargs:Any)->VBItT:
  console.print("[red]Warning:[/red] ",msg%args)
 def error(self,msg:Any,*args:Any,**kwargs:Any)->VBItT:
  console.print("[red]Error:[/red] ",msg%args)
 def exception(self,msg:Any,*args:Any,**kwargs:Any)->VBItT:
  console.print("[red]Error:[/red] ",msg%args)
  console.print_exception()
