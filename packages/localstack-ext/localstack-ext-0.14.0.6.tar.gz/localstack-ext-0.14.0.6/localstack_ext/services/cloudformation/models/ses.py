...
class SESTemplate(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return(template or[vJfwb])[0]
 def get_physical_resource_id(self,attribute=vJfwb,**kwargs):
 ...
 def update_resource(self,new_resource,stack_name,resources):
  ...
  if vJfwm(template.get(attr,"")==new_template.get(attr,"")for attr in["SubjectPart","TextPart","HtmlPart"]):
   return
...
class SESReceiptRuleSet(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute):
  return vJfwU(SESReceiptRuleSet,self).get_cfn_attribute(attribute)
 @vJfwB
 def add_defaults(resource,stack_name:vJfwa):
...
class SESReceiptRule(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute):
  return vJfwU(SESReceiptRule,self).get_cfn_attribute(attribute)
 @vJfwR
