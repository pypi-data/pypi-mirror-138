...
class ElastiCacheCluster(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  try:
   result=IjtbK(ElastiCacheCluster,self).get_cfn_attribute(attribute_name)
   assert result is not Ijtbn
   return result
  except IjtbC:
   props=self.props
   ...
   if parts[0]in candidates and IjtbB(parts)>1:
 ...
 def fetch_details(cls,cluster_name):
  ...
  return(match or[Ijtbn])[0]
 @Ijtbx
 def get_deploy_templates():
  return{"create":{"function":"create_cache_cluster","parameters":lambda_convert_types(lambda_rename_attributes({"ClusterName":"CacheClusterId","VpcSecurityGroupIds":"SecurityGroupIds"}),{"NumCacheNodes":IjtbM})},"delete":{"function":"delete_cache_cluster","parameters":{"CacheClusterId":"ClusterName"}}}
