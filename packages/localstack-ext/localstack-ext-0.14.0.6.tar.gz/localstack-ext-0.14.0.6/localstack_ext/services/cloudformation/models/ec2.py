...
class EC2ElasticIP(GenericBaseModel):
 ...
 def cloudformation_type():
  return "AWS::EC2::EIP"
 def get_physical_resource_id(self,attribute=GIlBh,**kwargs):
...
class SecurityGroupInOrEgress(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  for perm in perms:
   if GIlBD(perm["IpProtocol"])!=GIlBD(rp["IpProtocol"]):
    continue
 ...
 @GIlBu
 def get_vpc_and_name_for_security_group(cls,group_id:GIlBD)->Tuple[GIlBD,GIlBD]:
  client=aws_stack.connect_to_service("ec2")
  groups=client.describe_security_groups(GroupIds=[group_id])["SecurityGroups"]
  vpc_id=groups and groups[0].get("VpcId")or GIlBh
  group_name=groups and groups[0]["GroupName"]or GIlBh
 ...
 def get_deploy_templates(cls):
  def create_params(params,**kwargs):
   ...
   source_group_id=result.pop("SourceSecurityGroupId",GIlBh)
   vpc_id=GIlBh
   ...
   dst_group_id=result.pop("DestinationSecurityGroupId",GIlBh)
   ...
   if result.get("IpProtocol"):
    result["IpProtocol"]=GIlBD(result.get("IpProtocol"))
   description=result.pop("Description",GIlBh)
   cidr_ipv6=result.pop("CidrIpv6",GIlBh)
