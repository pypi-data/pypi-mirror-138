...
class CloudFrontOriginAccessIdentity(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return result and result[0]or xljUg
 @xljUD
...
class CloudFrontFunction(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  return xljUi(CloudFrontFunction,self).get_cfn_attribute(attribute_name)
 @xljUn
...
class CloudFrontDistribution(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  origins=origins if xljUI(origins,xljUp)else origins.get("Items")
  origins_to_create=xljUK([orig["DomainName"]for orig in origins])
  aliases=props.get("Aliases",[])
  aliases=aliases if xljUI(aliases,xljUp)else aliases.get("Items")
  aliases_to_create=xljUK(aliases)
  for distr in distrs:
   origins=xljUK([orig.get("DomainName")for orig in distr.get("Origins",{})["Items"]])
   aliases=xljUK(distr.get("Aliases",{}).get("Items",[]))
 ...
 def get_deploy_templates():
  def lambda_get_distribution_config(params,**kwargs):
   ...
   config["IsIPV6Enabled"]=config.pop("IPV6Enabled",xljUN)
   config["Enabled"]=xljUw(config.get("Enabled")).lower()=="true"
   cert=config["ViewerCertificate"]=config.get("ViewerCertificate",{})
   custom_errors=config.get("CustomErrorResponses",[])
   custom_errors=(custom_errors.get("Items",[])if xljUI(custom_errors,xljUL)else custom_errors)
   for item in custom_errors:
    item["ResponseCode"]=xljUw(item.get("ResponseCode"))
   cache=config["DefaultCacheBehavior"]
   cache["TrustedSigners"]=cache.get("TrustedSigners")or{"Quantity":0,"Enabled":xljUN}
   cache["MinTTL"]=cache.get("MinTTL")or 600
   forwarded=cache["ForwardedValues"]=cache.get("ForwardedValues",{})
   forwarded["QueryString"]=xljUw(forwarded.get("QueryString")).lower()=="true"
   ...
   def convert_list(key,parent=xljUg):
    parent=config if parent is xljUg else parent
    items=parent.get(key)or[]
    if xljUI(items,xljUL)and "Items" in items and "Quantity" in items:
     return
    parent[key]={"Quantity":xljUr(items),"Items":items}
   cache_behavs=config.get("CacheBehaviors",[])
   cache_behavs=(cache_behavs.get("Items",[])if xljUI(cache_behavs,xljUL)else cache_behavs)
