...
class StateFileLocator(abc.ABC):
 location="_none_"
 active_instance=CetGD
 @abc.abstractmethod
 def get_state_file_location_by_key(self,key:CetGc)->CetGc:
  pass
 @CetGs
 def get(cls,requested_file_locator:CetGc):
...
class ObjectStorageProvider(abc.ABC):
 ...
 active_instance=CetGD
 @CetGs
 def get(cls,state_file_locator:CetGc,requested_storage:CetGc,serializers:Dict[CetGc,CPVCSSerializer]):
  ...
  return cls.active_instance
 def __init__(self,state_file_locator:StateFileLocator,serializers:Dict[CetGc,CPVCSSerializer]):
 ...
 def get_state_file_location_by_key(self,key:CetGc)->CetGc:
  pass
 @abc.abstractmethod
 def get_version_by_key(self,key:CetGc)->Version:
  pass
 @abc.abstractmethod
 def get_revision_by_key(self,key:CetGc)->Revision:
  pass
 @abc.abstractmethod
 def get_revision_or_version_by_key(self,key:CetGc)->Union[Revision,Version]:
  pass
 @abc.abstractmethod
 def get_delta_file_by_key(self,key:CetGc)->CetGc:
  pass
 @abc.abstractmethod
 def upsert_objects(self,*args:CPVCSObj)->List[CetGc]:
  pass
 @abc.abstractmethod
 def update_revision_key(self,old_key:CetGc,new_key:CetGc,referenced_by_version:CetGc=CetGD):
  pass
 @abc.abstractmethod
 def version_exists(self,key:CetGc)->CetGN:
  pass
 @abc.abstractmethod
 def merge_remote_into_local_version(self,remote_location:CetGc,key:CetGc):
  pass
 @abc.abstractmethod
 def _update_key(self,old_key:CetGc,new_key:CetGc,base_path:CetGc)->CetGN:
  pass
 @abc.abstractmethod
 def _serialize(self,*args:CPVCSObj)->List[CetGc]:
  pass
 @abc.abstractmethod
 def _deserialize(self,key_serializer:Dict[CetGc,CetGc],remote_location:CetGc=CetGD)->List[CPVCSObj]:
  pass
class ObjectStorageLocal(ObjectStorageProvider):
 ...
 def _serialize(self,*args:CPVCSObj)->List[CetGc]:
  ...
  for cpvcs_obj in args:
   if CetGB(cpvcs_obj,Version):
    hash_refs.append(self.serializers["version"].store_obj(cpvcs_obj))
   elif CetGB(cpvcs_obj,Revision):
    hash_refs.append(self.serializers["revision"].store_obj(cpvcs_obj))
  return hash_refs
 def _deserialize(self,key_serializer:Dict[CetGc,CetGc],remote_location:CetGc=CetGD)->List[Union[Revision,Version]]:
