...
CAPTURE_METAMODEL_SEPARATELY=exbDu
...
class MetamodelDelta(SubtypesInstanceManager):
 def create_delta_for_state_files(self,sf1:StateFileRef,sf2:StateFileRef):
  ...
  if not sf1:
   backend2=load_persisted_object(config_context.get_obj_file_path(sf2.hash_ref))
   clazz=exbDS(backend2)
   backend1=_infer_backend_init(clazz,sf2)
  else:
   backend1=load_persisted_object(config_context.get_obj_file_path(sf1.hash_ref))
   clazz=exbDS(backend1)
   backend2=_infer_backend_init(clazz,sf1)
  diff=self.create_delta_json(backend1,backend2)
  return diff
 def create_delta_log(self,state_from:Set[StateFileRef],state_to:Set[StateFileRef])->exbDv:
  ...
  def _create_sf_lookup(state_files:Set[StateFileRef])->Dict[exbDv,StateFileRef]:
  ...
  for state_file_from in state_files_from_regular:
   ...
   if state_file_from.any_congruence(state_files_to_regular):
    ...
    diff_json=self.create_delta_for_state_files(state_file_from,exbDP)
  ...
  for state_files_to in state_files_to_lookup.values():
   ...
   diff_json=self.create_delta_for_state_files(exbDP,state_files_to)
...
class CommitMetamodelUtils:
 @exbDm
 def create_metadata_archive(cls,version:Version,delete_reference:bool=exbDu,overwrite:bool=exbDu,metamodels_file:exbDv=exbDP):
 ...
 def get_metamodel_delta(cls,prev_metamodel:Dict,this_metamodel:Dict)->Dict:
  ...
  return result
 @exbDm
 def create_delta_metamodel_file(cls,version:exbDO,revision:exbDO,store_to_zip=exbDu)->Tuple[Dict,Dict]:
 ...
 @exbDm
 def reconstruct_metamodel(cls,version:exbDO,revision:exbDO=exbDP)->Dict:
  if not revision:
   revision=infer_max_commit_diff(archive_path=PodFilePaths.get_version_meta_archive(version))
  result={}
  for rev in exbDF(1,revision+1):
  ...
  return result
 @exbDm
 def get_version_metamodel(cls,version:exbDO,revision:exbDO=exbDP)->Dict:
 ...
 @exbDm
 def get_commit_diff(cls,version_no:exbDO,commit_no:exbDO)->Optional[Dict]:
  ...
  return result
def _infer_backend_init(clazz:Type,sf:StateFileRef)->exbDM:
 if exbDl(clazz,exbDG):
  return{}
 constructor=exbDn(clazz,"__init__",exbDP)
...
def _create_metamodel_helper(obj,width=25,visited:set=exbDP):
 if obj is exbDP:
  return obj
 ...
 result=obj=obj_dict if obj_dict is not exbDP else obj
 if exbDl(obj,exbDG):
  result=exbDG(result)
  for field_name,field_value in result.items():
   result[field_name]=_create_metamodel_helper(field_value,width=width,visited=visited)
 elif exbDl(obj,exbDt):
  result=[_create_metamodel_helper(o,width=width,visited=visited)for o in obj]
  if exbDA(result)>width:
   result={"size":exbDA(result),"items":result[:width]}
 return result
def infer_max_commit_diff(archive_path:exbDv)->exbDO:
 max_commit=1
 with zipfile.ZipFile(archive_path)as archive:
