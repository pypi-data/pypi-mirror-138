...
class CPVCSConfigContext:
 ...
 def get_version_state_archive_path(self,version,with_format:ltHPf=ltHPi)->ltHPw:
 ...
 def update_ver_log(self,author:ltHPw,ver_no:ltHPJ,rev_id:ltHPw,rev_no:ltHPJ):
  with ltHPK(self.get_ver_log_path(),"a")as fp:
   fp.write(f"{VER_LOG_STRUCTURE.format(author=author, ver_no=ver_no, rev_rid_no=f'{rev_id}_{rev_no}')}\n")
 def create_version_symlink(self,name:ltHPw,key:ltHPw=ltHPA)->ltHPw:
  return self._create_symlink(name,key,self.get_ver_refs_path())
 def create_revision_symlink(self,name:ltHPw,key:ltHPw=ltHPA)->ltHPw:
  return self._create_symlink(name,key,self.get_rev_refs_path())
 def is_initialized(self)->ltHPf:
  return self.pod_root_dir and os.path.isdir(self.pod_root_dir)
 def _create_symlink(self,name:ltHPw,key:ltHPw,path:ltHPw)->ltHPw:
...
def zip_directories(zip_dest:ltHPw,directories:List[ltHPw])->ltHPw:
 ...
 return zip_dest
def add_file_to_archive(archive:ltHPw,entry_name:ltHPw,content:Union[ltHPw,ltHPm]):
 with zipfile.ZipFile(archive,"a")as zip_file:
  zip_file.writestr(entry_name,content)
def read_file_from_archive(archive_path:ltHPw,file_name:ltHPw)->ltHPw:
 try:
