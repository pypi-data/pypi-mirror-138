  ...
  try:
   if not self._check_if_available():
    try:
     ...
     if mtSBn(e,PackageInstallationException):
      raise
     else:
      suffix=(f" ({self.log_package_name})." if self.log_package_name is not mtSBX else ".")
   ...
   if raise_on_error:
    ...
    if self.logger.isEnabledFor(logging.DEBUG):
     ...
     self.logger.error(e)
 def _check_if_available(self)->mtSBw:
  raise mtSBb
 def _prepare_installation(self)->mtSBX:
  pass
 def _install_package(self)->mtSBX:
  raise mtSBb
 def _post_process(self)->mtSBX:
  pass
 def _verify_installation(self)->mtSBX:
  if not self._check_if_available():
   package_info=(f" of {self.log_package_name}" if self.log_package_name is not mtSBX else "")
...
class MultiPackageInstaller(PackageInstaller):
 def __init__(self,log_package_name:Optional[mtSBf],package_installers:Union[PackageInstaller,List[PackageInstaller]]):
  mtSBq(MultiPackageInstaller,self).__init__(log_package_name=log_package_name)
  self.package_installers=(package_installers if mtSBn(package_installers,List)else[package_installers])
 def install(self,raise_on_error:mtSBw=mtSBj):
...
class OSSpecificPackageInstaller(MultiPackageInstaller):
 def __init__(self,debian_installers:PackageInstaller,redhat_installers:PackageInstaller):
  ...
  mtSBq(OSSpecificPackageInstaller,self).__init__(log_package_name=self.__class__.__name__,package_installers=package_installers)
 def install(self,raise_on_error:mtSBw=mtSBj):
  try:
   if not in_docker():
    ...
    mtSBq(OSSpecificPackageInstaller,self).install(raise_on_error=raise_on_error)
  except PackageInstallationException as e:
   if raise_on_error:
    raise
   else:
...
class DebianPackageInstaller(PackageInstaller,ABC):
 def __init__(self,package_name:mtSBf):
  mtSBq(DebianPackageInstaller,self).__init__(log_package_name=package_name)
 def _install_os_packages(self,packages:Union[mtSBf,List[mtSBf]]):
  packages=packages if mtSBn(packages,List)else[packages]
  with INSTALL_LOCK:
   run(["apt-get","update"])
   run(["apt-get","install","-y","--no-install-recommends"]+packages)
class RedHatPackageInstaller(PackageInstaller,ABC):
 def __init__(self,package_name:mtSBf):
  mtSBq(RedHatPackageInstaller,self).__init__(log_package_name=package_name)
 def _install_os_packages(self,packages:Union[mtSBf,List[mtSBf]]):
  packages=packages if mtSBn(packages,List)else[packages]
  with INSTALL_LOCK:
...
class DebianPostgres11Installer(DebianPackageInstaller):
 def __init__(self):
  mtSBq(DebianPostgres11Installer,self).__init__("postgres11")
 def _check_if_available(self)->mtSBw:
...
class RedHatPostgres11Installer(RedHatPackageInstaller):
 def __init__(self):
  mtSBq(RedHatPostgres11Installer,self).__init__("postgres11")
 def _check_if_available(self)->mtSBw:
  return is_command_available("psql")
 def _prepare_installation(self)->mtSBX:
...
class DebianPlPythonInstaller(DebianPackageInstaller):
 def __init__(self):
  mtSBq(DebianPlPythonInstaller,self).__init__("plpython3")
 def _check_if_available(self)->mtSBw:
...
class RedHatPlPythonInstaller(RedHatPackageInstaller):
 def __init__(self):
  mtSBq(RedHatPlPythonInstaller,self).__init__("plpython3")
 def _check_if_available(self)->mtSBw:
...
class DebianMariaDBInstaller(DebianPackageInstaller):
 def __init__(self):
  mtSBq(DebianMariaDBInstaller,self).__init__("MariaDB")
 def _check_if_available(self)->mtSBw:
...
class RedHatMariaDBInstaller(RedHatPackageInstaller):
 def __init__(self):
  mtSBq(RedHatMariaDBInstaller,self).__init__("MariaDB")
 def _check_if_available(self)->mtSBw:
  return is_command_available("mysqld")
 def _install_package(self)->mtSBX:
...
class DebianTimescaleDBInstaller(DebianPackageInstaller):
 def __init__(self):
  mtSBq(DebianTimescaleDBInstaller,self).__init__("timescaledb")
 def _check_if_available(self)->mtSBw:
...
class RedHatTimescaleDBInstaller(RedHatPackageInstaller):
 def __init__(self):
  mtSBq(RedHatTimescaleDBInstaller,self).__init__("timescaledb")
 def _check_if_available(self)->mtSBw:
...
class DebianRedisInstaller(DebianPackageInstaller):
 def __init__(self):
  mtSBq(DebianRedisInstaller,self).__init__("Redis")
 def _check_if_available(self)->mtSBw:
...
class RedHatRedisInstaller(RedHatPackageInstaller):
 def __init__(self):
  mtSBq(RedHatRedisInstaller,self).__init__("Redis")
 def _check_if_available(self)->mtSBw:
  return is_command_available("redis-server")
 def _install_package(self)->mtSBX:
...
class DebianMosquittoInstaller(DebianPackageInstaller):
 def __init__(self):
  mtSBq(DebianMosquittoInstaller,self).__init__("Mosquitto")
 def _check_if_available(self)->mtSBw:
...
class RedHatMosquittoInstaller(RedHatPackageInstaller):
 def __init__(self):
  mtSBq(RedHatMosquittoInstaller,self).__init__("Mosquitto")
 def _check_if_available(self)->mtSBw:
  return is_command_available("mosquitto")
 def _install_package(self)->mtSBX:
