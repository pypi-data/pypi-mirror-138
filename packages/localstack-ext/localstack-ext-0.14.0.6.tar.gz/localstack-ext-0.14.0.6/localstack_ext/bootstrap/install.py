...
def install_libs():
 ...
 postgres_installer.install(raise_on_error=kDmit)
 timescaledb_installer.install(raise_on_error=kDmit)
 redis_installer.install(raise_on_error=kDmit)
 mosquitto_installer.install(raise_on_error=kDmit)
...
def setup_ssl_cert():
 ...
 try:
  ...
  try:
   ...
   raise
def download_github_artifact(url:kDmiL,target_file:kDmiL,timeout:int=kDmib):
 def do_download(url,print_error=kDmit):
  try:
