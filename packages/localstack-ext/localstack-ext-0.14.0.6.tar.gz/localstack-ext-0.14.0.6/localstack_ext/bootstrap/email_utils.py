...
SENT_EMAILS=[]
EMAIL_BLACKLIST=YqxyM()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",YqxyK(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=Yqxyv,from_name=Yqxyv,smtp_host=Yqxyv,smtp_user=Yqxyv,smtp_pass=Yqxyv,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if Yqxyh(recipients,YqxyJ)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=Yqxyv):
 ...
 result["From"]=formataddr((YqxyK(Header(from_name,"utf-8")),from_email))
 return result
