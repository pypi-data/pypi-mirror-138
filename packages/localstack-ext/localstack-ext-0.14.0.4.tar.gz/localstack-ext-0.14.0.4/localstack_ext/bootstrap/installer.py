  ...
  try:
   if not self._check_if_available():
    try:
     ...
     if NgIUr(e,PackageInstallationException):
      raise
     else:
      suffix=(f" ({self.log_package_name})." if self.log_package_name is not NgIUx else ".")
   ...
   if raise_on_error:
    ...
    if self.logger.isEnabledFor(logging.DEBUG):
     ...
     self.logger.error(e)
 def _check_if_available(self)->NgIUQ:
  raise NgIUl
 def _prepare_installation(self)->NgIUx:
  pass
 def _install_package(self)->NgIUx:
  raise NgIUl
 def _post_process(self)->NgIUx:
  pass
 def _verify_installation(self)->NgIUx:
  if not self._check_if_available():
   package_info=(f" of {self.log_package_name}" if self.log_package_name is not NgIUx else "")
...
class MultiPackageInstaller(PackageInstaller):
 def __init__(self,log_package_name:Optional[NgIUf],package_installers:Union[PackageInstaller,List[PackageInstaller]]):
  NgIUG(MultiPackageInstaller,self).__init__(log_package_name=log_package_name)
  self.package_installers=(package_installers if NgIUr(package_installers,List)else[package_installers])
 def install(self,raise_on_error:NgIUQ=NgIUt):
...
class OSSpecificPackageInstaller(MultiPackageInstaller):
 def __init__(self,debian_installers:PackageInstaller,redhat_installers:PackageInstaller):
  ...
  NgIUG(OSSpecificPackageInstaller,self).__init__(log_package_name=NgIUx,package_installers=package_installers)
 def install(self,raise_on_error:NgIUQ=NgIUt):
  if not in_docker():
   ...
   NgIUG(OSSpecificPackageInstaller,self).install(raise_on_error=raise_on_error)
class DebianPackageInstaller(PackageInstaller,ABC):
 def __init__(self,package_name:NgIUf):
  NgIUG(DebianPackageInstaller,self).__init__(log_package_name=package_name)
 def _install_os_packages(self,packages:Union[NgIUf,List[NgIUf]]):
  packages=packages if NgIUr(packages,List)else[packages]
  with INSTALL_LOCK:
   run(["apt-get","update"])
   run(["apt-get","install","-y","--no-install-recommends"]+packages)
class RedHatPackageInstaller(PackageInstaller,ABC):
 def __init__(self,package_name:NgIUf):
  NgIUG(RedHatPackageInstaller,self).__init__(log_package_name=package_name)
 def _install_os_packages(self,packages:Union[NgIUf,List[NgIUf]]):
  packages=packages if NgIUr(packages,List)else[packages]
  with INSTALL_LOCK:
...
class DebianPostgres11Installer(DebianPackageInstaller):
 def __init__(self):
  NgIUG(DebianPostgres11Installer,self).__init__("postgres11")
 def _check_if_available(self)->NgIUQ:
...
class RedHatPostgres11Installer(RedHatPackageInstaller):
 def __init__(self):
  NgIUG(RedHatPostgres11Installer,self).__init__("postgres11")
 def _check_if_available(self)->NgIUQ:
  return is_command_available("psql")
 def _prepare_installation(self)->NgIUx:
...
class DebianPlPythonInstaller(DebianPackageInstaller):
 def __init__(self):
  NgIUG(DebianPlPythonInstaller,self).__init__("plpython3")
 def _check_if_available(self)->NgIUQ:
...
class RedHatPlPythonInstaller(RedHatPackageInstaller):
 def __init__(self):
  NgIUG(RedHatPlPythonInstaller,self).__init__("plpython3")
 def _check_if_available(self)->NgIUQ:
...
class DebianMariaDBInstaller(DebianPackageInstaller):
 def __init__(self):
  NgIUG(DebianMariaDBInstaller,self).__init__("MariaDB")
 def _check_if_available(self)->NgIUQ:
...
class RedHatMariaDBInstaller(RedHatPackageInstaller):
 def __init__(self):
  NgIUG(RedHatMariaDBInstaller,self).__init__("MariaDB")
 def _check_if_available(self)->NgIUQ:
  return is_command_available("mysqld")
 def _install_package(self)->NgIUx:
...
class DebianTimescaleDBInstaller(DebianPackageInstaller):
 def __init__(self):
  NgIUG(DebianTimescaleDBInstaller,self).__init__("timescaledb")
 def _check_if_available(self)->NgIUQ:
...
class RedHatTimescaleDBInstaller(RedHatPackageInstaller):
 def __init__(self):
  NgIUG(RedHatTimescaleDBInstaller,self).__init__("timescaledb")
 def _check_if_available(self)->NgIUQ:
...
class DebianRedisInstaller(DebianPackageInstaller):
 def __init__(self):
  NgIUG(DebianRedisInstaller,self).__init__("Redis")
 def _check_if_available(self)->NgIUQ:
...
class RedHatRedisInstaller(RedHatPackageInstaller):
 def __init__(self):
  NgIUG(RedHatRedisInstaller,self).__init__("Redis")
 def _check_if_available(self)->NgIUQ:
  return is_command_available("redis-server")
 def _install_package(self)->NgIUx:
...
class DebianMosquittoInstaller(DebianPackageInstaller):
 def __init__(self):
  NgIUG(DebianMosquittoInstaller,self).__init__("Mosquitto")
 def _check_if_available(self)->NgIUQ:
...
class RedHatMosquittoInstaller(RedHatPackageInstaller):
 def __init__(self):
  NgIUG(RedHatMosquittoInstaller,self).__init__("Mosquitto")
 def _check_if_available(self)->NgIUQ:
  return is_command_available("mosquitto")
 def _install_package(self)->NgIUx:
