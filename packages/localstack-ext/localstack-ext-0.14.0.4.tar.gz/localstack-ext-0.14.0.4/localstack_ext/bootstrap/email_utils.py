...
SENT_EMAILS=[]
EMAIL_BLACKLIST=Msghy()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",MsghE(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=Msgho,from_name=Msgho,smtp_host=Msgho,smtp_user=Msgho,smtp_pass=Msgho,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if MsghG(recipients,MsghP)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=Msgho):
 ...
 result["From"]=formataddr((MsghE(Header(from_name,"utf-8")),from_email))
 return result
