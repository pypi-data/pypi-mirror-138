...
class StateFileLocator(abc.ABC):
 location="_none_"
 active_instance=uzglh
 @abc.abstractmethod
 def get_state_file_location_by_key(self,key:uzglO)->uzglO:
  pass
 @uzglX
 def get(cls,requested_file_locator:uzglO):
...
class ObjectStorageProvider(abc.ABC):
 ...
 active_instance=uzglh
 @uzglX
 def get(cls,state_file_locator:uzglO,requested_storage:uzglO,serializers:Dict[uzglO,CPVCSSerializer]):
  ...
  return cls.active_instance
 def __init__(self,state_file_locator:StateFileLocator,serializers:Dict[uzglO,CPVCSSerializer]):
 ...
 def get_state_file_location_by_key(self,key:uzglO)->uzglO:
  pass
 @abc.abstractmethod
 def get_version_by_key(self,key:uzglO)->Version:
  pass
 @abc.abstractmethod
 def get_revision_by_key(self,key:uzglO)->Revision:
  pass
 @abc.abstractmethod
 def get_revision_or_version_by_key(self,key:uzglO)->Union[Revision,Version]:
  pass
 @abc.abstractmethod
 def get_delta_file_by_key(self,key:uzglO)->uzglO:
  pass
 @abc.abstractmethod
 def upsert_objects(self,*args:CPVCSObj)->List[uzglO]:
  pass
 @abc.abstractmethod
 def update_revision_key(self,old_key:uzglO,new_key:uzglO,referenced_by_version:uzglO=uzglh):
  pass
 @abc.abstractmethod
 def version_exists(self,key:uzglO)->uzglD:
  pass
 @abc.abstractmethod
 def merge_remote_into_local_version(self,remote_location:uzglO,key:uzglO):
  pass
 @abc.abstractmethod
 def _update_key(self,old_key:uzglO,new_key:uzglO,base_path:uzglO)->uzglD:
  pass
 @abc.abstractmethod
 def _serialize(self,*args:CPVCSObj)->List[uzglO]:
  pass
 @abc.abstractmethod
 def _deserialize(self,key_serializer:Dict[uzglO,uzglO],remote_location:uzglO=uzglh)->List[CPVCSObj]:
  pass
class ObjectStorageLocal(ObjectStorageProvider):
 ...
 def _serialize(self,*args:CPVCSObj)->List[uzglO]:
  ...
  for cpvcs_obj in args:
   if uzglf(cpvcs_obj,Version):
    hash_refs.append(self.serializers["version"].store_obj(cpvcs_obj))
   elif uzglf(cpvcs_obj,Revision):
    hash_refs.append(self.serializers["revision"].store_obj(cpvcs_obj))
  return hash_refs
 def _deserialize(self,key_serializer:Dict[uzglO,uzglO],remote_location:uzglO=uzglh)->List[Union[Revision,Version]]:
