...
CAPTURE_METAMODEL_SEPARATELY=twyWe
...
class MetamodelDelta(SubtypesInstanceManager):
 def create_delta_for_state_files(self,sf1:StateFileRef,sf2:StateFileRef):
  ...
  if not sf1:
   backend2=load_persisted_object(config_context.get_obj_file_path(sf2.hash_ref))
   clazz=twyWc(backend2)
   backend1=_infer_backend_init(clazz,sf2)
  else:
   backend1=load_persisted_object(config_context.get_obj_file_path(sf1.hash_ref))
   clazz=twyWc(backend1)
   backend2=_infer_backend_init(clazz,sf1)
  diff=self.create_delta_json(backend1,backend2)
  return diff
 def create_delta_log(self,state_from:Set[StateFileRef],state_to:Set[StateFileRef])->twyWg:
  ...
  def _create_sf_lookup(state_files:Set[StateFileRef])->Dict[twyWg,StateFileRef]:
  ...
  for state_file_from in state_files_from_regular:
   ...
   if state_file_from.any_congruence(state_files_to_regular):
    ...
    diff_json=self.create_delta_for_state_files(state_file_from,twyWJ)
  ...
  for state_files_to in state_files_to_lookup.values():
   ...
   diff_json=self.create_delta_for_state_files(twyWJ,state_files_to)
...
class CommitMetamodelUtils:
 @twyWj
 def create_metadata_archive(cls,version:Version,delete_reference:bool=twyWe,overwrite:bool=twyWe,metamodels_file:twyWg=twyWJ):
 ...
 def get_metamodel_delta(cls,prev_metamodel:Dict,this_metamodel:Dict)->Dict:
  ...
  return result
 @twyWj
 def create_delta_metamodel_file(cls,version:twyWm,revision:twyWm,store_to_zip=twyWe)->Tuple[Dict,Dict]:
 ...
 @twyWj
 def reconstruct_metamodel(cls,version:twyWm,revision:twyWm=twyWJ)->Dict:
  result={}
  for rev in twyWq(1,revision+1):
  ...
  return result
 @twyWj
 def get_version_metamodel(cls,version:twyWm,revision:twyWm=twyWJ)->Dict:
 ...
 @twyWj
 def get_commit_diff(cls,version_no:twyWm,commit_no:twyWm)->Optional[Dict]:
  ...
  return result
def _infer_backend_init(clazz:Type,sf:StateFileRef)->twyWM:
 if twyWV(clazz,twyWX):
  return{}
 constructor=twyWN(clazz,"__init__",twyWJ)
...
def _create_metamodel_helper(obj,width=25,visited:set=twyWJ):
 if obj is twyWJ:
  return obj
