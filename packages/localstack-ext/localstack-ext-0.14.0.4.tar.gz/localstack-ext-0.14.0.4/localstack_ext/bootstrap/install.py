...
def install_libs():
 ...
 postgres_installer.install(raise_on_error=Hjuio)
 timescaledb_installer.install(raise_on_error=Hjuio)
 redis_installer.install(raise_on_error=Hjuio)
 mosquitto_installer.install(raise_on_error=Hjuio)
...
def setup_ssl_cert():
 ...
 try:
  ...
  try:
   ...
   raise
def download_github_artifact(url:Hjuix,target_file:Hjuix,timeout:int=Hjuit):
 def do_download(url,print_error=Hjuio):
  try:
