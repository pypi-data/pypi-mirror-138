...
class SESTemplate(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return(template or[ncRQi])[0]
 def get_physical_resource_id(self,attribute=ncRQi,**kwargs):
 ...
 def update_resource(self,new_resource,stack_name,resources):
  ...
  if ncRQM(template.get(attr,"")==new_template.get(attr,"")for attr in["SubjectPart","TextPart","HtmlPart"]):
   return
...
class SESReceiptRuleSet(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute):
  return ncRQm(SESReceiptRuleSet,self).get_cfn_attribute(attribute)
 @ncRQu
 def add_defaults(resource,stack_name:ncRQs):
...
class SESReceiptRule(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute):
  return ncRQm(SESReceiptRule,self).get_cfn_attribute(attribute)
 @ncRQv
