...
class EC2ElasticIP(GenericBaseModel):
 ...
 def cloudformation_type():
  return "AWS::EC2::EIP"
 def get_physical_resource_id(self,attribute=pBmkq,**kwargs):
...
class SecurityGroupInOrEgress(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  for perm in perms:
   if pBmkv(perm["IpProtocol"])!=pBmkv(rp["IpProtocol"]):
    continue
 ...
 @pBmkS
 def get_vpc_and_name_for_security_group(cls,group_id:pBmkv)->Tuple[pBmkv,pBmkv]:
  client=aws_stack.connect_to_service("ec2")
  groups=client.describe_security_groups(GroupIds=[group_id])["SecurityGroups"]
  vpc_id=groups and groups[0].get("VpcId")or pBmkq
  group_name=groups and groups[0]["GroupName"]or pBmkq
 ...
 def get_deploy_templates(cls):
  def create_params(params,**kwargs):
   ...
   source_group_id=result.pop("SourceSecurityGroupId",pBmkq)
   vpc_id=pBmkq
   ...
   dst_group_id=result.pop("DestinationSecurityGroupId",pBmkq)
   ...
   if result.get("IpProtocol"):
    result["IpProtocol"]=pBmkv(result.get("IpProtocol"))
   description=result.pop("Description",pBmkq)
   cidr_ipv6=result.pop("CidrIpv6",pBmkq)
