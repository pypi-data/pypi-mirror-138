...
class CloudFrontOriginAccessIdentity(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return result and result[0]or RzhXG
 @RzhXr
...
class CloudFrontFunction(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  return RzhXJ(CloudFrontFunction,self).get_cfn_attribute(attribute_name)
 @RzhXM
...
class CloudFrontDistribution(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  origins=origins if RzhXP(origins,RzhXT)else origins.get("Items")
  origins_to_create=RzhXv([orig["DomainName"]for orig in origins])
  aliases=props.get("Aliases",[])
  aliases=aliases if RzhXP(aliases,RzhXT)else aliases.get("Items")
  aliases_to_create=RzhXv(aliases)
  for distr in distrs:
   origins=RzhXv([orig.get("DomainName")for orig in distr.get("Origins",{})["Items"]])
   aliases=RzhXv(distr.get("Aliases",{}).get("Items",[]))
 ...
 def get_deploy_templates():
  def lambda_get_distribution_config(params,**kwargs):
   ...
   config["IsIPV6Enabled"]=config.pop("IPV6Enabled",RzhXu)
   config["Enabled"]=RzhXc(config.get("Enabled")).lower()=="true"
   cert=config["ViewerCertificate"]=config.get("ViewerCertificate",{})
   custom_errors=config.get("CustomErrorResponses",[])
   custom_errors=(custom_errors.get("Items",[])if RzhXP(custom_errors,RzhXL)else custom_errors)
   for item in custom_errors:
    item["ResponseCode"]=RzhXc(item.get("ResponseCode"))
   cache=config["DefaultCacheBehavior"]
   cache["TrustedSigners"]=cache.get("TrustedSigners")or{"Quantity":0,"Enabled":RzhXu}
   cache["MinTTL"]=cache.get("MinTTL")or 600
   forwarded=cache["ForwardedValues"]=cache.get("ForwardedValues",{})
   forwarded["QueryString"]=RzhXc(forwarded.get("QueryString")).lower()=="true"
   ...
   def convert_list(key,parent=RzhXG):
    parent=config if parent is RzhXG else parent
    items=parent.get(key)or[]
    if RzhXP(items,RzhXL)and "Items" in items and "Quantity" in items:
     return
    parent[key]={"Quantity":RzhXN(items),"Items":items}
   cache_behavs=config.get("CacheBehaviors",[])
   cache_behavs=(cache_behavs.get("Items",[])if RzhXP(cache_behavs,RzhXL)else cache_behavs)
