...
class ProCliPlugin(LocalstackCliPlugin):
 ...
 def should_load(self):
  ...
  return fTmXo if e else fTmXn
 def is_active(self):
  return self.should_load()
 def attach(self,cli:LocalstackCli)->fTmXJ:
...
@click.option("--revert",is_flag=fTmXo,help="Revert systemd-resolved settings for the docker interface")
def cmd_dns_systemd(revert:fTmXt):
...
def _cpvcs_initialized(pod_name:fTmXr)->fTmXt:
...
@click.option("-n","--name",help="Name of the cloud pod",required=fTmXo)
def cmd_cpvcs_init(name:fTmXr):
...
@click.option("-n","--name",help="Name of the cloud pod",required=fTmXo)
@click.option("-r","--remote",help="Whether the Pod should also be deleted remotely.",is_flag=fTmXo,default=fTmXn)
def cmd_cpvcs_delete(name:fTmXr,remote:fTmXt):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=fTmXo)
def cmd_cpvcs_register_remote(name:fTmXr):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Current Name of the cloud pod",required=fTmXo)
@click.option("-nn","--new-name",help="New name of the cloud pod",required=fTmXo)
def cmd_cpvcs_rename(name:fTmXr,new_name:fTmXr):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-n","--name",help="Name of the cloud pod",required=fTmXo)
def cmd_cpvcs_commit(message:fTmXr,name:fTmXr):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("--squash",is_flag=fTmXo,help="Squashes commits together, so only the latest commit is stored in the revision graph")
@click.option("--three-way",is_flag=fTmXo,default=fTmXn,help="")
@click.option("-m","--message",help="Add a comment describing the version")
@click.option("-n","--name",help="Name of the cloud pod",required=fTmXo)
def cmd_cpvcs_push(squash:fTmXt,message:fTmXr,name:fTmXr,three_way:fTmXt):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-n","--name",help="Name of the cloud pod",required=fTmXo)
@click.option("-v","--version",type=fTmXM)
@click.option("-m","--message",required=fTmXn)
def cmd_cpvcs_push_overwrite(version:fTmXM,message:fTmXr,name:fTmXr):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-v","--version",default="-1",type=fTmXM,help="Loads the state of the specified version - Most recent one by default")
@click.option("--reset",is_flag=fTmXo,default=fTmXn,help="Will reset the application state before injecting")
@click.option("-n","--name",help="Name of the cloud pod",required=fTmXo)
def cmd_cpvcs_inject(version:fTmXM,reset:fTmXt,name:fTmXr):
 from localstack_ext.bootstrap import pods_client
...
@click.option("--inject/--no-inject",default=fTmXo,help="Whether the latest version of the pulled pod should be injected")
@click.option("--reset/--no-reset",default=fTmXo,help="Whether the current application state should be reset after the pod has been pulled")
@click.option("-n","--name",help="Name of the cloud pod",required=fTmXo)
@click.option("--lazy/--eager",default=fTmXo,help="Will only fetch references to existing versions, i.e. version state is only downloaded when required")
@cpvcs.command(name="pull",help="Injects the state from a version into the application runtime")
def cmd_cpvcs_pull(name:fTmXr,inject:fTmXt,reset:fTmXt,lazy:fTmXt):
 from localstack_ext.bootstrap import pods_client
...
@click.option("--remote","-r",is_flag=fTmXo,default=fTmXn)
def cmd_cpvcs_list_pods(remote:fTmXt):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=fTmXo)
def cmd_cpvcs_versions(name:fTmXr):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",required=fTmXo,type=fTmXM)
@click.option("-n","--name",help="Name of the cloud pod",required=fTmXo)
def cmd_cpvcs_version_info(version:fTmXM,name:fTmXr):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",type=fTmXM,default=-1,help="Latest version by default")
@click.option("-n","--name",help="Name of the cloud pod",required=fTmXo)
def cmd_cpvcs_version_metamodel(version:fTmXM,name:fTmXr):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",required=fTmXo,type=fTmXM,help="The version the state should be set to")
@click.option("--inject/--no-inject",default=fTmXo,help="Whether the state should be directly injected into the application runtime after changing version")
@click.option("--reset/--no-reset",default=fTmXo,help="Whether the current application state should be reset before changing version")
@click.option("--commit-before",is_flag=fTmXn,help="Whether the current application state should be commited to the currently selected version before changing version")
@click.option("-n","--name",help="Name of the cloud pod",required=fTmXo)
def cmd_cpvcs_set_version(version:fTmXM,inject:fTmXt,reset:fTmXt,commit_before:fTmXt,name:fTmXr):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=fTmXo)
def cmd_cpvcs_commits(version:fTmXM,name:fTmXr):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("--version","-v",required=fTmXo)
@click.option("--commit","-c",required=fTmXo)
@click.option("-n","--name",help="Name of the cloud pod",required=fTmXo)
def cmd_cpvcs_commit_diff(version:fTmXM,commit:fTmXM,name:fTmXr):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
def cmd_pod_list():
 ...
 from localstack_ext.bootstrap import pods_client
 try:
  result=pods_client.list_pods(fTmXJ)
  status.stop()
...
def print_table(columns:Dict[fTmXr,fTmXr],rows:List[Dict[fTmXr,Any]],formatters:Dict[fTmXr,Callable[[Any],fTmXr]]=fTmXJ):
 from rich.table import Table
 if formatters is fTmXJ:
  formatters=fTmXG()
 t=Table()
...
class ConsoleLogger(logging.Logger):
 def __init__(self,name):
  fTmXR(ConsoleLogger,self).__init__(name)
 def info(self,msg:Any,*args:Any,**kwargs:Any)->fTmXJ:
  console.print(msg%args)
 def warning(self,msg:Any,*args:Any,**kwargs:Any)->fTmXJ:
  console.print("[red]Warning:[/red] ",msg%args)
 def error(self,msg:Any,*args:Any,**kwargs:Any)->fTmXJ:
  console.print("[red]Error:[/red] ",msg%args)
 def exception(self,msg:Any,*args:Any,**kwargs:Any)->fTmXJ:
  console.print("[red]Error:[/red] ",msg%args)
  console.print_exception()
