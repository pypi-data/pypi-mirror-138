...
class EC2ElasticIP(GenericBaseModel):
 ...
 def cloudformation_type():
  return "AWS::EC2::EIP"
 def get_physical_resource_id(self,attribute=JMjYq,**kwargs):
...
class SecurityGroupInOrEgress(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  for perm in perms:
   if JMjYV(perm["IpProtocol"])!=JMjYV(rp["IpProtocol"]):
    continue
 ...
 @JMjYc
 def get_vpc_and_name_for_security_group(cls,group_id:JMjYV)->Tuple[JMjYV,JMjYV]:
  client=aws_stack.connect_to_service("ec2")
  groups=client.describe_security_groups(GroupIds=[group_id])["SecurityGroups"]
  vpc_id=groups and groups[0].get("VpcId")or JMjYq
  group_name=groups and groups[0]["GroupName"]or JMjYq
 ...
 def get_deploy_templates(cls):
  def create_params(params,**kwargs):
   ...
   source_group_id=result.pop("SourceSecurityGroupId",JMjYq)
   vpc_id=JMjYq
   ...
   dst_group_id=result.pop("DestinationSecurityGroupId",JMjYq)
   ...
   if result.get("IpProtocol"):
    result["IpProtocol"]=JMjYV(result.get("IpProtocol"))
   description=result.pop("Description",JMjYq)
   cidr_ipv6=result.pop("CidrIpv6",JMjYq)
