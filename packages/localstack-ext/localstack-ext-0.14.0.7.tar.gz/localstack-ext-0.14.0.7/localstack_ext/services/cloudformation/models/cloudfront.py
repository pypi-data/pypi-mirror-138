...
class CloudFrontOriginAccessIdentity(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return result and result[0]or oMbxU
 @oMbxT
...
class CloudFrontFunction(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  return oMbxt(CloudFrontFunction,self).get_cfn_attribute(attribute_name)
 @oMbxq
...
class CloudFrontDistribution(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  origins=origins if oMbxp(origins,oMbxa)else origins.get("Items")
  origins_to_create=oMbxc([orig["DomainName"]for orig in origins])
  aliases=props.get("Aliases",[])
  aliases=aliases if oMbxp(aliases,oMbxa)else aliases.get("Items")
  aliases_to_create=oMbxc(aliases)
  for distr in distrs:
   origins=oMbxc([orig.get("DomainName")for orig in distr.get("Origins",{})["Items"]])
   aliases=oMbxc(distr.get("Aliases",{}).get("Items",[]))
 ...
 def get_deploy_templates():
  def lambda_get_distribution_config(params,**kwargs):
   ...
   config["IsIPV6Enabled"]=config.pop("IPV6Enabled",oMbxL)
   config["Enabled"]=oMbxe(config.get("Enabled")).lower()=="true"
   cert=config["ViewerCertificate"]=config.get("ViewerCertificate",{})
   custom_errors=config.get("CustomErrorResponses",[])
   custom_errors=(custom_errors.get("Items",[])if oMbxp(custom_errors,oMbxh)else custom_errors)
   for item in custom_errors:
    item["ResponseCode"]=oMbxe(item.get("ResponseCode"))
   cache=config["DefaultCacheBehavior"]
   cache["TrustedSigners"]=cache.get("TrustedSigners")or{"Quantity":0,"Enabled":oMbxL}
   cache["MinTTL"]=cache.get("MinTTL")or 600
   forwarded=cache["ForwardedValues"]=cache.get("ForwardedValues",{})
   forwarded["QueryString"]=oMbxe(forwarded.get("QueryString")).lower()=="true"
   ...
   def convert_list(key,parent=oMbxU):
    parent=config if parent is oMbxU else parent
    items=parent.get(key)or[]
    if oMbxp(items,oMbxh)and "Items" in items and "Quantity" in items:
     return
    parent[key]={"Quantity":oMbxs(items),"Items":items}
   cache_behavs=config.get("CacheBehaviors",[])
   cache_behavs=(cache_behavs.get("Items",[])if oMbxp(cache_behavs,oMbxh)else cache_behavs)
