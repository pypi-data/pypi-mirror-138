...
class RDSDBSubnetGroup(GenericBaseModel):
 ...
 def get_physical_resource_id(self,attribute=BmAgt,**kwargs):
...
class RDSDBCluster(GenericBaseModel):
 ...
 @BmAga
 def add_defaults(resource:Dict,stack_name:BmAgP):
 ...
 def get_cfn_attribute(self,attribute):
  ...
  return BmAgQ(RDSDBCluster,self).get_cfn_attribute(attribute)
 @BmAga
 def get_deploy_templates():
  ...
  def _params(params,**kwargs):
   ...
   if scaling_config:
    if "MinCapacity" in scaling_config:
     scaling_config["MinCapacity"]=BmAgO(scaling_config["MinCapacity"])
    if "MaxCapacity" in scaling_config:
     scaling_config["MaxCapacity"]=BmAgO(scaling_config["MaxCapacity"])
   return params
  result={"create":{"function":"create_db_cluster","parameters":_params,"types":{"BackupRetentionPeriod":BmAgO,"Port":BmAgO}},"delete":{"function":"delete_db_cluster","parameters":["DBClusterIdentifier"]}}
  return result
...
class RDSDBInstance(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute):
  ...
  return BmAgQ(RDSDBInstance,self).get_cfn_attribute(attribute)
 def fetch_state(self,stack_name,resources):
 ...
 def get_deploy_templates():
  ...
  return{"create":{"function":"create_db_instance","parameters":attrs+[{"VpcSecurityGroupIds":"VPCSecurityGroups"}],"types":{"AllocatedStorage":BmAgO,"Port":BmAgO}},"delete":{"function":"delete_db_instance","parameters":["DBInstanceIdentifier"]}}
