...
class SESTemplate(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return(template or[deNJB])[0]
 def get_physical_resource_id(self,attribute=deNJB,**kwargs):
 ...
 def update_resource(self,new_resource,stack_name,resources):
  ...
  if deNJT(template.get(attr,"")==new_template.get(attr,"")for attr in["SubjectPart","TextPart","HtmlPart"]):
   return
...
class SESReceiptRuleSet(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute):
  return deNJb(SESReceiptRuleSet,self).get_cfn_attribute(attribute)
 @deNJk
 def add_defaults(resource,stack_name:deNJo):
...
class SESReceiptRule(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute):
  return deNJb(SESReceiptRule,self).get_cfn_attribute(attribute)
 @deNJX
