...
class ProCliPlugin(LocalstackCliPlugin):
 ...
 def should_load(self):
  ...
  return DeamV if e else DeamJ
 def is_active(self):
  return self.should_load()
 def attach(self,cli:LocalstackCli)->DeamU:
...
@click.option("--revert",is_flag=DeamV,help="Revert systemd-resolved settings for the docker interface")
def cmd_dns_systemd(revert:Deamq):
...
def _cpvcs_initialized(pod_name:Deamf)->Deamq:
...
@click.option("-n","--name",help="Name of the cloud pod",required=DeamV)
def cmd_cpvcs_init(name:Deamf):
...
@click.option("-n","--name",help="Name of the cloud pod",required=DeamV)
@click.option("-r","--remote",help="Whether the Pod should also be deleted remotely.",is_flag=DeamV,default=DeamJ)
def cmd_cpvcs_delete(name:Deamf,remote:Deamq):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=DeamV)
def cmd_cpvcs_register_remote(name:Deamf):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Current Name of the cloud pod",required=DeamV)
@click.option("-nn","--new-name",help="New name of the cloud pod",required=DeamV)
def cmd_cpvcs_rename(name:Deamf,new_name:Deamf):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-n","--name",help="Name of the cloud pod",required=DeamV)
def cmd_cpvcs_commit(message:Deamf,name:Deamf):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("--squash",is_flag=DeamV,help="Squashes commits together, so only the latest commit is stored in the revision graph")
@click.option("--three-way",is_flag=DeamV,default=DeamJ,help="")
@click.option("-m","--message",help="Add a comment describing the version")
@click.option("-n","--name",help="Name of the cloud pod",required=DeamV)
def cmd_cpvcs_push(squash:Deamq,message:Deamf,name:Deamf,three_way:Deamq):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-n","--name",help="Name of the cloud pod",required=DeamV)
@click.option("-v","--version",type=Deamj)
@click.option("-m","--message",required=DeamJ)
def cmd_cpvcs_push_overwrite(version:Deamj,message:Deamf,name:Deamf):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-v","--version",default="-1",type=Deamj,help="Loads the state of the specified version - Most recent one by default")
@click.option("--reset",is_flag=DeamV,default=DeamJ,help="Will reset the application state before injecting")
@click.option("-n","--name",help="Name of the cloud pod",required=DeamV)
def cmd_cpvcs_inject(version:Deamj,reset:Deamq,name:Deamf):
 from localstack_ext.bootstrap import pods_client
...
@click.option("--inject/--no-inject",default=DeamV,help="Whether the latest version of the pulled pod should be injected")
@click.option("--reset/--no-reset",default=DeamV,help="Whether the current application state should be reset after the pod has been pulled")
@click.option("-n","--name",help="Name of the cloud pod",required=DeamV)
@click.option("--lazy/--eager",default=DeamV,help="Will only fetch references to existing versions, i.e. version state is only downloaded when required")
@cpvcs.command(name="pull",help="Injects the state from a version into the application runtime")
def cmd_cpvcs_pull(name:Deamf,inject:Deamq,reset:Deamq,lazy:Deamq):
 from localstack_ext.bootstrap import pods_client
...
@click.option("--remote","-r",is_flag=DeamV,default=DeamJ)
def cmd_cpvcs_list_pods(remote:Deamq):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=DeamV)
def cmd_cpvcs_versions(name:Deamf):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",required=DeamV,type=Deamj)
@click.option("-n","--name",help="Name of the cloud pod",required=DeamV)
def cmd_cpvcs_version_info(version:Deamj,name:Deamf):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",type=Deamj,default=-1,help="Latest version by default")
@click.option("-n","--name",help="Name of the cloud pod",required=DeamV)
def cmd_cpvcs_version_metamodel(version:Deamj,name:Deamf):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",required=DeamV,type=Deamj,help="The version the state should be set to")
@click.option("--inject/--no-inject",default=DeamV,help="Whether the state should be directly injected into the application runtime after changing version")
@click.option("--reset/--no-reset",default=DeamV,help="Whether the current application state should be reset before changing version")
@click.option("--commit-before",is_flag=DeamJ,help="Whether the current application state should be commited to the currently selected version before changing version")
@click.option("-n","--name",help="Name of the cloud pod",required=DeamV)
def cmd_cpvcs_set_version(version:Deamj,inject:Deamq,reset:Deamq,commit_before:Deamq,name:Deamf):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=DeamV)
def cmd_cpvcs_commits(version:Deamj,name:Deamf):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("--version","-v",required=DeamV)
@click.option("--commit","-c",required=DeamV)
@click.option("-n","--name",help="Name of the cloud pod",required=DeamV)
def cmd_cpvcs_commit_diff(version:Deamj,commit:Deamj,name:Deamf):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
def cmd_pod_list():
 ...
 from localstack_ext.bootstrap import pods_client
 try:
  result=pods_client.list_pods(DeamU)
  status.stop()
...
def print_table(columns:Dict[Deamf,Deamf],rows:List[Dict[Deamf,Any]],formatters:Dict[Deamf,Callable[[Any],Deamf]]=DeamU):
 from rich.table import Table
 if formatters is DeamU:
  formatters=DeamS()
 t=Table()
...
class ConsoleLogger(logging.Logger):
 def __init__(self,name):
  DeamX(ConsoleLogger,self).__init__(name)
 def info(self,msg:Any,*args:Any,**kwargs:Any)->DeamU:
  console.print(msg%args)
 def warning(self,msg:Any,*args:Any,**kwargs:Any)->DeamU:
  console.print("[red]Warning:[/red] ",msg%args)
 def error(self,msg:Any,*args:Any,**kwargs:Any)->DeamU:
  console.print("[red]Error:[/red] ",msg%args)
 def exception(self,msg:Any,*args:Any,**kwargs:Any)->DeamU:
  console.print("[red]Error:[/red] ",msg%args)
  console.print_exception()
