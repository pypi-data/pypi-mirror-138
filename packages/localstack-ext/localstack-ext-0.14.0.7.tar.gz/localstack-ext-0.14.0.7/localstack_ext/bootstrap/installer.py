  ...
  try:
   if not self._check_if_available():
    try:
     ...
     if epTIn(e,PackageInstallationException):
      raise
     else:
      suffix=(f" ({self.log_package_name})." if self.log_package_name is not epTIP else ".")
   ...
   if raise_on_error:
    ...
    if self.logger.isEnabledFor(logging.DEBUG):
     ...
     self.logger.error(e)
 def _check_if_available(self)->epTIf:
  raise epTIQ
 def _prepare_installation(self)->epTIP:
  pass
 def _install_package(self)->epTIP:
  raise epTIQ
 def _post_process(self)->epTIP:
  pass
 def _verify_installation(self)->epTIP:
  if not self._check_if_available():
   package_info=(f" of {self.log_package_name}" if self.log_package_name is not epTIP else "")
...
class MultiPackageInstaller(PackageInstaller):
 def __init__(self,log_package_name:Optional[epTIx],package_installers:Union[PackageInstaller,List[PackageInstaller]]):
  epTIh(MultiPackageInstaller,self).__init__(log_package_name=log_package_name)
  self.package_installers=(package_installers if epTIn(package_installers,List)else[package_installers])
 def install(self,raise_on_error:epTIf=epTIU):
...
class OSSpecificPackageInstaller(MultiPackageInstaller):
 def __init__(self,debian_installers:PackageInstaller,redhat_installers:PackageInstaller):
  ...
  epTIh(OSSpecificPackageInstaller,self).__init__(log_package_name=self.__class__.__name__,package_installers=package_installers)
 def install(self,raise_on_error:epTIf=epTIU):
  try:
   if not in_docker():
    ...
    epTIh(OSSpecificPackageInstaller,self).install(raise_on_error=raise_on_error)
  except PackageInstallationException as e:
   if raise_on_error:
    raise
   else:
...
class DebianPackageInstaller(PackageInstaller,ABC):
 def __init__(self,package_name:epTIx):
  epTIh(DebianPackageInstaller,self).__init__(log_package_name=package_name)
 def _install_os_packages(self,packages:Union[epTIx,List[epTIx]]):
  packages=packages if epTIn(packages,List)else[packages]
  with INSTALL_LOCK:
   run(["apt-get","update"])
   run(["apt-get","install","-y","--no-install-recommends"]+packages)
class RedHatPackageInstaller(PackageInstaller,ABC):
 def __init__(self,package_name:epTIx):
  epTIh(RedHatPackageInstaller,self).__init__(log_package_name=package_name)
 def _install_os_packages(self,packages:Union[epTIx,List[epTIx]]):
  packages=packages if epTIn(packages,List)else[packages]
  with INSTALL_LOCK:
...
class DebianPostgres11Installer(DebianPackageInstaller):
 def __init__(self):
  epTIh(DebianPostgres11Installer,self).__init__("postgres11")
 def _check_if_available(self)->epTIf:
...
class RedHatPostgres11Installer(RedHatPackageInstaller):
 def __init__(self):
  epTIh(RedHatPostgres11Installer,self).__init__("postgres11")
 def _check_if_available(self)->epTIf:
  return is_command_available("psql")
 def _prepare_installation(self)->epTIP:
...
class DebianPlPythonInstaller(DebianPackageInstaller):
 def __init__(self):
  epTIh(DebianPlPythonInstaller,self).__init__("plpython3")
 def _check_if_available(self)->epTIf:
...
class RedHatPlPythonInstaller(RedHatPackageInstaller):
 def __init__(self):
  epTIh(RedHatPlPythonInstaller,self).__init__("plpython3")
 def _check_if_available(self)->epTIf:
...
class DebianMariaDBInstaller(DebianPackageInstaller):
 def __init__(self):
  epTIh(DebianMariaDBInstaller,self).__init__("MariaDB")
 def _check_if_available(self)->epTIf:
...
class RedHatMariaDBInstaller(RedHatPackageInstaller):
 def __init__(self):
  epTIh(RedHatMariaDBInstaller,self).__init__("MariaDB")
 def _check_if_available(self)->epTIf:
  return is_command_available("mysqld")
 def _install_package(self)->epTIP:
...
class DebianTimescaleDBInstaller(DebianPackageInstaller):
 def __init__(self):
  epTIh(DebianTimescaleDBInstaller,self).__init__("timescaledb")
 def _check_if_available(self)->epTIf:
...
class RedHatTimescaleDBInstaller(RedHatPackageInstaller):
 def __init__(self):
  epTIh(RedHatTimescaleDBInstaller,self).__init__("timescaledb")
 def _check_if_available(self)->epTIf:
...
class DebianRedisInstaller(DebianPackageInstaller):
 def __init__(self):
  epTIh(DebianRedisInstaller,self).__init__("Redis")
 def _check_if_available(self)->epTIf:
...
class RedHatRedisInstaller(RedHatPackageInstaller):
 def __init__(self):
  epTIh(RedHatRedisInstaller,self).__init__("Redis")
 def _check_if_available(self)->epTIf:
  return is_command_available("redis-server")
 def _install_package(self)->epTIP:
...
class DebianMosquittoInstaller(DebianPackageInstaller):
 def __init__(self):
  epTIh(DebianMosquittoInstaller,self).__init__("Mosquitto")
 def _check_if_available(self)->epTIf:
...
class RedHatMosquittoInstaller(RedHatPackageInstaller):
 def __init__(self):
  epTIh(RedHatMosquittoInstaller,self).__init__("Mosquitto")
 def _check_if_available(self)->epTIf:
  return is_command_available("mosquitto")
 def _install_package(self)->epTIP:
