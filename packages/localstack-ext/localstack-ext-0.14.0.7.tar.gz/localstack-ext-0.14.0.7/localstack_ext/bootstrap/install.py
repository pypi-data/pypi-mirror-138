...
def install_libs():
 ...
 postgres_installer.install(raise_on_error=bJydS)
 timescaledb_installer.install(raise_on_error=bJydS)
 redis_installer.install(raise_on_error=bJydS)
 mosquitto_installer.install(raise_on_error=bJydS)
...
def setup_ssl_cert():
 ...
 try:
  ...
  try:
   ...
   raise
def download_github_artifact(url:bJydG,target_file:bJydG,timeout:int=bJydx):
 def do_download(url,print_error=bJydS):
  try:
