...
SENT_EMAILS=[]
EMAIL_BLACKLIST=AKGsi()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",AKGsn(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=AKGsk,from_name=AKGsk,smtp_host=AKGsk,smtp_user=AKGsk,smtp_pass=AKGsk,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if AKGsu(recipients,AKGsI)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=AKGsk):
 ...
 result["From"]=formataddr((AKGsn(Header(from_name,"utf-8")),from_email))
 return result
