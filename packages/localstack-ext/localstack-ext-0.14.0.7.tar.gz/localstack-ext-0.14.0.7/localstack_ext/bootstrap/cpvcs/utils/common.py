...
class CPVCSConfigContext:
 ...
 def get_version_state_archive_path(self,version,with_format:PYAms=PYAme)->PYAmo:
 ...
 def update_ver_log(self,author:PYAmo,ver_no:PYAmR,rev_id:PYAmo,rev_no:PYAmR):
  with PYAmS(self.get_ver_log_path(),"a")as fp:
   fp.write(f"{VER_LOG_STRUCTURE.format(author=author, ver_no=ver_no, rev_rid_no=f'{rev_id}_{rev_no}')}\n")
 def create_version_symlink(self,name:PYAmo,key:PYAmo=PYAmv)->PYAmo:
  return self._create_symlink(name,key,self.get_ver_refs_path())
 def create_revision_symlink(self,name:PYAmo,key:PYAmo=PYAmv)->PYAmo:
  return self._create_symlink(name,key,self.get_rev_refs_path())
 def is_initialized(self)->PYAms:
  return self.pod_root_dir and os.path.isdir(self.pod_root_dir)
 def _create_symlink(self,name:PYAmo,key:PYAmo,path:PYAmo)->PYAmo:
...
def zip_directories(zip_dest:PYAmo,directories:List[PYAmo])->PYAmo:
 ...
 return zip_dest
def add_file_to_archive(archive:PYAmo,entry_name:PYAmo,content:Union[PYAmo,PYAmX]):
 with zipfile.ZipFile(archive,"a")as zip_file:
  zip_file.writestr(entry_name,content)
def read_file_from_archive(archive_path:PYAmo,file_name:PYAmo)->PYAmo:
 try:
