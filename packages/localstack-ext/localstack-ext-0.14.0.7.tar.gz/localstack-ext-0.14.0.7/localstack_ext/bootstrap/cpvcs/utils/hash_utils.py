...
def random_hash()->Vgkij:
 return VgkiJ(random.getrandbits(160))
def compute_file_hash(file_path:Vgkij,accum=Vgkit)->Optional[Vgkij]:
 try:
  with Vgkiq(file_path,"rb")as fp:
   if accum:
...
def compute_version_archive_hash(version_no:Vgkif,state_archive:Vgkij)->Vgkij:
 ...
 m.update(Vgkij(version_no).encode("utf-8"))
 rm_rf(tmp_state_dir)
 return m.hexdigest()
def compute_revision_hash(cpvcs_node:Revision)->Vgkij:
 if not cpvcs_node.state_files:
  return random_hash()
 state_file_keys=VgkiX(lambda state_file:state_file.hash_ref,cpvcs_node.state_files)
 m=hashlib.sha1()
 for key in state_file_keys:
  try:
   with Vgkiq(os.path.join(config_context.get_obj_store_path(),key),"rb")as fp:
    m.update(fp.read())
  except Vgkir as e:
   LOG.warning(f"Failed to open file and compute hash for {key}: {e}")
 if Vgkie(cpvcs_node,Revision):
  m.update(cpvcs_node.rid.encode("utf-8"))
  m.update(Vgkij(cpvcs_node.revision_number).encode("utf-8"))
 elif Vgkie(cpvcs_node,Version):
  m.update(Vgkij(cpvcs_node.version_number).encode("utf-8"))
 return m.hexdigest()
