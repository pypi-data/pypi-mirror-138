...
CAPTURE_METAMODEL_SEPARATELY=tlGfS
...
class MetamodelDelta(SubtypesInstanceManager):
 def create_delta_for_state_files(self,sf1:StateFileRef,sf2:StateFileRef):
  ...
  if not sf1:
   backend2=load_persisted_object(config_context.get_obj_file_path(sf2.hash_ref))
   clazz=tlGfT(backend2)
   backend1=_infer_backend_init(clazz,sf2)
  else:
   backend1=load_persisted_object(config_context.get_obj_file_path(sf1.hash_ref))
   clazz=tlGfT(backend1)
   backend2=_infer_backend_init(clazz,sf1)
  diff=self.create_delta_json(backend1,backend2)
  return diff
 def create_delta_log(self,state_from:Set[StateFileRef],state_to:Set[StateFileRef])->tlGfJ:
  ...
  def _create_sf_lookup(state_files:Set[StateFileRef])->Dict[tlGfJ,StateFileRef]:
  ...
  for state_file_from in state_files_from_regular:
   ...
   if state_file_from.any_congruence(state_files_to_regular):
    ...
    diff_json=self.create_delta_for_state_files(state_file_from,tlGfV)
  ...
  for state_files_to in state_files_to_lookup.values():
   ...
   diff_json=self.create_delta_for_state_files(tlGfV,state_files_to)
...
class CommitMetamodelUtils:
 @tlGfD
 def create_metadata_archive(cls,version:Version,delete_reference:bool=tlGfS,overwrite:bool=tlGfS,metamodels_file:tlGfJ=tlGfV):
 ...
 def get_metamodel_delta(cls,prev_metamodel:Dict,this_metamodel:Dict)->Dict:
  ...
  return result
 @tlGfD
 def create_delta_metamodel_file(cls,version:tlGfB,revision:tlGfB,store_to_zip=tlGfS)->Tuple[Dict,Dict]:
 ...
 @tlGfD
 def reconstruct_metamodel(cls,version:tlGfB,revision:tlGfB=tlGfV)->Dict:
  if not revision:
   revision=infer_max_commit_diff(archive_path=PodFilePaths.get_version_meta_archive(version))
  result={}
  for rev in tlGfp(1,revision+1):
  ...
  return result
 @tlGfD
 def get_version_metamodel(cls,version:tlGfB,revision:tlGfB=tlGfV)->Dict:
 ...
 @tlGfD
 def get_commit_diff(cls,version_no:tlGfB,commit_no:tlGfB)->Optional[Dict]:
  ...
  return result
def _infer_backend_init(clazz:Type,sf:StateFileRef)->tlGfy:
 if tlGfk(clazz,tlGfA):
  return{}
 constructor=tlGfx(clazz,"__init__",tlGfV)
...
def _create_metamodel_helper(obj,width=25,visited:set=tlGfV):
 if obj is tlGfV:
  return obj
 ...
 result=obj=obj_dict if obj_dict is not tlGfV else obj
 if tlGfk(obj,tlGfA):
  result=tlGfA(result)
  for field_name,field_value in result.items():
   result[field_name]=_create_metamodel_helper(field_value,width=width,visited=visited)
 elif tlGfk(obj,tlGfa):
  result=[_create_metamodel_helper(o,width=width,visited=visited)for o in obj]
  if tlGfH(result)>width:
   result={"size":tlGfH(result),"items":result[:width]}
 return result
def infer_max_commit_diff(archive_path:tlGfJ)->tlGfB:
 max_commit=1
 with zipfile.ZipFile(archive_path)as archive:
