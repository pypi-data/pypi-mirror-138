...
class Display:
    ...
    def agent(self, step: Step = None, agent_name: str = None, location: Location = None, rotation: float = None, color=None, size: float = 40.0, show_trajectory: bool = True):
        ...
        if agent_name not in self.agents:
            self.agents[agent_name], = self.ax.plot(location.x, location.y, marker=Agent_markers.robot() if step.agent_name=="predator" else Agent_markers.mouse(), c=color, markersize=size)
