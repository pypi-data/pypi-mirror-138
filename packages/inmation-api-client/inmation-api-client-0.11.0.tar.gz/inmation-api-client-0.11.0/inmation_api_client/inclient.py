...
class Client(object):
    ...
    def Connect(self, host: str = '127.0.0.1', port: int = 8002, options=None, ssl=None):
        """ Connect to the WebSocket server.
        Args:
            host: Host name or IP address where the Web API is running
            port: The port of the Web API
            options (object): Options object
            ssl: Use secure websocket (ssl=None: Insecure, ssl=True: Secure, ssl=ssl.SSLContext object: Advanced SSL config)
        Returns:
            void
        """
        try:
            if ssl:
                url = 'wss://{}:{}/ws'.format(host, str(port))
            else:
                url = 'ws://{}:{}/ws'.format(host, str(port))
            self.GetEventLoop().run_until_complete(self._ws_client.connect(url, options, ssl))
