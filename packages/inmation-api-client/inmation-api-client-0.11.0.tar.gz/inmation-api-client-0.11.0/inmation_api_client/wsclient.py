...
class WSClient(EventEmitter):
    ...
    async def receive_msg(self):
        try:
            msg = await asyncio.wait_for(self._ws.recv(), timeout=self.options.tim or 10)
    ...
    async def send(self, data, retry=True):
        """Send a data message.
        Args:
        ...
        if self._ws is not None:
            if self._ws.state == State.OPEN:
                ...
                while True:
                    if self._ws._pop_message_waiter is None:
                        ...
                        if response is not None:
                            res_reqid = response.get('reqid', None)
                            if res_reqid == None or res_reqid == reqid:
                                return response
                            elif res_reqid < reqid:
                                # Got a response for an aborted previous request. Try to receive again
                                continue
                    return None
            else:
                if retry and self.auto_reconnect:
                    await self.reconnect()
                    # Retry the request
                    return await self.send(data, False)
        return None
