...
class WorkerThread(threading.Thread):
        ...
        try:
            while True:
                ...
                if package == b'JOB_CANCELLED_BY_USER':
                    logger.info(f"User sent cancel signal for job {self.compute_state['job_id']}")
    ...
    def terminate(self) -> None:
        ...
        if self._job_worker_process:
            ...
            for _ in range(10):
                if self._job_worker_process.exitcode is not None:
                    logger.debug(f'Worker process exitcode {self._job_worker_process.exitcode}')
                    break
                else:
                    logger.debug('Waiting for worker process to exit...')
                    time.sleep(1)

            if self._job_worker_process.exitcode is None:
                # TODO: Figure out if more error handling is necessary here
                logger.error(f'Worker process for job {job_id} did not exit within 10 seconds')
        ...
        logger.debug(f'Worker thread for job {job_id} terminated')
        sys.exit()
