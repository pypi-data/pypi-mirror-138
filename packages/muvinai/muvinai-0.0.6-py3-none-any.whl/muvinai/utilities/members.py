...
def member_unsubscribe(member, reason, unsubscribe_request=False):
    ...
    if "history" in member.keys():
        member["history"].append(history)
    else:
        member["history"] = [history]

    member["next_payment_date"] = None
    member["status"] = "inactivo" if not unsubscribe_request else "baja"

    db.clientes.update_one({"_id": member["_id"]}, {"$set": member})
