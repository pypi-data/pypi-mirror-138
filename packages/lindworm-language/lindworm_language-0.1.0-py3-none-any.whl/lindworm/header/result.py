...
OriginalSignature = t.Callable[Q, Result[W]]


def on_propagate_err(optional_func: t.Callable[[UnwrapError], t.Any] = lambda x: err) -> t.Callable[OriginalSignature, OriginalSignature]:
    """
...
def propagate_err(func: OriginalSignature) -> OriginalSignature:
    """Decorator that returns Err() if the function raises an UnwrapError."""
    return on_propagate_err()(func)


def wrap_result(func: t.Callable[Q, W]) -> t.Callable[Q, Result[W]]:
    """Decorator that wraps the result of the function into Ok if it was successful or Err if it raised an exception."""
