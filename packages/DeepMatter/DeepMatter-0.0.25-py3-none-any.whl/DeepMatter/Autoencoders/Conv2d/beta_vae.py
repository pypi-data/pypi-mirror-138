...
from tensorflow.keras.layers import (Input,Reshape,Activation,Attention,MaxPool1D,Dense, Conv1D, Convolution2D, GRU, LSTM, Lambda, Bidirectional, TimeDistributed,
                          Dropout, Flatten, layer_normalization,RepeatVector, Reshape, MaxPooling1D, UpSampling1D, BatchNormalization)
...
class model_builder:
    ...
    def identity_block(self, X, name,
        ...
        X = layers.layer_normalization(axis=1, name=bn_name_base + '_res_end')(X)
        X = layers.Activation('relu')(X)
    ...
    def model_constructor(self, input_data):
        ...
        X = layers.layer_normalization(axis=1, name='batch_normal')(X)
