        ...
        for collection in kind.collections():
            ...
            print(
                "{:<15s}{:<15s}{:>15s} {:<20s}".format(
                    kind.type,
                    latest.updated_at,
                    mc.util.to_readable_size(latest.size),
                    latest.name,
                )
            )
...
def upload(path, name, description, type, alias):
    ...
    artifact = mc.Artifact(name=artifact_name, type=type, description=description)
    ...
    run = mc.init(entity=entity, project=project, config={"path": path}, job_type="cli_put")
...
@click.argument("run", envvar=mc.env.RUN_ID)
@click.option("--project", "-p", envvar=mc.env.PROJECT, help="The project you want to download.")
@click.option(
    "--entity",
    "-e",
    default="models",
...
def pull(run, project, entity):
    ...
    for name in urls:
        if api.file_current(name, urls[name]["md5"]):
            ...
            dirname = os.path.dirname(name)
            if dirname != "":
                mc.util.mkdir_exists_ok(dirname)
            with click.progressbar(
                length=length,
                label="File %s" % name,
                fill_char=click.style("&", fg="green"),
