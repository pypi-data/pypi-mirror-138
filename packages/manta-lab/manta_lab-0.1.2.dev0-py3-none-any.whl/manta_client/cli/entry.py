...
def manta_api(settings=None):
    s = settings or mc.setup().settings
    api = mc.api.MantaAPI(settings=s)
    api.cli_setup()
    return api
...
def login(key, host, cloud, relogin):
    ...
    mc.login(api_key=key, relogin=relogin, base_url=host)
...
@click.option("--entity", help="The type of artifacts to list")
@click.option("--experiment", "-e", help="The type of artifacts to list")
@click.option("--type", "-t", help="The type of artifacts to list")
@display_error
def ls(project, entity, experiment, type):
    api = manta_api()

    s = mc.Settings(entity=entity, project=project, experiment_name=experiment)
...
def create(name, description, group, labels):
    api = manta_api()
    entity, project, experiment, artifact_name = api.parse_artifact_path(name)
    s = mc.Settings(entity=entity, project=project, experiment_name=experiment)

    artifact = mc.Artifact(artifact_name, description=description, group=group, labels=labels)
