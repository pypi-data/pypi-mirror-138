...
class Gui:
    def __init__(self,config=None,help=True,pargs={},**kwargs):
        ...
        #print('... init figure ...')
        w,wm=0.5,0.025
        axpos = list(np.array([1-wm-w,wm+0.05,w,16/9*w]))
        self.pargs={'figsize':'f','axPos':axpos,'pOpt':'Xpt'}
        self.pargs.update(pargs)
        self.fig,self.ax = dsp.stddisp(**self.pargs)
        wb,hb=0.1,0.075
        self.axBtn=self.fig.add_axes([1-w-2*wm-wb,1-hb-wm,wb,hb])
        bn_mode = Button(self.axBtn, 'toggle mode')
        bn_mode.on_clicked(self.toggle_mode)
        self.mode_txt=self.fig.text(0.5,0.5,self.mode,fontsize=15,color='g')
    ...
    def toggle_mode(self,event):
        print('ok')
        mode='rotate'
        if self.mode=='rotate':mode='frames'
        self.set_mode(mode)

    def set_mode(self,mode):
        self.mode = mode
        self.mode_txt.label=self.mode
