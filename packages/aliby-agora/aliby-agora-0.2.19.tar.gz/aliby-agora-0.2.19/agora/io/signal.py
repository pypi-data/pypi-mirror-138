    ...
    def apply_prepost(self, dataset: str):
        ...
        with h5py.File(self.filename, "r") as f:
            ...
            if merges.any():
                ...
                mergeable_ids = merges.reshape(-1, 2)
                valid_ids = mergeable_ids[  # Casting allows fast multiindexing
                    (
                        mergeable_ids.T[:, :, None]
                ...
                merged = self.apply_merge(df.loc[df.loc[map(tuple, valid_ids)]], merges)

                nonmergeable_ids = df.index.difference(mergeable_ids)
