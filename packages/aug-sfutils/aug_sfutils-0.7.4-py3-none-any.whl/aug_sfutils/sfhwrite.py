...
# SIGNALS list generated automatically, override input entry
    ...
    sigs.objlist = pack('>%d%s' %(sigs.nitems, sfmt), *obj_list)
...
# Set lengths and addresses
    ...
    for jid in range(len(sfo.sfh)):
        ...
        if otyp == Diagnostic:
            sfobj.length = sfobj.length # GIT: not really evaluated, determine at the end
            sfobj.address = addr
        elif otyp == List:
            if jid == 1:
            ...
            addr += n_block*((sfobj.length + n_block-1)//n_block) + (shape_arr[1]-1)*2
        elif otyp == AreaBase:
            sfobj.length = object_length(sfobj)
            sfobj.address = addr
            ...
            continue
        if otyp not in (Diagnostic, ParamSet):
            len_tot += sfobj.length
 
    logger.debug('Total length %d %d', len_tot, sfobj.address + sfobj.length - len_psets)
    return sfo
...
# Write SFH file
    ...
    for jid in range(len(sfo.sfh)):
        ...
        sfobj.level = 0
        sfobj.status = 65535
        logger.debug('Obj: %s, type: %d', sfobj.objnam, sfobj.obj_type)
        logger.debug('Addr: %d', sfobj.address)
        logger.debug('Len : %d', sfobj.length)
