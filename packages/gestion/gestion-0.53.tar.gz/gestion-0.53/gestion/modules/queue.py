			...
			try:
				...
				if doc:
					...
					try:
						...
						doc[ "status" ]	= "done"
						log_id			= node.write_to_log( f"Loop_queue: {results.output}" )
...
def process_queued_item( queued_item ):
	...
	if name in addons.available_addons():
		...
		a = 0
		while addon.state:
			print( f"Addon runnning, iteration:{a}" )
	...
	print( "Going to get output or error" )
