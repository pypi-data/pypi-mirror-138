	...
	def set_all_values( self, for_insert = True ):
		...
		if for_insert:
			...
			for key in self.keys( ignore = [ "id", "saved" ] ):
				...
				if _function != None:
					_saved[ "_functions" ][ key ] = f'{_function[ "_func_url" ]}'
			_saved[ "_id" ] = self._id
					...
					for key in result.keys():
						...
						if not key.startswith( "_" ):
							...
							if instance.function != None:
								add_web_command( instance, check_exists = True )
						elif key == "_local":
...
class Datatype( dict ):
	...
	def __setattr__( self, attr, value ):
		if attr == "value":
			...
			function = { "_func_module" : value.__module__, "_func_name" : value.__name__ }
			dict.__setitem__( self, f"_{attr}", function )
			if attr == "function":
				function[ "_func_url" ] = add_web_command( self )
	...
	def as_html( self, doc:Document ):
		# other name, htmlizer or something?
		if self.html == None:
			return self.value
		else:
			return self.html( doc )
...
#-----------------------------------------------------------------------
def add_web_command( datatype, check_exists = False ):
	# change name to add_as_webscript? or something
	from modules import web
	exists 	= False
	func	= datatype.function
	url		= f"/cli/{func.__name__}"
	...
	if web.server != None:
		...
		if not exists:
			pass
			# ~ web.create_command( func, name = url )
...
# new name? -> _registered_database_changes
changes_collection 		= create_collection( "_database_changes", changes )


#-----------------------------------------------------------------------
def check_for_changes( collection ):
	# ~ print( f"check_for_changes( {collection} )" )
	# ~ coll 		= get_collection( "_database_changes" )
...
# ~ d = Boolean()
# ~ d.function = check_for_changes
# ~ print( add_web_command( d ) )
