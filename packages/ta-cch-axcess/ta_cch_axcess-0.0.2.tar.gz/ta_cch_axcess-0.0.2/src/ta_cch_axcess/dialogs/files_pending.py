...
class FilesPending(CchDialog):
    def __init__(self, cch_app):
        super(FilesPending, self).__init__(
			cch_app,
			title_re='Files Pending Approval',
            found_index=0,
			visible_only=False
		)
    def process_files_in_document(self, tax_year, client_id):
        ...
        self.window_popup.child_window(
        ...
        self.window_popup_approve = self.app.window(
            title="Approve Files"
        )
        self.window_popup_approve.wait('exists', timeout=10)
        self.window_popup_approve.child_window(
