...
class _Client:
    ...
    def open_connection(self, timeout=0):
        if timeout:
            self.sock.settimeout(timeout)
        try:
            self.sock = socket.socket()
            self.sock.connect(("localhost", 4741))
            print("Connected to AlphAI, type help(alphai) for more info")
            self.robot_connected = self._send("connected", res=True)
        except ConnectionRefusedError:
            print("No AlphAI instance running, you can connect by calling")
    ...
    def connect(self, robot_number=True):
        ...
        if isinstance(robot_number, bool):
            self.robot_connected = self._send("connect", robot_number,
                                              res=True)
