...
class ModelEnumValue:
    ...
    избежать дублирования имен и использования в коде и базе одинакового идентификатора, что упростит задачу задачу при
    ...
    def __init__(self, key=None, **kwargs):
        ...
        """
        self._required_fields = [KEY_FIELD_NAME]
...
class ModelEnumMetaclass(ModelBase):
    """
    Метакласс для моделей-перечислений с переопределенным набором параметрав в Meta. Поведение достигается путем замены
...
class ModelEnum(Model, metaclass=ModelEnumMetaclass):
    ...
    подчеркивание. Значением перечисления является именованный кортеж ModelEnumValue, содержащий поля value и title.
    ...
    - Если key нет в БД, но есть в значених перечисления, то было добавлено новое значение перечисления.
    ...
    отключение плагина. Если модель-пречисление расширяемая, то при отключении плагина, значения элементов перечисления
    не будут удаляться из БД при актуализации значений. Т.к. ссылка на перечисление представлена в виде внешнего ключа,
    то при попытке удаления значения из модели-пересления, в случае использования, будет получена ошибка о
    ...
    @classmethod
    def extend(cls, key, **kwargs):
        """
        Метод расширения модели-перечисления, например из плагина. Необходимо, чтобы сама модель-пречисление была
        расширяяемой. Для этого необходимо, чтобы был установлен extensible = True в Meta

        Args:
            key: ключ элемента перечисления, указывается заглавными буквами с разделителем нижнее подчеркивание
            value: значение перечисления, которое будет использоваться в качестве первичного ключа при создании записи
                в таблице перечисления
            title: наименование элемента перечисления, обычно используемое для отображения пользователю
        """
        setattr(cls, key, ModelEnumValue(key=key, **kwargs))
