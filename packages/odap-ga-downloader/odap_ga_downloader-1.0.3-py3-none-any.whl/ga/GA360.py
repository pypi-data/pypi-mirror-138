...
class GA360:
    ...
    def __init__(
        ...
        self.credentials = service_account.Credentials.from_service_account_info(gcp_service_account_key_file)
    ...
    def extract_bigquery_table_to_storage(self, extractor_setting=None):
        ...
        try:
            ...
            extract_job = self.bigquery.extract_table(
                table_ref, destination_uri, location="EU", job_config=extractor_setting  # Location must match that of the source table.
            )  # API request

            extract_job.result()  # Waits for job to complete.
            logging.info("Exported {}:{}.{} to {}".format(self.gcp_project_id, self.gcp_dataset_id, self.gcp_table_id, destination_uri))
    ...
    def _available_blobs(self):
        ...
        return list(
            filter(
                lambda blob: blob.name.find(self.gcp_file_name_mask) >= 0,
                self.storage.get_bucket(self.gcp_bucket_name).list_blobs(prefix=self.gcp_bucket_folder),
            )
        )
    ...
    def merge_files_on_google_storage(self):
        ...
        """
        logging.info(f"Merge all files with mask {self.gcp_file_name_mask} in the folder {self.gcp_bucket_folder}")
        ...
        for i, chunks in enumerate(compose_chunks):
            ...
            self.storage.get_bucket(self.gcp_bucket_name).blob(gcp_merged_file_name_full_path).compose(chunks)
...
class Ga360Builder:
    ...
    def auth(self, file: str = None, private_key: str = None, private_key_id: str = None, client_id: str = None, client_email: str = None):

        self.__auth = (
            GoogleAuthProvider.from_json(file)
            if file
            else GoogleAuthProvider(
    ...
    def build(self, force=False):
        ...
        return self.GA360Handler(
            ...
            self.__dataset_list,
            self.__bucket_name,
            self.__bucket_folder,  # since 20.10.2021, for backward compatibility
            self.__auth.service_key if not self.__service_key else self.__service_key,
            self.__persisting_handler,
        )

    class GA360Handler:
        def __init__(self, project_id: str, dataset: list, bucket_name: str, bucket_folder: str, service_key, persisting_handler):
        ...
        def last_date_in_stored_dataset(self, gcp_dataset_id):
            ...
            return (
                state.get(gcp_dataset_id)["date"]
                if state.get(gcp_dataset_id)
        ...
        def job_sequence_for_dataset(self, extractor: GA360):
            ...
            try:
                ...
                logging.error(f"ERROR: Unexpected error during exporting table stage: {extractor.gcp_table_id}, reason: {error}")
            finally:
                extractor.delete_files_on_google_storage()
        ...
        def fetch_daily_dataset(self, gcp_dataset: Dataset, pointer_date: datetime):
            ...
            logging.info("Processing table for date: " + ga_sessions_table_suffix)
            ...
            for dataset in self.gcp_ga_datasets:
                ...
                with multiprocessing.Pool(multiprocessing.cpu_count()) as pool:
                    ...
                    processed_dates = list(
                        pool.starmap(self.fetch_daily_dataset, zip(gcp_dataset_for_each_process, time_series_to_download))
                    )
                ...
                if processed_dates:
                    last_date_string = TimeSeries.to_date(max([re.split("_|\.", x)[-2] for x in processed_dates])).strftime(
                        TimeSeries.FORMAT
                    )
