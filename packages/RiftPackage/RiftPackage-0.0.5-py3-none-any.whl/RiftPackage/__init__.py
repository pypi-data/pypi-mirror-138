...
with open('Settings.json') as f:
    try:
        ...
        print(colored("There was an error in one of the bot's files!", "red"))
        print(colored(f'\n {e}', 'red'))
        exit(1)
...
async def get_authorization_code():
    while True:
        response = await aioconsole.ainput("ONE TIME EVENT! Go to https://rebrand.ly/authcode and sign in with the bots account and paste the response: ")
...
@app.route('/')
def main():
  return "Thank You For Using Rift, for any more help join our discord with the link in the console!"
...
async def event_ready():
    ...
    print(colored(f'Bots Name: {bot.user.display_name}', 'green'))
    print(colored('Discord: https://riftbot.net/discord', 'green'))
    print(colored('----------------', 'green'))
    member = bot.party.me
