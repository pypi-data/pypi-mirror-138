        ...
        def queued_function():
            for v in range(1, 8):
                ...
                if c is not None:
                    ...
                    if not c._is_aborted:
                        pump_volumes = {1: 0, 2: 0}
                        tstart = c.experiment_start_time
                        tinoc = np.float32(c._inoculation_time)
                        for pump_number in c.pumps:
                            ...
                            if (time.time() - last_pump_time) > self.drying_prevention_pump_period_hrs * 3600:
                                pump_volumes[pump_number] = self.drying_prevention_pump_volume
                        if pump_volumes[1] > 0 or pump_volumes[2] > 0:
                            c.dilute(pump1_volume=pump_volumes[1], pump2_volume=pump_volumes[2], pump3_volume=0)
