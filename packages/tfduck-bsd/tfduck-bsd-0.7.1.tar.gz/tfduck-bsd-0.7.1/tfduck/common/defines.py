...
class Dj44HttpApi(object):
    ...
    def run_sptasks(self, host_name, user_token, task_names, task_param):
        """
        @des: http外部接口运行非定时的任务
        一般用于多个工程的上游依赖，A->B
        当A工程执行完毕后, 执行B工程的所有任务
        @param host_name [必填]: tfduck的地址比如，http://localhost:8000
               user_token[必填]: tfduck的用户token
        ...
        #
        url = os.path.join(host_name, 'run_sptasks')
        data = {
