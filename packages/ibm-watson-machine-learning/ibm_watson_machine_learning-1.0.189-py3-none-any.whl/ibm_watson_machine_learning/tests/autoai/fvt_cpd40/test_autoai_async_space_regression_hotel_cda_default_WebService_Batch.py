...
class TestAutoAIRemote(AbstractTestAutoAIAsync, AbstractTestWebservice, AbstractTestBatch, unittest.TestCase):
    ...
    def test_48a_update_batch_deployment_hardware_spec(self):

        old_deployment_details = AbstractTestBatch.service_batch.get_params()
        old_hardware_spec = old_deployment_details['entity']['hybrid_pipeline_hardware_specs'][0]['hardware_spec']

        update_deployment_props = dict()
        update_deployment_props[
        ...
        self.wml_client.deployments.update(AbstractTestBatch.service_batch.id, update_deployment_props)

        new_hardware_spec = AbstractTestBatch.service_batch.get_params()['entity']['hybrid_pipeline_hardware_specs'][0][
            'hardware_spec']

        self.assertEqual(new_hardware_spec.get('name'), 'S')
        self.assertNotEqual(new_hardware_spec, old_hardware_spec,
                            msg=f"Hybid pipeline software spec was not updated succesfully."
                                f"Previous hardware spec: {old_hardware_spec} \n"
                                f"Updated hardware spec: {new_hardware_spec}")
