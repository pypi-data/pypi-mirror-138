...
class AbstractTestTSAsync(abc.ABC):
    ...
    def test_00_install_xgboost_090(self):
        self.install(f"autoai-ts-libs")
        required_version = "0.90"
        self.install(f"xgboost=={required_version}")
        time.sleep(15)
        try:
            installed_module_version = fallback_to_pip_for_version_check('xgboost')

        except Exception:
            installed_module_version = pkg_resources.get_distribution('xgboost').version

        self.assertEqual(installed_module_version, required_version)
    ...
    def test_99z_uninstall_xgboost_090(self):
        ...
        self.install(f"xgboost=={default_version}")
        time.sleep(15)

        installed_version = pkg_resources.get_distribution("xgboost").version
        self.assertEqual(installed_version, default_version)