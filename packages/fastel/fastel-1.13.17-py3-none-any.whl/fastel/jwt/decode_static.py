...
def decode_static(token: str, pem: bool = False) -> Dict[str, Any]:
    ...
    if unverified["payload"]["env"] == "stg":
        plugin_resp: Tuple[bool, Any] = (
            is_plugin and public_key.stg_cert() or (False, None)
        )
        if plugin_resp[0]:
            signing_keys = plugin_resp[1]
        else:
            signing_keys = stg_cert
    else:
        plugin_resp = is_plugin and public_key.prod_cert() or (False, None)
        if plugin_resp[0]:
            signing_keys = plugin_resp[1]
        else:
            signing_keys = prod_cert
